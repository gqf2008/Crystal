...
impl MockNetwork {
    ...
    /// 处理游戏层发送的事件
    fn handle_game_event(event: NetworkEvent, response_tx: &Sender<NetworkEvent>) {
        ...
        match event {
            ...
            NetworkEvent::StartGameRequest { character_index } => {
                ...
                // 发送开始游戏响应
                let _ = response_tx.send(NetworkEvent::StartGame { delay: 500 });
            ...
            // 移动请求
            NetworkEvent::MoveRequest { .. }
            | NetworkEvent::WalkRequest { .. }
            ...
            // 聊天请求
            NetworkEvent::ChatRequest { message, chat_type } => {
