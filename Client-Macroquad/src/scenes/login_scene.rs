...
impl LoginScene {
    ...
    fn get_or_create_egui_texture(
        ...
        if let Ok(info) = lib.get_or_create_texture(index) {
            if let Some(ref texture) = info.image {
                ...
                texture_cache.insert(key.clone(), handle.clone());
                return Some(handle);
            }
        }
    ...
    fn draw_login_background(&mut self) -> (f32, f32, f32, f32) {
        ...
        // 获取背景纹理并计算居中位置
        let (dialog_w, dialog_h, dialog_x, dialog_y) = if let Some(ref mut lib) = self.prguse_lib {
            if let Ok(info) = lib.get_or_create_texture(1084) {
        ...
        if let Some(ref mut lib) = self.title_lib {
            if let Ok(info) = lib.get_or_create_texture(30) {
        ...
        if let Some(ref mut lib) = self.title_lib {
            if let Ok(info) = lib.get_or_create_texture(31) {
                if let Some(ref tex) = info.image {
        ...
        if let Some(ref mut lib) = self.title_lib {
            if let Ok(info) = lib.get_or_create_texture(32) {
                if let Some(ref tex) = info.image {
    ...
    fn draw_new_account_background(&mut self) {
        ...
        if let Some(ref mut lib) = self.prguse_lib {
            if let Ok(info) = lib.get_or_create_texture(63) {
                if let Some(ref bg_tex) = info.image {
                    let w = bg_tex.width();
                    let h = bg_tex.height();
                    let x = (screen_w - w) / 2.0;
    ...
    fn draw_change_password_background(&mut self) {
        ...
        // 获取背景纹理并计算居中位置
        if let Some(ref mut lib) = self.prguse_lib {
            if let Ok(info) = lib.get_or_create_texture(50) {
                if let Some(ref bg_tex) = info.image {
                    let w = bg_tex.width();
                    let h = bg_tex.height();
                    let x = (screen_w - w) / 2.0;
    ...
    fn draw_login_ui(&mut self, ctx: &egui::Context) {
        ...
        egui::Area::new(egui::Id::new("login_dialog_inputs"))
            ...
            .show(ctx, |ui| {
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 85.0, rect.min.y + 85.0), egui::vec2(136.0, 15.0)),
                ...
                let password_response = ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 85.0, rect.min.y + 108.0), egui::vec2(136.0, 15.0)),
                ...
                if self.draw_image_button_abs(ui, ctx, "title", 320, 321, 322, egui::pos2(rect.min.x + 227.0, rect.min.y + 81.0)) {
                    self.on_login_clicked();
                }
                
                if self.draw_image_button_abs(ui, ctx, "title", 323, 324, 325, egui::pos2(rect.min.x + 60.0, rect.min.y + 163.0)) {
                    self.show_new_account = true;
                    self.show_login_dialog = false;
                }
                
                if self.draw_image_button_abs(ui, ctx, "title", 326, 327, 328, egui::pos2(rect.min.x + 166.0, rect.min.y + 163.0)) {
                    self.show_change_password = true;
                    self.show_login_dialog = false;
                }
                
                if self.draw_image_button_abs(ui, ctx, "title", 332, 333, 334, egui::pos2(rect.min.x + 60.0, rect.min.y + 189.0)) {
                    println!("⚠ InputKeyDialog not implemented");
                }
                
                if self.draw_image_button_abs(ui, ctx, "title", 329, 330, 331, egui::pos2(rect.min.x + 166.0, rect.min.y + 189.0)) {
                    std::process::exit(0);
                }
            });
    }
    ...
    fn draw_new_account_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Area::new(egui::Id::new("new_account_dialog"))
            ...
            .show(ctx, |ui| {
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 103.0), egui::vec2(136.0, 15.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 129.0), egui::vec2(136.0, 15.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 155.0), egui::vec2(136.0, 15.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 189.0), egui::vec2(136.0, 15.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 215.0), egui::vec2(136.0, 15.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 250.0), egui::vec2(190.0, 15.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 276.0), egui::vec2(190.0, 15.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 226.0, rect.min.y + 311.0), egui::vec2(136.0, 15.0)),
                ...
                if self.draw_image_button_abs(ui, ctx, "title", 200, 201, 202, egui::pos2(rect.min.x + 135.0, rect.min.y + 425.0)) {
                    self.on_create_account();
                }
                
                if self.draw_image_button_abs(ui, ctx, "title", 203, 204, 205, egui::pos2(rect.min.x + 409.0, rect.min.y + 425.0)) {
    ...
    fn draw_change_password_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Area::new(egui::Id::new("change_password_dialog"))
            ...
            .show(ctx, |ui| {
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 178.0, rect.min.y + 75.0), egui::vec2(136.0, 18.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 178.0, rect.min.y + 113.0), egui::vec2(136.0, 18.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 178.0, rect.min.y + 151.0), egui::vec2(136.0, 18.0)),
                ...
                ui.put(
                    egui::Rect::from_min_size(egui::pos2(rect.min.x + 178.0, rect.min.y + 188.0), egui::vec2(136.0, 18.0)),
                ...
                if self.draw_image_button_abs(ui, ctx, "title", 107, 108, 109, egui::pos2(rect.min.x + 80.0, rect.min.y + 236.0)) {
                    self.on_change_password();
                }
                
                if self.draw_image_button_abs(ui, ctx, "title", 110, 111, 112, egui::pos2(rect.min.x + 222.0, rect.min.y + 236.0)) {
            ...
            .show(ctx, |ui| {
                ...
                match self.message_box_buttons {
                    MessageBoxButtons::Ok => {
                        if self.draw_image_button_abs(ui, ctx, "title", 200, 201, 202, egui::pos2(rect.min.x + 360.0, rect.min.y + 157.0)) {
                            self.show_message_box = false;
                        }
                    }
                    MessageBoxButtons::OkCancel => {
                        if self.draw_image_button_abs(ui, ctx, "title", 200, 201, 202, egui::pos2(rect.min.x + 260.0, rect.min.y + 157.0)) {
                            self.show_message_box = false;
                        }
                        if self.draw_image_button_abs(ui, ctx, "title", 203, 204, 205, egui::pos2(rect.min.x + 360.0, rect.min.y + 157.0)) {
                            self.show_message_box = false;
                        }
                    }
                    MessageBoxButtons::YesNo => {
                        if self.draw_image_button_abs(ui, ctx, "title", 206, 207, 208, egui::pos2(rect.min.x + 260.0, rect.min.y + 157.0)) {
                            self.show_message_box = false;
                        }
                        if self.draw_image_button_abs(ui, ctx, "title", 210, 211, 212, egui::pos2(rect.min.x + 360.0, rect.min.y + 157.0)) {
                            self.show_message_box = false;
                        }
                    }
                }
...
impl SceneHandler for LoginScene {
    ...
    fn render(&mut self) -> GameResult {
        ...
        egui_macroquad::ui(|ctx| {
            ...
            static FIRST_EGUI: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(true);
            if FIRST_EGUI.swap(false, std::sync::atomic::Ordering::Relaxed) {
                println!("🎨 egui pixels_per_point: {}", ctx.pixels_per_point());
                println!("🎨 egui native_pixels_per_point: {:?}", ctx.native_pixels_per_point());
            }
            ...
            egui::Window::new("🔍 坐标系统调试")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.label(format!("  macroquad(100) = egui({})", 100.0 / pixels_per_point));
                    
                    ui.separator();
                    ui.label(format!("显示登录框: {}", self.show_login_dialog));
                    ui.label(format!("资源已加载: {}", self.resources_loaded));
                });
        });
