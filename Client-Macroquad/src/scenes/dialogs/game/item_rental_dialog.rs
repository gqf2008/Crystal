...
impl ItemRentalDialogHybrid {
    ...
    pub fn draw(&mut self, screen_w: f32, screen_h: f32, mouse_pos: Vec2,
        ...
        for (i, entry) in self.items.iter().enumerate() {
            ...
            // 槽位编号
            draw_text_cn(&format!("槽位 {}", i + 1), dialog_x + 15.0, y + 5.0, 12.0,
                Color::from_rgba(150, 150, 150, 255));

            // 物品名称
            if let Some(ref item) = entry.item {
                let name = item.info.as_ref().map(|info| info.name.as_str()).unwrap_or("未知物品");
                draw_text_cn(name, dialog_x + 15.0, y + 18.0, 12.0,
                    Color::from_rgba(200, 200, 200, 255));
            } else {
                draw_text_cn("[空]", dialog_x + 60.0, y + 18.0, 11.0,
        ...
        draw_text_cn(&format!("租赁费用: {} 金币 | 期限: {} 天", self.fee, self.period),
            dialog_x + 15.0, info_y + 5.0, 13.0, WHITE);

        // 锁定状态
        let lock_y = info_y + 20.0;
        let my_lock = if self.locked_by_me { "[已锁定]" } else { "[未锁定]" };
        let partner_lock = if self.locked_by_partner { "[已锁定]" } else { "[未锁定]" };
        draw_text_cn(&format!("我: {}  |  {}: {}", my_lock, self.partner_name, partner_lock),
            dialog_x + 15.0, lock_y + 5.0, 12.0,
        ...
        let lock_label = if self.locked_by_me { "解锁" } else { "锁定" };
        draw_rectangle(lock_x, btn_y, btn_w, btn_h,
            if self.locked_by_me { Color::from_rgba(160, 80, 80, 255) }
            else { Color::from_rgba(80, 160, 80, 255) });
        draw_text_cn(lock_label, lock_x + 20.0, btn_y + 7.0, 14.0, WHITE);

        if left_clicked && mouse_pos.x >= lock_x && mouse_pos.x <= lock_x + btn_w
        ...
        draw_rectangle(confirm_x, btn_y, btn_w, btn_h,
            if can_confirm { Color::from_rgba(80, 120, 220, 255) }
            else { Color::from_rgba(60, 60, 60, 255) });
        draw_text_cn("确认", confirm_x + 20.0, btn_y + 7.0, 14.0, WHITE);

        if left_clicked && can_confirm && mouse_pos.x >= confirm_x && mouse_pos.x <= confirm_x + btn_w
            && mouse_pos.y >= btn_y && mouse_pos.y <= btn_y + btn_h {
