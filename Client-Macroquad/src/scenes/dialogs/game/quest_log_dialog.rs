...
impl QuestLogDialogHybrid {
    ...
    pub fn update_quest_progress_from_text(&mut self, quest_id: u32, progress_text: &str) {
        if let Some(quest) = self.quests.iter_mut().find(|q| q.id == quest_id) {
            let tasks: Vec<&str> = progress_text
                .split(';')
    ...
    fn draw_background(&self) {
        ...
        let label = format!("List: {}/{}", self.quests.len(), MAX_CONCURRENT_QUESTS);
        draw_text_cn(
            &label,
    ...
    fn draw_quest_list(&mut self, mouse_pos: Vec2) {
        ...
        for (group_name, indices) in groups.iter() {
            ...
            let expanded = self.expanded_groups.is_empty()
                || self.expanded_groups.iter().any(|g| g == group_name);
            if expanded {
        ...
        for (group_name, indices) in groups.iter() {
            ...
            let header_visible =
                header_rect.y + header_rect.h > list_top && header_rect.y < list_bottom;
            ...
            for quest_idx in indices.iter().copied() {
                ...
                let item_visible =
                    item_rect.y + item_rect.h > list_top && item_rect.y < list_bottom;
    ...
    fn draw_quest_details(&mut self, _mouse_pos: Vec2) {
        let Some(selected_idx) = self.selected_quest else {
            return;
        };
        let Some(quest) = self.quests.get(selected_idx) else {
            return;
        };
        ...
        if !quest.npc_name.is_empty() {
            draw_text_cn(
                &format!("来源: {}", quest.npc_name),
                panel_x + 10.0,
                panel_y + 70.0,
                10.0,
                Color::from_rgba(180, 180, 255, 255),
            );
        }

        // 等级需求
        if quest.level_required > 0 {
            let lvl_color = if quest.level_required > 0 {
                Color::from_rgba(255, 200, 100, 255)
            } else {
                GRAY
            };
            draw_text_cn(
                &format!("需要等级: {}", quest.level_required),
                panel_x + 10.0,
                panel_y + 85.0,
                10.0,
                lvl_color,
        ...
        if quest.max_progress > 0 {
            ...
            draw_text_cn(
                &progress_text,
                progress_x + (progress_w - dims.width) / 2.0,
                progress_y + 9.0,
                9.0,
                WHITE,
            );
        ...
        // 奖励信息
        let reward_y = if quest.max_progress > 0 {
            panel_y + 118.0
        } else {
            panel_y + 100.0
        };
        let mut reward_text = format!(
            "奖励: {} 经验, {} 金币",
            quest.rewards.experience, quest.rewards.gold
        );
        if !quest.rewards.items.is_empty() {
            let item_names: Vec<&str> = quest
                .rewards
                .items
                .iter()
                .map(|i| i.name.as_str())
        ...
        for (i, (quest_id, remaining)) in self.completion_notifications.iter().enumerate() {
            if let Some(quest) = self.quests.iter().find(|q| q.id == *quest_id) {
                ...
                draw_rectangle(
                    ...
                    text_w,
                    text_h,
                    Color::from_rgba(20, 40, 20, (alpha as f32 * 0.8).min(255.0) as u8),
                );
