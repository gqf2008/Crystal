...
impl ChatControlBar {
    ...
    fn draw_control_bar(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) -> (bool, bool) {
        ...
        if let Some(info) = LibraryName::Prguse.get_egui_texture(ctx, bg_index) {
            ...
            let default_rect =
                egui::Rect::from_min_size(self.position, egui::vec2(default_width, 15.0));
    ...
    fn draw_function_buttons(
        ...
        let size_clicked =
            self.draw_clickable_button(ui, ctx, bg_rect, size_btn_x, 1.0, 2057, false);
        ...
        let settings_clicked =
            self.draw_clickable_button(ui, ctx, bg_rect, settings_btn_x, 1.0, 2060, false);
