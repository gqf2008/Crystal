...
impl CharacterDialogHybrid {
    ...
    pub fn update_and_draw(&mut self) {
        ...
        if is_key_pressed(KeyCode::Key1) {
            self.switch_tab(CharacterTabHybrid::Character);
        }
        if is_key_pressed(KeyCode::Key2) {
            self.switch_tab(CharacterTabHybrid::Status);
        }
        if is_key_pressed(KeyCode::Key3) {
            self.switch_tab(CharacterTabHybrid::State);
        }
        if is_key_pressed(KeyCode::Key4) {
            self.switch_tab(CharacterTabHybrid::Skills);
        }
        ...
        draw_text_cn(
            &self.name,
            self.position.x + 132.0 - self.name.len() as f32 * 4.0,
        ...
        if let Some(ref guild) = self.guild {
            draw_text_cn(
                guild,
                self.position.x + 132.0 - guild.len() as f32 * 3.0,
                self.position.y + 45.0,
                12.0,
                GOLD,
            );
        }
    ...
    fn draw_character_page(&mut self, mouse: Vec2) {
        ...
        if let Some(ref tex) = self.page_textures[0] {
            draw_texture(
                tex,
                self.position.x + Self::PAGE_OFFSET.0,
                self.position.y + Self::PAGE_OFFSET.1,
                WHITE,
            );
        }
    ...
    fn draw_status_page(&self) {
        ...
        draw_text_cn(
            &format!("{}/{}", self.stats.health.0, self.stats.health.1),
            x,
            y,
            12.0,
            WHITE,
        ...
        draw_text_cn(
            &format!("{}/{}", self.stats.mana.0, self.stats.mana.1),
            x,
            y + h,
            12.0,
            WHITE,
        ...
        draw_text_cn(
            &format!("{}-{}", self.stats.ac.0, self.stats.ac.1),
            x,
            y + h * 2.0,
            12.0,
            WHITE,
        ...
        draw_text_cn(
            &format!("{}-{}", self.stats.mac.0, self.stats.mac.1),
            x,
            y + h * 3.0,
            12.0,
            WHITE,
        ...
        draw_text_cn(
            &format!("{}-{}", self.stats.dc.0, self.stats.dc.1),
            x,
            y + h * 4.0,
            12.0,
            WHITE,
        ...
        draw_text_cn(
            &format!("{}-{}", self.stats.mc.0, self.stats.mc.1),
            x,
            y + h * 5.0,
            12.0,
            WHITE,
        ...
        draw_text_cn(
            &format!("{}-{}", self.stats.sc.0, self.stats.sc.1),
            x,
            y + h * 6.0,
            12.0,
            WHITE,
    ...
    fn draw_state_page(&self) {
        ...
        draw_text_cn(
            "经验: 0%",
            self.position.x + 134.0,
            self.position.y + 110.0,
            12.0,
            WHITE,
        );
    }
    ...
    fn draw_character_model(&mut self) {
        ...
        // 如果没有任何装备有外观，显示一个占位符
        let has_any_visual = self.equipment[0]
            .as_ref()
            .is_some_and(|e| e.state_image > 0)
            || self.equipment[1]
                .as_ref()
                .is_some_and(|e| e.state_image > 0)
            || self.equipment[2]
