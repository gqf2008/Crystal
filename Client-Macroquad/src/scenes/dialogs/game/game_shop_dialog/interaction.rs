...
impl GameShopDialogHybrid {
    ...
    pub(super) fn draw_search_box(&mut self, pos: Vec2) {
        ...
        let hovered = mouse_pos.x >= search_x
            && mouse_pos.x <= search_x + search_w
            && mouse_pos.y >= search_y
            && mouse_pos.y <= search_y + search_h;
    ...
    pub(super) fn draw_preview_window(&mut self, pos: Vec2) {
        if let Some(idx) = self.preview_item {
            ...
            if draw_library_button_with_offset(
                ...
                self.preview_direction = if self.preview_direction == 1 {
                    8
                } else {
            ...
            if draw_library_button_with_offset(
                ...
                self.preview_direction = if self.preview_direction == 8 {
                    1
                } else {
                    self.preview_direction + 1
                };
            ...
            // 方向显示
            draw_text_cn(
                &format!("方向: {}/8", self.preview_direction),
                preview_x + 105.0,
                dir_y + 14.0,
                10.0,
