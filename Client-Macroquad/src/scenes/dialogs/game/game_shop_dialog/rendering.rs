...
use super::super::native_ui_utils::{
    draw_library_button_with_offset, draw_library_image_with_offset, mouse_pos as ui_mouse_pos,
};
use super::dialog::GameShopDialogHybrid;
use super::types::{GameShopBuyAction, ShopCategoryHybrid, ShopClassHybrid, ShopSectionHybrid};
...
impl GameShopDialogHybrid {
    ...
    pub(super) fn draw_section_tabs(&mut self, pos: Vec2) {
        ...
        for (i, section) in ShopSectionHybrid::ALL.iter().enumerate() {
            ...
            let hovered = Rect::new(tab_x, tab_y, Self::SECTION_TAB_W, Self::SECTION_TAB_H)
                .contains(mouse_pos);
    ...
    /// 备用分类标签绘制
    pub(super) fn draw_fallback_section_tab(
        &self,
        x: f32,
        y: f32,
        section: &ShopSectionHybrid,
        selected: bool,
        hovered: bool,
    ) {
    ...
    /// 备用职业标签绘制
    pub(super) fn draw_fallback_class_tab(
        &self,
        x: f32,
        y: f32,
        class: &ShopClassHybrid,
        selected: bool,
        hovered: bool,
    ) {
        let bg_color = if selected {
    ...
    pub(super) fn draw_category_list(&mut self, pos: Vec2) {
        ...
        let filter_bg_hit_rect =
            draw_library_image_with_offset(LibraryName::Title, 769, filter_bg_pos, WHITE)
                .unwrap_or_else(|| {
                    // 备用背景
                    draw_rectangle(
                        filter_bg_pos.x,
        ...
        if !self.dragging {
            ...
            if wheel_y.abs() > 0.0 && filter_bg_hit_rect.contains(mouse_pos) {
                let max_scroll = self
                    .categories
                    .len()
                    .saturating_sub(Self::CATEGORY_MAX_VISIBLE);
                if max_scroll > 0 {
                    if wheel_y > 0.0 {
    ...
    pub(super) fn draw_category_scrollbar(&mut self, pos: Vec2) {
        ...
        let scroll_ratio = if self.categories.len() > Self::CATEGORY_MAX_VISIBLE {
            self.category_scroll as f32
                / (self.categories.len() - Self::CATEGORY_MAX_VISIBLE) as f32
        } else {
            0.0
        };
    ...
    pub(super) fn draw_item_cell(&mut self, x: f32, y: f32, item_idx: usize) {
        ...
        // 检测图标区域悬停（用于显示物品提示框）
        let mouse_pos = ui_mouse_pos();
        let icon_hovered = mouse_pos.x >= icon_x
            && mouse_pos.x <= icon_x + icon_w
            && mouse_pos.y >= icon_y
            && mouse_pos.y <= icon_y + icon_h;
        if icon_hovered {
        ...
        let is_previewable = matches!(
            item.category,
            ShopCategoryHybrid::Weapon | ShopCategoryHybrid::Armor
        );
    ...
    pub(super) fn draw_payment_options(&mut self, pos: Vec2) {
        ...
        let gold_hovered = mouse_pos.x >= gold_x + gold_off_x
            && mouse_pos.x <= gold_x + gold_off_x + 120.0
            && mouse_pos.y >= gold_y + gold_off_y
            && mouse_pos.y <= gold_y + gold_off_y + gold_h.max(checkbox_size);
        ...
        let credit_hovered = mouse_pos.x >= credit_x + credit_off_x
            && mouse_pos.x <= credit_x + credit_off_x + 130.0
            && mouse_pos.y >= credit_y + credit_off_y
            && mouse_pos.y <= credit_y + credit_off_y + credit_h.max(checkbox_size);
