...
impl RefineDialogHybrid {
    ...
    pub fn draw(
        ...
        let dialog_w = 300.0;
        let dialog_h =
            title_h + (self.stats.len() as f32) * stat_h + btn_h * 2.0 + padding * 4.0 + 25.0;
        ...
        draw_rectangle(
            ...
            dialog_w,
            dialog_h,
            Color::from_rgba(30, 25, 40, 230),
        );
        draw_text_cn(
            &format!("精炼: {}", self.item_name),
            dialog_x + 15.0,
            dialog_y + 10.0,
            16.0,
        ...
        for (i, stat) in self.stats.iter().enumerate() {
            ...
            let line = if delta > 0 {
                format!(
                    "{}: {} → {} (+{})",
                    stat.name, stat.current, stat.after, delta
                )
            } else {
        ...
        draw_text_cn(
            &format!(
                "材料: {} {}/{}",
                self.material_name, self.material_have, self.material_need
            ),
            dialog_x + 15.0,
            mat_y + 5.0,
            13.0,
