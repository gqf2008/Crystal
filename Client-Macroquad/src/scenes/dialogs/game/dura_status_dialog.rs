...
impl DuraStatusDialogHybrid {
    ...
    pub fn draw(
        ...
        for (i, entry) in self.items.iter().enumerate() {
            ...
            let ratio = if entry.max > 0 {
                entry.current as f32 / entry.max as f32
            } else {
                0.0
            };
        ...
        let mouse_over_close = mouse_pos.x >= dialog_x + 60.0
            && mouse_pos.x <= dialog_x + dialog_w - 60.0
            && mouse_pos.y >= close_y
            && mouse_pos.y <= close_y + 18.0;
