...
impl ChatDialog {
    ...
    fn draw_chat(&self, ui: &mut egui::Ui, ctx: &egui::Context) -> egui::Rect {
        ...
        let base_rect = if let Some(info) = LibraryName::Prguse.get_egui_texture(ctx, bg_index) {
            if let Some(bg_texture) = info.egui_texture {
                ...
                let message_area_width = if self.resolution_index == 0 {
                    380.0
                } else {
                    600.0
                };
                ...
                let msg_bg_rect = egui::Rect::from_min_size(
                    egui::pos2(
                        bg_rect.min.x + message_area_x,
                        bg_rect.min.y + message_area_y,
                    ),
                ...
                ui.painter()
                    .rect_filled(r, 2.0, egui::Color32::from_rgb(50, 50, 50));
    ...
    fn draw_message_background(&self, ui: &mut egui::Ui, base_rect: &egui::Rect) {
        ...
        let msg_bg_rect = egui::Rect::from_min_size(
            egui::pos2(
                base_rect.min.x + message_area_x,
                base_rect.min.y + message_area_y,
            ),
            egui::vec2(message_area_width, message_area_height),
        );
        ...
        ui.painter()
            .rect_filled(msg_bg_rect, 0.0, egui::Color32::from_rgb(30, 30, 30));
    }
    ...
    fn draw_scroll_buttons(
        ...
        let countbar_x = if self.resolution_index == 0 {
            398.0
        } else {
            622.0
        };
        let posbar_x = if self.resolution_index == 0 {
            395.0
        } else {
            619.0
        };
        ...
        let countbar_height =
            if let Some(info) = LibraryName::Prguse.get_egui_texture(ctx, countbar_index) {
    ...
    fn draw_input(&mut self, ui: &mut egui::Ui, base_rect: &egui::Rect, ctx: &egui::Context) {
        ...
        // 绘制白色背景，让输入框更明显
        ui.painter()
            .rect_filled(input_rect, 1.0, egui::Color32::WHITE);
        ...
        if response.lost_focus() && ctx.input(|i| i.key_pressed(egui::Key::Enter)) {
            if !self.input_text.is_empty() {
                ...
                // 添加到本地消息列表
                self.add_message(format!("[我] {}", self.input_text), egui::Color32::WHITE);
