...
pub fn update(ctx: &mut GameContext, _delay_time: f32) -> GameResult {
    ...
    let mut mock_spawns: Vec<(
        u32,
        crate::network::handlers::ObjectType,
        crate::resources::LibraryName,
        i32,
        i32,
        i32,
    )> = Vec::new();
    ...
    let (local_player_object_id, local_player_name) = local_player_entity
        .and_then(|e| {
            ctx.world
                .get::<&crate::components::PlayerData>(e)
                .ok()
    ...
    for event in ctx.events().network_events() {
        match event {
            ...
            NetworkEvent::MagicLearned { magic, hero } => {
                tracing::debug!(
                    "✨ Magic learned: {:?} level={} hero={}",
                    magic.spell,
                    magic.level,
                    hero
                );
                if *hero {
                    hero_magic_learned.push((
                        magic.spell as u8,
                        magic.level,
                        magic.experience as u32,
                        magic.key,
                    ));
                } else if let Some(e) = local_player_entity {
                    if let Ok(mut magic_list) =
                        ctx.world.get::<&mut crate::components::spell::MagicList>(e)
                    {
                        NetworkApplySystem::update_learned_magic(
                            &mut magic_list,
                            magic.spell as u8,
                            magic.level,
                            magic.experience as u32,
            ...
            NetworkEvent::MagicLeveledUp {
                ...
                if let Some(e) = local_player_entity {
                    if let Ok(mut magic_list) =
                        ctx.world.get::<&mut crate::components::spell::MagicList>(e)
                    {
                        NetworkApplySystem::update_magic_level(
                            &mut magic_list,
                            *spell as u8,
                            *level,
                        );
            ...
            NetworkEvent::MagicDelayReceived {
                object_id: _,
                spell,
                delay,
            } => {
            ...
            NetworkEvent::ObjectEffectReceived {
                object_id,
                effect,
                effect_type,
                delay_time,
                ...
                tracing::trace!(
                    "✨ Object {} effect={} type={} delay={} duration={}",
                    object_id,
                    effect,
                    effect_type,
                    delay_time,
            ...
            NetworkEvent::ObjectProjectileReceived {
                spell,
                source,
                destination,
            } => {
            ...
            NetworkEvent::SpellToggled {
                spell,
                can_use,
                hero,
            } => {
                tracing::trace!(
                    "🔄 Spell toggle: {:?} can_use={} hero={}",
                    spell,
                    can_use,
                    hero
                );
                if *hero {
                    ...
                    if let Ok(mut magic_list) =
                        ctx.world.get::<&mut crate::components::spell::MagicList>(e)
                    {
                        NetworkApplySystem::update_spell_toggle(
                            &mut magic_list,
                            (*spell).into(),
                            *can_use,
                        );
            ...
            NetworkEvent::ObjectHeroSpawned { packet } => {
                ...
                if local_player_name
                    .as_ref()
                    .is_some_and(|n| *n == packet.owner_name)
                {
                    if let Some(e) = local_player_entity {
            ...
            NetworkEvent::ObjectTeleportingOut {
                object_id,
                teleport_type: _,
            } => {
                hidden_objects.push(*object_id);
            ...
            NetworkEvent::ObjectSatDown {
                object_id,
                direction,
                location,
            } => {
                tracing::trace!(
                    "🪑 Object {} sat down at ({},{}) dir={}",
                    object_id,
                    location.0,
                    location.1,
                    direction
            ...
            NetworkEvent::NewMapInfoReceived { packet } => {
                tracing::trace!(
                    "🗺️ New map info received: idx={} title={}",
                    packet.map_index,
                    packet.title
                );
            }
            ...
            NetworkEvent::TimeOfDayChanged { time_of_day } => {
                ...
                if let Some((entity, _)) = ctx
                    .world
                    .query::<(hecs::Entity, &TimeOfDay)>()
                    .iter()
                    .next()
                {
            ...
            NetworkEvent::PlayerUpdated {
                ...
                player_appearance_updates.push((
                    *object_id,
                    *weapon,
                    *weapon_effect,
                    *armor,
            ...
            // ===== 战斗扩展 =====
            NetworkEvent::DuraChanged {
                unique_id,
                durability,
            } => {
                dura_changes.push((*unique_id, *durability));
            }
            NetworkEvent::PlayerPoisoned {
                object_id,
                poison_type,
            } => {
                poisoned_objects.push((*object_id, *poison_type));
            }
            NetworkEvent::ObjectPoisonedEvent {
                object_id,
                poison_type,
            } => {
                poisoned_objects.push((*object_id, *poison_type));
            }
            NetworkEvent::RangeAttacked {
                target_id,
                target_x,
                target_y,
                spell,
            ...
            NetworkEvent::ObjectDashAttacked {
                object_id,
                location_x,
                location_y,
                direction: _,
            ...
            NetworkEvent::ItemDropped {
                ...
                tracing::trace!(
                    "📦 Item dropped: uid={} count={} success={}",
                    unique_id,
                    count,
                    success
                );
            ...
            NetworkEvent::ItemSealed {
                ...
                let Some(e) = local_player_entity else {
                    continue;
                };
            ...
            NetworkEvent::TradeItemDeposited { from_slot, success } => {
                tracing::trace!(
                    "🤝 Trade item deposited: from={} success={}",
                    from_slot,
                    success
                );
            }
            NetworkEvent::TradeItemRetrieved { from_slot, success } => {
                tracing::trace!(
                    "🤝 Trade item retrieved: from={} success={}",
                    from_slot,
                    success
                );
            }
            ...
            NetworkEvent::TradeItemAdded { items } => {
                tracing::trace!(
                    "📦 Trade item added: {} items",
                    items.iter().filter(|i| i.is_some()).count()
                );
            }
            ...
            NetworkEvent::GuildMemberUpdated { name, rank, online } => {
                tracing::trace!(
                    "🏰 Guild member updated: {} rank={} online={}",
                    name,
                    rank,
                    online
                );
            ...
            NetworkEvent::GuildStorageGoldChanged { delta, total } => {
                tracing::trace!(
                    "🏰 Guild storage gold changed: delta={} total={}",
                    delta,
                    total
                );
            }
            NetworkEvent::GuildStorageItemChanged { change_type, slot } => {
                tracing::trace!(
                    "🏰 Guild storage item changed: type={} slot={}",
                    change_type,
                    slot
                );
            }
            ...
            NetworkEvent::GuildTerritoryPageReceived { territories } => {
                tracing::trace!(
                    "🏰 Guild territory page received: {} territories",
                    territories.len()
                );
            }
            ...
            NetworkEvent::NPCRefineReceived { rate, refining } => {
                tracing::trace!(
                    "🔨 NPC refine received (rate={}, refining={})",
                    rate,
                    refining
                );
            }
            ...
            NetworkEvent::AwakeningNeedMaterialsReceived { item_id, materials } => {
                tracing::trace!(
                    "🌟 Awakening need materials: item_id={} mats={}",
                    item_id,
                    materials.len()
                );
            }
            NetworkEvent::AwakeningLockedItemReceived { unique_id, locked } => {
                tracing::trace!(
                    "🌟 Awakening locked item: uid={} locked={}",
                    unique_id,
                    locked
                );
            }
            NetworkEvent::AwakeningReceived { result, remove_id } => {
                tracing::trace!(
                    "🌟 Awakening received: result={} remove_id={}",
                    result,
                    remove_id
                );
            }
            NetworkEvent::NPCPearlGoodsReceived { rate, item_list } => {
                tracing::trace!(
                    "🔮 NPC pearl goods received: rate={} items={}",
                    rate,
                    item_list.len()
                );
            }
            ...
            NetworkEvent::HeroCreateRequested { can_create_class } => {
                tracing::trace!(
                    "🦸 Hero create requested: {} classes",
                    can_create_class.len()
                );
            }
            ...
            NetworkEvent::HeroAutoPotItemSet { slot, item_id } => {
                tracing::trace!(
                    "🦸 Hero auto pot item set: slot={} item_id={}",
                    slot,
                    item_id
                );
            }
            ...
            NetworkEvent::NPCConsignEvent => {
                tracing::trace!("🏪 NPC consign event");
            }
            NetworkEvent::NPCMarketEvent2 { .. } => {
                tracing::trace!("🏪 NPC market event 2");
            }
            NetworkEvent::NPCMarketPageEvent2 { .. } => {
                tracing::trace!("🏪 NPC market page event 2");
            }
            NetworkEvent::ConsignItemEvent { .. } => {
                tracing::trace!("📦 Consign item event");
            }
            ...
            NetworkEvent::NewIntelligentCreatureReceived { creature_type: _ } => {
                tracing::trace!("🐾 New intelligent creature");
            }
            NetworkEvent::IntelligentCreatureListUpdated { creatures } => {
                tracing::trace!("🐾 Creature list updated: {} creatures", creatures.len());
            }
            NetworkEvent::IntelligentCreatureRenameEnabled { can_rename } => {
                tracing::trace!("🐾 Creature rename enabled: {}", can_rename);
            }
            NetworkEvent::IntelligentCreaturePickupReceived { enabled } => {
                tracing::trace!("🐾 Creature pickup: {}", enabled);
            }
            NetworkEvent::MarriageRequested2 { requester } => {
                tracing::trace!("💒 Marriage requested by {}", requester);
            }
            NetworkEvent::DivorceRequested2 { lover_name } => {
                tracing::trace!("💔 Divorce requested by {}", lover_name);
            }
            NetworkEvent::MentorRequested2 { mentor_name } => {
                tracing::trace!("🎓 Mentor requested by {}", mentor_name);
            }
            ...
            NetworkEvent::RentalItemsReceived { items: _ } => {
                tracing::trace!("📦 Rental items received");
            }
            NetworkEvent::ItemRentalRequested => {
                tracing::trace!("📦 Item rental requested");
            }
            NetworkEvent::ItemRentalFeeReceived { fee } => {
                tracing::trace!("📦 Rental fee: {}", fee);
            }
            NetworkEvent::ItemRentalPeriodReceived { period } => {
                tracing::trace!("📦 Rental period: {}", period);
            }
            NetworkEvent::RentalItemDeposited {
                unique_id: _,
                success: _,
            } => {
                tracing::trace!("📦 Rental item deposited");
            }
            NetworkEvent::RentalItemRetrieved {
                unique_id: _,
                success: _,
            } => {
                tracing::trace!("📦 Rental item retrieved");
            }
            NetworkEvent::RentalItemUpdated { fee, period } => {
                tracing::trace!("📦 Rental item updated: fee={} period={}", fee, period);
            }
            NetworkEvent::ItemRentalCancelled { success } => {
                tracing::trace!("📦 Item rental cancelled: success={}", success);
            }
            NetworkEvent::ItemRentalLocked { locked } => {
                tracing::trace!("📦 Item rental locked: {}", locked);
            }
            NetworkEvent::ItemRentalPartnerLocked { locked } => {
                tracing::trace!("📦 Rental partner locked: {}", locked);
            }
            NetworkEvent::ItemRentalConfirmable { can_confirm } => {
                tracing::trace!("📦 Item rental confirmable: {}", can_confirm);
            }
            NetworkEvent::ItemRentalConfirmed { success } => {
                tracing::trace!("📦 Item rental confirmed: success={}", success);
            }
            ...
            NetworkEvent::ReincarnationRequested => {
                tracing::trace!("🔄 Reincarnation requested");
            }
            NetworkEvent::ReincarnationCancelled => {
                tracing::trace!("🔄 Reincarnation cancelled");
            }
            NetworkEvent::RankingsReceived { rankings } => {
                tracing::trace!("🏆 Rankings received: {} entries", rankings.len());
            }
            ...
            NetworkEvent::TimerSet { timer_id, seconds } => {
                tracing::trace!("⏱️ Timer {} set: {}s", timer_id, seconds);
            }
            NetworkEvent::TimerExpired { timer_id } => {
                tracing::trace!("⏱️ Timer {} expired", timer_id);
            }
            NetworkEvent::NoticeUpdated { notice } => {
                tracing::trace!("📢 Notice updated: {}", notice);
            }
            NetworkEvent::RollReceivedEvent { value } => {
                tracing::trace!("🎲 Roll received: {}", value);
            }
            NetworkEvent::CompassUpdated { location } => {
                tracing::trace!("🧭 Compass updated: {:?}", location);
            }
            NetworkEvent::BrowserOpened { url } => {
                tracing::trace!("🌐 Browser opened: {}", url);
            }
            ...
            NetworkEvent::MapEffectReceived {
                ...
                tracing::trace!(
                    "🌈 Map effect received: effect={} loc=({},{}) value={}",
                    effect,
                    location_x,
                    location_y,
                    value
            ...
            NetworkEvent::OutputMessageReceived {
                ...
                tracing::trace!("💬 Message (type={}): {}", message_type, message);
            }
            ...
            NetworkEvent::NewRecipeInfoReceived { recipe_id } => {
                tracing::trace!("📜 New recipe info received: id={}", recipe_id);
            }
            NetworkEvent::ChatItemStatsReceived { .. } => {
                tracing::trace!("💬 Chat item stats received");
            }
            ...
            NetworkEvent::ElementalSet {
                ...
                tracing::trace!(
                    "🔥 Elemental set: object={} enabled={} element={} value={} expire={}",
                    object_id,
                    enabled,
                    element,
                    value,
            ...
            NetworkEvent::CraftItemRequest {
                ...
                tracing::trace!(
                    "🔨 Craft item request: unique_id={}, count={}, slots={:?}",
                    recipe_unique_id,
                    count,
                    slots
                );
    ...
    for (spell, source, destination) in projectiles {
        ...
        if let (Some(from), Some(to)) = (from_pos, to_pos) {
            if let Some(projectile_type) = NetworkApplySystem::spell_to_projectile_type(spell) {
                ctx.events_mut().send_presentation(
    ...
    for object_id in delayed_explosions {
        if let Some((x, y)) =
            NetworkApplySystem::object_position(&ctx.world, &entity_index, object_id)
        {
            ctx.events_mut().send_presentation(
    ...
    if let Some(e) = local_player_entity {
        ...
        // Group state
        if group_disbanded
            || !group_members_added.is_empty()
            || !group_members_removed.is_empty()
            || group_allow_join.is_some()
        {
    ...
    if let Some(trapped) = trap_rock_state {
        if let Some(e) = local_player_entity {
            let existing = ctx
                .world
                .get::<&crate::components::InTrapRock>(e)
                .map(|t| t.trapped)
                .ok();
            match existing {
                ...
                None => {
                    let _ = ctx
                        .world
                        .insert_one(e, crate::components::InTrapRock { trapped });
                }
            }
        }
    }
    ...
    if !ground_items.is_empty() {
        use crate::components::{
            GroundItem as GroundItemComp, NetworkObjectType, NetworkSync, Position,
        };
        for packet in &ground_items {
            ...
            if existing.is_none() {
                let entity = ctx.world.spawn((
                    ...
                    Position {
                        x: packet.location_x as f32,
                        y: packet.location_y as f32,
                    },
                ));
                tracing::trace!(
                    "📍 Ground item spawned: {} at ({}, {})",
                    entity.id(),
                    packet.location_x,
                    packet.location_y
                );
    ...
    for (_, p) in object_moves {
        match p {
            RemoteMovePacket::Walk(p) => {
                NetworkApplySystem::apply_object_walk(ctx, &entity_index, p)
            }
    ...
    if !mount_updates.is_empty() {
        ...
        for (object_id, mount_type, riding_mount) in mount_updates {
            let target_entity = if let Some(&e) = entity_index.get(&object_id) {
                Some(e)
            } else {
            ...
            let changed =
                prev.map(|p| (p.mount_type, p.riding_mount)) != Some((mount_type, riding_mount));
    ...
    if let Some(e) = local_player_entity {
        ...
        if let Some((cur, max)) = player_mana_changed {
            ...
            let mut inserted = false;
            {
                if let Ok(mut mp) = ctx.world.get::<&mut crate::components::Mana>(e) {
                    mp.current = if new_max > 0 {
                        new_current.clamp(0, new_max)
                    } else {
                        new_current
                    };
                    if new_max != 0 {
            ...
            if !inserted {
                ...
                let _ = ctx.world.insert_one(
                    ...
                    crate::components::Mana {
                        current: if effective_max > 0 {
                            new_current.clamp(0, effective_max)
                        } else {
                            new_current
                        },
                        max: effective_max,
        ...
        if gold_delta_sum != 0 {
            ...
            if has_currency {
                ...
                let _ = ctx
                    .world
                    .insert_one(e, crate::components::Currency { gold, credit: 0 });
            }
        }
        ...
        if !items_gained.is_empty() {
            ...
            if ctx.world.get::<&crate::components::Inventory>(e).is_err() {
                let _ = ctx
                    .world
                    .insert_one(e, crate::components::Inventory::default());
            }
            if let Ok(mut inv) = ctx.world.get::<&mut crate::components::Inventory>(e) {
    ...
    for (object_id, attacker_id, damage) in object_struck {
        ...
        {
            use crate::components::{
                Monster, MountStatus, PlayerAppearance, SoundTrigger, SoundType,
            };

            // helper: attacker weapon shape (unknown => -1)
            let struck_weapon: i16 = entity_index
                .get(&attacker_id)
                .and_then(|&att| {
                    ctx.world
                        .get::<&PlayerAppearance>(att)
                        .ok()
                        .map(|a| a.weapon)
                })
                .unwrap_or(-1);
            ...
            if let Some(monster_type) = monster_type {
                ...
                // 动画：怪物受击动作
                NetworkApplySystem::set_monster_anim(
                    &ctx.world,
                    &entity_index,
                    object_id,
                    Some(crate::components::MirAction::Struck),
                    None,
                );
                ...
                .map(|a| (a.class, a.armour))
            {
                ...
                if is_riding {
                    ...
                    let base = match struck_weapon {
                        // sword-ish groups
                        0 | 23 | 1 | 12 | 28 | 40 | 2 | 8 | 11 | 15 | 18 | 20 | 25 | 31 | 33
                        | 34 | 37 | 41 | 3 | 5 | 7 | 9 | 13 | 19 | 24 | 26 | 29 | 32 | 35 => 10070,
                        // axe
                        4 | 14 | 16 | 38 => 10071,
                        // long stick / club
                ...
                if ctx
                    .world
                    .get::<&crate::components::DeathState>(target)
                    .is_err()
                {
        ...
        if had_hp {
            NetworkApplySystem::upsert_component(
                ctx,
                target,
                crate::components::HealthBarAnim {
                    displayed: hp_after_damage as f32,
                },
            );
        }
                ...
                .is_err()
        {
            let _ = ctx
                .world
                .insert_one(target, crate::components::DeathState::new());
            let _ = ctx
                .world
                .remove_one::<crate::components::AttackState>(target);
        }

        if !had_hp {
            let mut hp = crate::components::Health {
                current: 100,
                max: 100,
            };
            hp.take_damage(damage);
        ...
        if let Some(&target) = entity_index.get(&object_id) {
            ...
            if ctx.world.get::<&crate::components::Player>(target).is_ok() {
                NetworkApplySystem::upsert_component(
                    ctx,
                    target,
                    crate::components::DeathState::new(),
                );
                let _ = ctx
                    .world
                    .remove_one::<crate::components::AttackState>(target);
            ...
            if ctx
                .world
                .get::<&crate::components::LocalPlayer>(target)
                .is_ok()
            {
            ...
            {
                use crate::components::{
                    MirGender, Monster, PlayerAppearance, SoundTrigger, SoundType,
                };
                let monster_type = ctx
                    .world
                    .get::<&Monster>(target)
                ...
                if let Some(monster_type) = monster_type {
                    ...
                    NetworkApplySystem::set_monster_anim(
                        ...
                        &entity_index,
                        object_id,
                        Some(crate::components::MirAction::Die),
                        None,
                    );
        ...
        if let Some(e) = local_player_entity {
            ...
            if let Some(oid) = local_player_object_id {
                NetworkApplySystem::set_monster_anim(
                    ...
                    &entity_index,
                    oid,
                    Some(crate::components::MirAction::Dead),
                    dir,
                );
            }
    ...
    for (object_id, buff_id, expire_time, infinite, paused) in buff_adds {
        ...
        if let Some(&e) = entity_index.get(&object_id) {
            ...
            if let Some(cb) = combat_buff {
                ...
                buff.paused = paused;
                NetworkApplySystem::with_component::<crate::components::combat::BuffList>(
                    ctx,
                    e,
                    |bl| {
    ...
    for (object_id, buff_id) in buff_removes {
        tracing::trace!(
            "🔮 Buff removed: object_id={}, buff_id={}",
            object_id,
            buff_id
        );
        if let Some(&e) = entity_index.get(&object_id) {
    ...
    for object_id in dash_failed {
        NetworkApplySystem::set_monster_anim(
            ...
            &entity_index,
            object_id,
            Some(crate::components::MirAction::DashFail),
            None,
        );
    }
    ...
    for object_id in sat_down {
        NetworkApplySystem::set_monster_anim(
            ...
            &entity_index,
            object_id,
            Some(crate::components::MirAction::SitDown),
            None,
        );
    }
    ...
    for (object_id, x, y, dir) in harvested {
        ...
        NetworkApplySystem::set_monster_anim(
            ...
            &entity_index,
            object_id,
            Some(crate::components::MirAction::Harvest),
            direction,
        );
    }
    ...
    for (object_id, poison_type) in poisoned_objects {
        tracing::trace!(
            "☠ Object poisoned: object_id={}, type={}",
            object_id,
            poison_type
        );
        if let Some(&e) = entity_index.get(&object_id) {
    ...
    for (from_id, target_id, target_x, target_y, spell) in range_projectiles {
        ...
        if let (Some(from), Some(projectile_type)) = (
            from_pos,
            NetworkApplySystem::spell_to_projectile_type(spell as u8),
        ) {
            ctx.events_mut().send_presentation(
                crate::event_bus::PresentationEvent::ProjectileEffect {
                    projectile_type,
    ...
    for amount in &player_exp_gains {
        if let Some(e) = local_player_entity {
            ...
            if let Some((x, y)) = NetworkApplySystem::entity_position(ctx, e) {
                ctx.events_mut().send_presentation(
                    crate::event_bus::PresentationEvent::FloatingExperience {
                        amount: *amount,
    ...
    for (object_id, colour) in object_colours {
        if let Some(&e) = entity_index.get(&object_id) {
            NetworkApplySystem::upsert_component(
                ctx,
                e,
                crate::components::NameColor(colour as i32),
            );
        }
    }
    ...
    for (object_id, guild_name) in object_guild_names {
        if let Some(&e) = entity_index.get(&object_id) {
            if let Ok(mut other) = ctx.world.get::<&mut crate::components::OtherPlayer>(e) {
                other.guild_name = if guild_name.is_empty() {
                    None
                } else {
                    Some(guild_name)
                };
    ...
    for (object_id, deco, remove) in deco_updates {
        if let Some(&e) = entity_index.get(&object_id) {
            if remove {
                let _ = ctx
                    .world
                    .remove_one::<crate::components::render::ObjectDeco>(e);
            } else {
                NetworkApplySystem::upsert_component(
                    ctx,
                    e,
    ...
    for (object_id, _spell) in object_spells {
        NetworkApplySystem::set_monster_anim(
            ...
            &entity_index,
            object_id,
            Some(crate::components::MirAction::Spell),
            None,
        );
    }
    ...
    for amount in hero_exp_gains {
        if let Some(e) = local_player_entity {
            ...
            let hero_id = ctx
                .world
                .get::<&crate::components::HeroState>(e)
                .ok()
                .map(|h| h.hero_object_id)
                .unwrap_or(0);
            if hero_id != 0 {
                if let Some((x, y)) =
                    NetworkApplySystem::object_position(&ctx.world, &entity_index, hero_id)
                {
                    ctx.events_mut().send_presentation(
                        crate::event_bus::PresentationEvent::FloatingExperience {
                            amount,
                ...
                if let Some(&hero_entity) = entity_index.get(&hero_id) {
                    let mut updated_hp = false;
                    {
                        if let Ok(mut health) =
                            ctx.world.get::<&mut crate::components::Health>(hero_entity)
                        {
                    ...
                    {
                        if let Ok(mut mana) =
                            ctx.world.get::<&mut crate::components::Mana>(hero_entity)
                        {
    ...
    for (spell, level, experience, key) in hero_magic_learned {
        NetworkApplySystem::with_hero_magic_list(
            ctx,
            &entity_index,
            local_player_entity,
            |magic_list| {
    ...
    for (unique_id, durability) in dura_changes {
        tracing::trace!(
            "🔧 Item durability changed: unique_id={}, durability={}",
            unique_id,
            durability
        );
        let Some(e) = local_player_entity else {
    ...
    for (object_id, object_type, library, index, x, y) in mock_spawns {
        NetworkApplySystem::apply_mock_library_sprite_spawn(
            ctx,
            object_id,
            object_type,
            library,
    ...
    for (object_id, spell, target_id) in spell_casts {
        let caster_pos = entity_index.get(&object_id).and_then(|&e| {
            ctx.world
                .get::<&crate::components::Position>(e)
                .ok()
                .map(|p| *p)
        });
        let target_pos = entity_index.get(&target_id).and_then(|&e| {
            ctx.world
                .get::<&crate::components::Position>(e)
                .ok()
                .map(|p| *p)
        ...
        let (effect_text, effect_color) = match spell_enum {
            ...
            x if x == Spell::ThunderBolt as u8 => (
                "雷电",
                macroquad::prelude::Color::from_rgba(150, 150, 255, 255),
            ),
            x if x == Spell::Lightning as u8 => (
                "闪电",
                macroquad::prelude::Color::from_rgba(100, 100, 255, 255),
            ),
            x if x == Spell::Healing as u8 => ("治疗术", macroquad::prelude::GREEN),
            x if x == Spell::Poisoning as u8 => (
                "施毒",
                macroquad::prelude::Color::from_rgba(128, 0, 128, 255),
            ),
            x if x == Spell::Teleport as u8 => ("瞬移", macroquad::prelude::WHITE),
            x if x == Spell::MagicShield as u8 => (
                "魔法盾",
                macroquad::prelude::Color::from_rgba(100, 200, 255, 255),
            ),
            x if x == Spell::HalfMoon as u8 => (
                "半月",
                macroquad::prelude::Color::from_rgba(255, 255, 200, 255),
            ),
            x if x == Spell::ShoulderDash as u8 => (
                "野蛮冲撞",
                macroquad::prelude::Color::from_rgba(200, 200, 200, 255),
            ),
            _ => (
                "施法",
                macroquad::prelude::Color::from_rgba(255, 255, 100, 255),
            ),
        };
        ...
        if let Some(&caster) = entity_index.get(&object_id) {
            if ctx
                .world
                .get::<&crate::components::DeathState>(caster)
                .is_err()
    ...
    for (object_id, effect, effect_type) in effect_received {
        ...
        let (px, py) = NetworkApplySystem::object_position(&ctx.world, &entity_index, object_id)
            .unwrap_or((0.0, 0.0));
    ...
    if !spell_delays.is_empty() {
        if let Some(e) = local_player_entity {
            if ctx
                .world
                .get::<&crate::components::spell::SpellCooldowns>(e)
                .is_err()
            {
                let _ = ctx
                    .world
                    .insert_one(e, crate::components::spell::SpellCooldowns::new());
            }
            if let Ok(mut cooldowns) = ctx
                .world
                .get::<&mut crate::components::spell::SpellCooldowns>(e)
            {
    ...
    if let Some((state, success)) = fishing_status {
        if let Some(e) = local_player_entity {
            let dead = ctx.world.get::<&crate::components::DeathState>(e).is_ok();
