...
pub fn pump_network_messages_to_ui(ctx: &mut GameContext) {
    ...
    let mut pending_monster_info: Vec<mir2_shared::data::client_data::ClientMonsterInfo> =
        Vec::new();
    let mut pending_npc_info: Vec<mir2_shared::data::client_data::ClientNPCInfo> = Vec::new();

    // 获取本地玩家的 object_id 用于过滤 buff 等事件
    let local_object_id: Option<u32> = ctx.world.iter().find_map(|e| {
        e.get::<&crate::components::PlayerData>()
            .map(|pd| pd.object_id)
    });
    ...
    for ev in ctx.events().network_events() {
        match ev {
            ...
            NetworkEvent::FriendUpdated { friends } => {
                cmds.push(UiCommand::UpdateFriendList {
                    friends: friends.clone(),
                });
            }
            NetworkEvent::GuildInvite {
                inviter,
                guild_name,
            } => {
                cmds.push(UiCommand::ShowInviteConfirm {
            ...
            NetworkEvent::GuildJoined {
                ...
                cmds.push(UiCommand::SetGuildName {
                ...
                sys_chat(
                    ...
                    format!(
                        "你已加入行会「{}」({}) Lv.{} {}/{}人",
                        guild_name, rank_name, level, member_count, max_members
                    ),
                );
            }
            ...
            NetworkEvent::GuildNoticeUpdated { notice } => {
                ...
                cmds.push(UiCommand::GuildNoticeUpdated {
                    notice: notice.clone(),
                });
            }
            ...
            NetworkEvent::HeroCreateRequested { can_create_class } => {
                let classes: Vec<&str> = can_create_class
                    .iter()
                    .enumerate()
                    .filter(|(_, &c)| c)
                    .map(|(i, _)| match i {
                    ...
                    .collect();
                cmds.push(UiCommand::PushHeroSystemChat(format!(
                    "可选职业: {}",
                    classes.join("/")
                )));
            }
            ...
            NetworkEvent::HeroBehaviourSet { behaviour } => {
                cmds.push(UiCommand::UpdateHeroBehaviour {
                    behaviour: *behaviour,
                });
            }
            ...
            NetworkEvent::LevelUp { new_level } => {
                cmds.push(UiCommand::PlayerLevelUp {
                    new_level: *new_level,
                });
            }
            ...
            NetworkEvent::HeroLevelUp { new_level } => {
                cmds.push(UiCommand::HeroLevelUp {
                    new_level: *new_level,
                });
                cmds.push(UiCommand::PushHeroSystemChat(format!(
                    "英雄升级到 Lv.{}",
            ...
            NetworkEvent::BuffRemoved { object_id, buff_id } => {
                if local_object_id.is_none_or(|loid| loid == *object_id) {
                    cmds.push(UiCommand::RemoveBuff {
                        buff_type: *buff_id,
                    });
                }
            }
            ...
            NetworkEvent::HeroAutoPotSet { pot_type, value } => {
                cmds.push(UiCommand::SetHeroAutoPotValue {
                    pot_type: *pot_type,
                    value: *value,
                });
            }
            NetworkEvent::HeroAutoPotItemSet { slot, item_id } => {
                cmds.push(UiCommand::SetHeroAutoPotItem {
                    slot: *slot,
                    item_id: *item_id,
                });
            }
            NetworkEvent::HeroManageReceived { heroes } => {
                let entries: Vec<_> = heroes
                    .iter()
                    .map(
                        |h| crate::scenes::dialogs::game::hero_dialog::ManageHeroEntry {
                            index: h.index,
                            name: h.name.clone(),
                            level: h.level,
                            class: h.class as u8,
            ...
            NetworkEvent::HeroBaseStatsReceived { stats } => {
                cmds.push(UiCommand::SetHeroBaseStats {
                    stats: stats.clone(),
                });
            }
            ...
            NetworkEvent::IntelligentCreatureRenameEnabled { can_rename } => {
                cmds.push(UiCommand::SetCreatureCanRename {
            ...
            NetworkEvent::BuffPaused {
                ...
                if local_object_id.is_none_or(|loid| loid == *object_id) {
                    cmds.push(UiCommand::SetBuffPaused {
                        buff_id: *buff_id,
                        paused: *paused,
                    });
                }
            ...
            NetworkEvent::CompassUpdated { location } => {
                cmds.push(UiCommand::UpdateCompass {
                    location: *location,
                });
            }
            // 交易进度事件
            NetworkEvent::TradeStarted { partner } => {
                cmds.push(UiCommand::OpenTradeDialog {
            ...
            NetworkEvent::TradeItemDeposited { from_slot, success } => {
                cmds.push(UiCommand::TradeItemDeposited {
                    from_slot: *from_slot,
                    success: *success,
                });
            }
            NetworkEvent::TradeItemRetrieved { from_slot, success } => {
                cmds.push(UiCommand::TradeItemRetrieved {
                    from_slot: *from_slot,
                    success: *success,
                });
            }
            ...
            NetworkEvent::MailReceived { mails } => {
                let entries: Vec<_> = mails
                    .iter()
                    .map(|m| crate::ui::ui_state::MailEntry {
                        mail_id: m.mail_id,
            ...
            NetworkEvent::QuestCompleted { quest_id } => {
                cmds.push(UiCommand::QuestCompleted {
                    quest_id: *quest_id,
                });
            }
            ...
            NetworkEvent::NPCMarketEvent2 { pages } => {
                ...
                cmds.push(UiCommand::UpdateMerchantItems {
                    items: vec![],
                    page: 1,
                    total,
                });
            ...
            NetworkEvent::NPCMarketPageEvent2 { listings } => {
                ...
                let items: Vec<MerchantItem> = listings
                    .iter()
                    .map(|l| {
                        let remaining_hours = ((l.consignment_date + seven_days_secs - now_secs)
                            / 3600)
                            .max(0) as u32;
                        MerchantItem {
                            item: l.item.clone(),
            ...
            NetworkEvent::NPCPearlGoodsReceived { rate, item_list } => {
                let pearl_items: Vec<mir2_shared::data::item::UserItem> = item_list
                    .iter()
            ...
            NetworkEvent::NPCRequestInputReceived {
                ...
                cmds.push(UiCommand::ShowTextInput {
                    kind: crate::scenes::dialogs::game::main_dialog::TextInputKind::NPCInput {
                        npc_id: *npc_id,
                    },
            ...
            NetworkEvent::GuildBuffListReceived { buff_ids } => {
                cmds.push(UiCommand::UpdateGuildBuffs {
            ...
            NetworkEvent::SellItemReceived {
                ...
                if *success {
                    ...
                    sys_chat(
                        &mut cmds,
                        format!("物品出售成功 (id={}, 数量={})", unique_id, count),
                    );
                } else {
            ...
            NetworkEvent::CraftItemReceived {
                ...
                if *success {
                    ...
                    sys_chat(
                        &mut cmds,
                        format!("合成/制作完成 (id={}, 数量={})", unique_id, count),
                    );
                } else {
            ...
            NetworkEvent::ItemRepairedEvent {
                ...
                cmds.push(UiCommand::ItemDuraChanged {
                    unique_id: *unique_id,
                    current_dura: *current_dura as i32,
                });
                sys_chat(
                    &mut cmds,
                    format!(
                        "物品修理完成 (id={}, 耐久={}/{})",
                        unique_id, current_dura, max_dura
                    ),
                );
            }
            ...
            NetworkEvent::AwakeningNeedMaterialsReceived { item_id, materials } => {
                // 显示觉醒对话框（材料由服务器推送）
                let mat_entries: Vec<
                    crate::scenes::dialogs::game::npc_awake_dialog::AwakeningMaterial,
                > = materials
                    .iter()
                    .map(|(id, count)| {
                        crate::scenes::dialogs::game::npc_awake_dialog::AwakeningMaterial {
            ...
            NetworkEvent::MailLockedItemReceived { unique_id, locked } => {
                ...
                sys_chat(
                    &mut cmds,
                    format!("邮件附件{} (unique_id={})", state, unique_id),
                );
            }
            NetworkEvent::MailSendRequestReceived { mail_id } => {
                sys_chat(
                    &mut cmds,
                    format!("请输入收件人和邮件内容 (mail_id={})", mail_id),
                );
            }
            ...
            NetworkEvent::ItemRentalRequested => {
                cmds.push(UiCommand::OpenItemRental {
                    partner: "未知".to_string(),
                });
            }
            ...
            NetworkEvent::RentalItemDeposited { unique_id, success } => {
                if *success {
                    sys_chat(
                        &mut cmds,
                        format!("租赁物品已存入 (unique_id={})", unique_id),
                    );
                } else {
            ...
            NetworkEvent::RentalItemRetrieved { unique_id, success } => {
                if *success {
                    ...
                    sys_chat(
                        &mut cmds,
                        format!("租赁物品已取回 (unique_id={})", unique_id),
                    );
                } else {
            ...
            NetworkEvent::RentalItemUpdated { fee, period } => {
                sys_chat(
                    &mut cmds,
                    format!("租赁物品已更新 (费用={}, 周期={})", fee, period),
                );
            }
            NetworkEvent::RentalItemsReceived { items } => {
                cmds.push(UiCommand::UpdateRentalItemList {
                    items: items.clone(),
                });
            ...
            NetworkEvent::TimerExpired { timer_id } => {
                cmds.push(UiCommand::TimerExpired {
                    timer_id: *timer_id,
                });
            }
            // 排行榜
            NetworkEvent::RankingsReceived { rankings } => {
                let entries: Vec<_> = rankings
                    .iter()
                    .map(|r| {
                        (
                            r.rank as u32,
            ...
            NetworkEvent::PlayerRevived => {
                cmds.push(UiCommand::PushChatNotice {
                    text: "你已复活".to_string(),
                });
            }
            NetworkEvent::PlayerPoisoned {
                ...
                cmds.push(UiCommand::PushChatNotice {
                    text: format!("你中毒了！(类型={})", poison_type),
                });
            }
            NetworkEvent::ObjectPoisonedEvent {
                ...
                cmds.push(UiCommand::PushChatNotice {
                    text: format!("目标中毒(类型={})", poison_type),
                });
            }
            ...
            NetworkEvent::BaseStatsReceived { stats } => {
                cmds.push(UiCommand::SetBaseStats {
                    stats: stats.clone(),
                });
            }
            ...
            NetworkEvent::WorldMapSetupReceived { icons } => {
                cmds.push(UiCommand::UpdateWorldMapIcons {
                    icons: icons.clone(),
                });
            }
            ...
            NetworkEvent::MarriageRequested2 { requester } => {
                cmds.push(UiCommand::SetMarriageRequester {
            ...
            NetworkEvent::DuraChanged {
                ...
                cmds.push(UiCommand::ItemDuraChanged {
                    unique_id: *unique_id,
                    current_dura: *durability,
                });
            }
            NetworkEvent::DelayedExplosionRemoved { object_id } => {
                sys_chat(
                    &mut cmds,
                    format!("延迟爆炸已移除 (object_id={})", object_id),
                );
            }
            ...
            NetworkEvent::UserStorageReceived { items } => {
                cmds.push(UiCommand::UpdateStorageItems {
                    items: items.clone(),
                });
            }
            ...
            NetworkEvent::LogOutSuccess { characters } => {
                ...
                tracing::info!(
                    "🚪 LogOutSuccess → CharacterSelect with {} characters",
                    count
                );
            }
            ...
            NetworkEvent::NoticeUpdated { notice } => {
                cmds.push(UiCommand::ShowNotice {
                    text: notice.clone(),
                });
            }
            // 魔法/技能事件
            NetworkEvent::MagicLearned { magic, hero } => {
                ...
                sys_chat(
                    &mut cmds,
                    format!("学会技能：{} Lv.{}", magic.name, magic.level),
                );
            }
            NetworkEvent::MagicLeveledUp {
                ...
                cmds.push(UiCommand::MagicLeveledUp {
                    spell: *spell as u8,
                    level: *level,
                    experience: 0,
                });
            ...
            NetworkEvent::MagicRemoved { spell, hero } => {
                cmds.push(UiCommand::MagicRemoved {
                    spell: *spell as u8,
                    hero: *hero,
                });
            ...
            NetworkEvent::SpellToggled {
                ...
                cmds.push(UiCommand::SpellToggled {
                    spell: *spell as u8,
                    can_use: *can_use,
                    hero: *hero,
                });
            ...
            | NetworkEvent::ItemLost { unique_id, .. } => {
                cmds.push(UiCommand::RemoveDuraEntry {
                    unique_id: *unique_id,
                });
            }
            NetworkEvent::ItemGained { item } => {
                ...
                sys_chat(
                    ...
                    format!(
                        "获得物品: {} x{} (UID={})",
                        name, item.count, item.unique_id
                    ),
                );
            }
            NetworkEvent::ItemDropped {
                ...
                if !success {
                    ...
                    sys_chat(
                        &mut cmds,
                        format!("丢弃物品 x{} (UID={})", count, unique_id),
                    );
                }
            }
            ...
            NetworkEvent::ItemSealed {
                ...
                sys_chat(
                    &mut cmds,
                    format!("物品封印: UID={} 到期={}", unique_id, expiry_date),
                );
            }
            ...
            NetworkEvent::NewItemInfoReceived {
                ...
                sys_chat(
                    &mut cmds,
                    format!("新物品信息: {} (ID={})", item_name, item_index),
                );
            }
            ...
            NetworkEvent::ItemUpgraded { item } => {
                ...
                sys_chat(
                    &mut cmds,
                    format!("物品升级成功: {} (UID={})", name, item.unique_id),
                );
            }
            NetworkEvent::ItemSplit {
                ...
                sys_chat(
                    &mut cmds,
                    format!("物品拆分: UID={} 数量={}", unique_id, count),
                );
            }
            ...
            NetworkEvent::PlayerStruck {
                ...
                if *damage > 0 {
                    sys_chat(
                        &mut cmds,
                        format!("受到 {} 点伤害 (来自 ObjectID={})", damage, attacker_id),
                    );
                }
            }

            // ===== PR #1169: Warehouse password responses =====
            NetworkEvent::StorageUnlockResultReceived {
                result,
                has_password,
            } => {
                let msg = match *result {
            ...
            NetworkEvent::StoragePasswordResultReceived {
                result, removing, ..
            } => {
                let msg = match *result {
