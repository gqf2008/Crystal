...
use crate::components::{
    ClientOnly, CombatStats, Health, LocalPlayer, MirClass, Monster, MountState, MountStatus,
    NetworkObjectType, NetworkSync, PlayerAppearance, Position, SoundTrigger, SoundType,
};
...
impl LogicSystem for CombatSystem {
    fn update(&mut self, ctx: &mut GameContext, _delay_time: f32) -> GameResult {
        ...
        if let Ok(hp) = ctx.world.get::<&Health>(player_entity) {
            if hp.current <= 0 {
                if let Ok(mut input) = ctx
                    .world
                    .get::<&mut crate::components::PlayerInput>(player_entity)
                {
        ...
        if ctx
            .world
            .get::<&crate::components::AttackState>(player_entity)
            .is_ok()
        {
            return Ok(());
        }
        ...
        if let Ok(mut player) = ctx
            .world
            .get::<&mut crate::components::Player>(player_entity)
        {
...
impl CombatSystem {
    pub(crate) fn choose_player_attack_sound_id(
        world: &World,
        ...
        let has_class_weapon = {
            ...
            match group {
                ...
                _ => matches!(
                    class,
                    MirClass::Wizard | MirClass::Warrior | MirClass::Taoist
                ),
            }
        };
        ...
        let riding_from_state = mount_state
            .map(|m| m.mount_index.is_some())
            .unwrap_or(false);
    ...
    pub fn calculate_physical_damage(
        ...
        if level_diff > 0 {
            damage =
                (damage as f32 * (1.0 + level_diff as f32 * 0.02)).min(damage as f32 * 1.5) as i32;
        } else if level_diff < 0 {
            damage =
                (damage as f32 * (1.0 + level_diff as f32 * 0.02)).max(damage as f32 * 0.5) as i32;
        }
    ...
    pub fn calculate_magic_damage(
        ...
        if level_diff > 0 {
            damage =
                (damage as f32 * (1.0 + level_diff as f32 * 0.03)).min(damage as f32 * 2.0) as i32;
        } else if level_diff < 0 {
            damage =
                (damage as f32 * (1.0 + level_diff as f32 * 0.03)).max(damage as f32 * 0.3) as i32;
        }
    ...
    /// 处理受到伤害
    pub fn take_damage(world: &mut World, target_id: u32, damage: i32, damage_type: DamageType) {
        // 查找目标并扣血
        ...
        let take_damage_result: Option<(hecs::Entity, u16, bool, bool)> =
            world.iter().find_map(|eref| {
                ...
                if combat_log_enabled() {
                    println!(
                        "🩸 {} 受到 {} 点{:?}伤害 (HP: {} → {})",
                        target_id, damage, damage_type, old_hp, health.current
                    );
                }
        ...
        for (_local, health) in world.query_mut::<(&LocalPlayer, &mut Health)>() {
            ...
            if combat_log_enabled() {
                println!(
                    "❤️ 玩家受到 {} 点{:?}伤害 (HP: {} → {})",
                    damage, damage_type, old_hp, health.current
                );
            }
    ...
    /// 查找攻击范围内的目标
    pub fn find_targets_in_range(world: &World, center: Position, range: f32) -> Vec<u32> {
