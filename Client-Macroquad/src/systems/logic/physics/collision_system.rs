...
impl LogicSystem for CollisionSystem {
    fn update(&mut self, ctx: &mut GameContext, delay_time: f32) -> GameResult {
        ...
        let Some(width) = map_width else {
            return Ok(());
        };
        let Some(height) = map_height else {
            return Ok(());
        };
        let Some(cells) = map_cells else {
            return Ok(());
        };

        // 检查移动方向的下一个格子是否有障碍物
        // 重要：MovementSystem 已经更新了 position，本系统负责“阻止进一步进入阻挡”和“把已经进入阻挡的情况拉回”。
        use crate::components::{MovementMode, Player, PlayerInput};
        ...
        for (pos, vel, player_input, path, _player) in ctx.world.query_mut::<(
            ...
            if !Self::is_walkable(&cells, width, height, cur_gx, cur_gy) {
                ...
                if Self::is_walkable(&cells, width, height, prev_gx, prev_gy) {
                    pos.x = prev_x;
                    pos.y = prev_y;
                } else if let Some((gx, gy)) =
                    Self::nearest_walkable(&cells, width, height, (cur_gx, cur_gy), 8)
                {
            ...
            if has_obstacle {
                ...
                if is_player {
                    tracing::warn!(
                        "🛑 碰撞！停止velocity但保持动画 - NextGrid({}, {}), CurPos({:.1}, {:.1})",
                        grid_x,
                        grid_y,
                        pos.x,
                        pos.y
