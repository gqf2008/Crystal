...
async fn main() {
    ...
    let font = match load_ttf_font("C:/Windows/Fonts/msyh.ttc").await {
        ...
        Err(_) => {
            ...
            load_ttf_font("C:/Windows/Fonts/simsun.ttc")
                .await
                .expect("无法加载任何中文字体")
        }
    };
    ...
    loop {
        ...
        // 统计信息
        let chinese_count = char_history
            .iter()
            .filter(|(_, code, _)| *code > 0x4E00)
            .count();
        let ascii_count = char_history
            .iter()
            .filter(|(_, code, _)| *code < 0x80 && *code >= 0x20)
            .count();
