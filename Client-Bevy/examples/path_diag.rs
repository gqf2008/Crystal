...
fn main() {
    ...
    for x in 0..map_reader.width {
        ...
        for y in 0..map_reader.height {
            col.push(map_reader.map_cells[x as usize][y as usize].is_walkable());
        }
        walkable.push(col);
    }
    let map = LoadedMap {
        name: "3".into(),
        width: map_reader.width,
        height: map_reader.height,
        walkable,
        doors: vec![vec![0u8; map_reader.height as usize]; map_reader.width as usize],
    };
