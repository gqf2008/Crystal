...
fn save(buf: &RgbaImage, name: &str) {
    let _ = image::DynamicImage::ImageRgba8(buf.clone())
        .into_rgb8()
        .save(name);
    println!("[save] {}", name);
}
...
fn main() {
    ...
    check(
        "对话框 (348,274,328,220)",
        content(&buf, 348, 274, 328, 220),
    );
    check(
        "标题 Title[30] (461,286,102,24)",
        content(&buf, 461, 286, 102, 24),
    );
    check("OK 按钮 (575,355,48,48)", content(&buf, 575, 355, 48, 48));
    check(
        "账号输入框 (433,359,136,15)",
        content(&buf, 433, 359, 136, 15),
    );
    check(
        "关闭按钮 (514,463,100,25)",
        content(&buf, 514, 463, 100, 25),
    );
    ...
    check(
        "对话框 Prguse[63] (218,154,588,460)",
        content(&buf, 218, 154, 588, 460),
    );
    check(
        "账号输入(顶) (444,257,136,18)",
        content(&buf, 444, 257, 136, 18),
    );
    check(
        "问题输入(190宽) (444,404,190,18)",
        content(&buf, 444, 404, 190, 18),
    );
    ...
    check(
        "对话框 Prguse[50] (338,250,348,268)",
        content(&buf, 338, 250, 348, 268),
    );
    check(
        "账号输入 (516,325,136,18)",
        content(&buf, 516, 325, 136, 18),
    );
    check(
        "OK Title[107] (418,486,90,25)",
        content(&buf, 418, 486, 90, 25),
    );
    check(
        "Cancel Title[110] (560,486,68,25)",
        content(&buf, 560, 486, 68, 25),
    );
    ...
    println!("[4/7] 选角界面 抽检：");
    check(
        "标题 Title[40] (468,20,84,19)",
        content(&buf, 468, 20, 84, 19),
    );
    check(
        "角色槽[0] (637,194,288,56)",
        content(&buf, 637, 194, 288, 56),
    );
    check(
        "角色槽[3] (637,506,288,56)",
        content(&buf, 637, 506, 288, 56),
    );
    check(
        &format!("预览 ChrSel[20] @({},{})", pvx, pvy),
        content(&buf, pvx.max(0) as u32, pvy.max(0) as u32, 180, 260),
    );
    check(
        "开始按钮 (132,736,100,25)",
        content(&buf, 132, 736, 100, 25),
    );
    ...
    check(
        "对话框 Prguse[73] (218,154,588,460)",
        content(&buf, 218, 154, 588, 460),
    );
    check(
        "标题 Title[20] (424,165,187,20)",
        content(&buf, 424, 165, 187, 20),
    );
    check(
        &format!("预览 @({},{})", pvx, pvy),
        content(&buf, pvx.max(0) as u32, pvy.max(0) as u32, 150, 250),
    );
    check(
        "名字输入 (543,422,240,20)",
        content(&buf, 543, 422, 240, 20),
    );
    check(
        "战士按钮(选中) (541,450,44,42)",
        content(&buf, 541, 450, 44, 42),
    );
    check("弓手按钮 (741,450,44,42)", content(&buf, 741, 450, 44, 42));
    check(
        "OK Title[360] (378,579,60,25)",
        content(&buf, 378, 579, 60, 25),
    );
    ...
    check(
        "对话框 Prguse[360] (284,289,456,190)",
        content(&buf, 284, 289, 456, 190),
    );
    check(
        "Yes Title[206] (544,446,76,25)",
        content(&buf, 544, 446, 76, 25),
    );
    check(
        "No Title[210] (644,446,76,25)",
        content(&buf, 644, 446, 76, 25),
    );
    ...
    check(
        "对话框 Prguse[660] (368,306,288,156)",
        content(&buf, 368, 306, 288, 156),
    );
    check("输入框 (391,392,240,19)", content(&buf, 391, 392, 240, 19));
    check(
        "OK Title[200] (428,429,76,25)",
        content(&buf, 428, 429, 76, 25),
    );
    check(
        "Cancel Title[203] (528,429,76,25)",
        content(&buf, 528, 429, 76, 25),
    );
