...
def main():
    ...
    a = ap.parse_args()
    rows = parse(CS.read_text(encoding="utf-8", errors="replace"))
    entries = rust_entries(rows)
    block = render(entries)
    print("# C# MirAction.Spell 分支共 %d 条 Effect，其中魔法库条目 %d 条" % (len(rows), len(entries)))
    if not a.write:
        print(block)
        return
    out = Path("Client-Bevy/src/game/spell_effects.rs")
    text = out.read_text(encoding="utf-8")
    i = text.index(OUT_MARK_BEGIN)
    j = text.index(OUT_MARK_END) + len(OUT_MARK_END)
    out.write_text(text[:i] + block + text[j:], encoding="utf-8")
    print("# 已写回 %s（%d 条）" % (out, len(entries)))
