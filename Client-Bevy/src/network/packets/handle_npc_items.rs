...
pub(crate) fn handle_npc_items(    net: &mut NetConnection,
    ...
    match opcode {
        ...
        // #270：NPC 寄售（空包）
        x if x == ServerPacketIds::NPCConsign as i16 => {
            if market_system::NPCConsign::read_body(&mut cur).is_ok() {
                tracing::debug!("📦 NPC 寄售面板");
            }
        }
        ...
        x if x == ServerPacketIds::NPCAwakening as i16 => {
            if awakening_system::NPCAwakening::read_body(&mut cur).is_ok() {
                tracing::debug!("🔮 NPC 觉醒面板");
            }
        }

        // #258：合成 / 物品升级
        x if x == ServerPacketIds::CombineItem as i16 => {
            if let Ok(p) = item_operations::CombineItem::read_body(&mut cur) {
                tracing::info!(
                    "🔨 合成: {} → {} success={} destroy={}",
                    p.id_from,
                    p.id_to,
        ...
        x if x == ServerPacketIds::ItemUpgraded as i16 => {
            if let Ok(p) = item_operations::ItemUpgraded::read_body(&mut cur) {
                server_events.write(ServerEvent::ItemUpgraded {
                    item: to_inv_item(&p.item),
                });
                tracing::info!("⬆️ 物品升级 uid={}", p.item.unique_id);
            }
        }
        ...
        x if x == ServerPacketIds::ItemRepaired as i16 => {
            if let Ok(p) = item::ItemRepaired::read_body(&mut cur) {
                server_events.write(ServerEvent::ItemRepaired {
        ...
        x if x == ServerPacketIds::ItemSlotSizeChanged as i16 => {
            if let Ok(p) = item::ItemSlotSizeChanged::read_body(&mut cur) {
                server_events.write(ServerEvent::ItemSlotSizeChanged {
                    unique_id: p.unique_id,
                    slot_size: p.slot_size,
                });
                tracing::debug!("📐 物品槽位变化 uid={} size={}", p.unique_id, p.slot_size);
            }
        }
