...
/// C# `Gold.ToString("###,###,##0")`（`MailDialogs.cs:1221`）：千分位、0 → `""`? 不，
/// C# 的 `"###,###,##0"` 对 0 会输出空串，故 C# 只在有赠金时才看得见数字；本端保留
/// **同一条格式串**的等价实现（0 → 空），避免"0"这种原版不会出现的文本。
...
mod tests {
    ...
    /// 赠金文本 = C# `Gold.ToString("###,###,##0")`：千分位、0 → 空串（原版不显示 0）。
    #[test]
    fn gold_label_matches_csharp_thousands() {
        assert_eq!(format_gold(0), "");
