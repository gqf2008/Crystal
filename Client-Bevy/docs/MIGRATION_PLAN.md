...
### M61: 服务端 NPC/怪物登出清理（2026-08-03 完成）
- [x] 断线/登出时若该地图已无其他玩家 → 清理该地图全部 NPC/怪物（修复多次登录后地图 NPC 重复泄漏）
...
- [x] 已知遗留（非本次范围）：服务端 NPC 登出未清理；mount 无 RideMount 包（坐骑对话框暂无价值）；M10/M11 深度玩法（魔法 F1-F8 排布、动画系统打磨）为后续迭代
- [x] 对话框里程碑全部完成：56 个对话框骨架 → 数据驱动 + 真实 ServerRust 联调（Issue #12 每里程碑 E2E 证据）
