...
- Hero：C# 英雄装备/技能是**同一个 `CharacterDialog`** 的页（4 个页签 `CharacterButton`/`StatusButton`/
  `StateButton`/`SkillButton` @(8/70/132/194, 70) 64x20，`CharacterDialog.cs:146-200`）；本端英雄侧是三个独立窗
  （`HeroInventory`/`HeroEquipment`/`HeroSkill`，均 `Title[504]` @(760,0)），面板/页/槽位与 C# 一致
  （`hero_equipment_dialog_aligned`/`hero_skills_dialog_aligned`），但**4 个页签按钮未实现**（切窗靠各自热键）。
