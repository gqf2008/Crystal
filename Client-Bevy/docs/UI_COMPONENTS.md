...
- Hero：C# 英雄装备/技能/状态/状态二页是**同一个 `CharacterDialog`** 的 4 页（页签 `Title[500..503]`
  @(8/70/132/194,70) 64x20，`CharacterDialog.cs:146-200`）。**批57 已补齐页签与状态两页**（见 `hero_pages.rs`），
  页内容与文案逐条对齐 C#；**仍存差异**：本端英雄侧是**两个独立窗**（`HeroEquipment` 承载装备/状态/状态二页、
  `HeroSkill` 承载技能页、`HeroInventory` 另立），而 C# 是一个 dialog 的 4 页 —— 拖动与显隐按窗聚合
  （页签已按 C# 语义路由到对应窗，用户可见行为一致；窗口拖动仍是各窗独立）。
