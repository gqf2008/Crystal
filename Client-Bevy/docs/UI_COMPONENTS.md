...
- `cargo test`（Client-Bevy）：**514 lib** + 2 bin + 1 smoke + 24 alignment 通过（批24 收尾 + 后续修复「固定排不过滤」后基线，含合并前在 `fix/bevy-questdetail24u3` 本机复跑与修复分支复跑）；ServerRust **680 lib** + 6 integration（同上，含 `QuestItemReward` ItemInfo 协议）；SharedRust **187 + 11**（2 ignored，含 `QuestItemReward` 往返无损）。
