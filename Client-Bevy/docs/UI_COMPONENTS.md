...
| 批58 #2892 英雄装备/技能两窗合并回 C# 的单窗四页结构 | C# 英雄侧是**一个** `HeroDialog = CharacterDialog(MirGridType.HeroEquipment, hero)`（四页同窗、拖动整窗，`HeroEquipment` 用的 `Title[504]`@(760,0)）+ 独立的 `HeroInventoryDialog`。批57 已补齐四页签与状态两页，但本端仍是 `HeroEquipment`/`HeroSkill` **两个窗**（拖动/显隐按窗聚合）。本批按 C# 合并：① `HeroSkill` 独立 kind 与其窗口删除，技能页（`Title[508]` + 7 行 + 行内容/点击分配）改为 `spawn_hero_skill_page` 挂进同一个英雄窗的页容器；`HeroSkillPlugin` 只保留行内容刷新系统 ② 页签与快捷键/HUD 英雄菜单改为**同窗切页**（`HeroPageState`）：装备/技能键按 C# `GameScene.cs:581-606`（`!Visible || !CharacterPage.Visible → Show()+ShowXPage()`）语义「已在目标页再按 → 关窗，否则开窗并切页」；HUD 英雄菜单三钮同义（`HeroDialogs.cs:396-455`）③ `control.rs` 的 `hero_skill` 降为**别名**（→ `HeroEquipment`，同 `npc_drop`/`trust_merchant` 先例），`DialogKind` 变体数 49→48 ④ 对齐测试合并：技能页断言并入 `hero_equipment_dialog_aligned`，覆盖守卫计数同步。**结果：英雄侧与 C# 同构（一个 dialog 四页 + 独立英雄背包窗）**。 | 本 PR |
| 批57 #2892 英雄对话框补 C# 的状态页/状态二页 + 四页签 | C# `HeroDialog = new CharacterDialog(MirGridType.HeroEquipment, Hero)`（`CharacterDialog.cs:27-200`）——英雄窗与角色窗同源，含 **4 个页签**（`CharacterButton`/`StatusButton`/`StateButton`/`SkillButton` = `Title[500..503]` 64x20 @(8/70/132/194,70)）与 **4 页**：装备页 `Prguse[340]`、状态页 `Title[506]`、状态二页 `Title[507]`（均 @(8,90)，标签 x=126、y=20+18i）、技能页 `Title[508]`。本端此前英雄侧只有装备页/技能页两个窗、**状态两页与页签整体缺失**。本批：① **协议** `S.HeroInformation` 补英雄属性块（AC/MAC/DC/MC/SC、暴击率/伤害、攻速、命中、敏捷、幸运、魔法/毒躲避、体力/魔法/毒恢复、神圣、冰冻、毒攻击 + 当前背包/穿戴/手持重量与三项上限，共 30 个 i32；`HeroInformation : UserInformation` 的 `Stats` 子集），并修掉 `experience` 之前硬编码 0 的问题② **服务端** `hero_stats.rs` 扩属性（装备加成复用 `EquipmentBonuses`）、新增 `hero_current_bag_weight`/`hero_current_wear_weights`（C# `HumanObject.cs:1782-1834`：背包 Σ`Weight*Count`；武器/火把计手持、其余计穿戴）③ **客户端** 新增 `hero_pages.rs`：四页签 + 状态页 13 标签 + 状态二页 12 标签（文案逐条复刻 `CharacterDialog.cs:96-132`，如 `"{hp}/{max}"`/`"{min}-{max}"`/`"+{v}"`/经验 `"{:.2}%"`），页签切换改共享 `HeroPageState` 并按页路由窗口 ④ 页根显隐（装备页/技能页各归其窗、状态两页就地切换）。**仍存差异**：本端英雄侧是**两个独立窗**（C# 是同一个 dialog 的 4 页），拖动/显隐按窗聚合（见 §7）。 | 本 PR |
| 批56 #2892 批B 收口（4 窗补断言 + 全覆盖守卫 + 删自造技能窗） | ① **删除 C# 不存在的独立技能窗**：C# 技能列表是 `CharacterDialog.SkillPage`（`Title[508]` @(8,90)，`CharacterDialog.cs:136-143`），靠 `SkillButton` 切页；本端此前另有一个 `DialogKind::Skills` 独立窗（`Title[508]`@(360,180) + 自造滚动列表），**无任何打开入口**（键位注释亦为「无独立技能窗口，打开角色对话框技能页」）→ 按 C# 结构删除（`skills.rs` 窗实现 + `DialogKind` 变体 + control RPC 名 + 计数断言同步）② 补 4 个此前只有 smoke 插件注册、**无任何对齐断言**的窗口：设置（`Title[411]` 259x354 `Center`）、举报（`Prguse[1633]` `Center`）、英雄装备（`Title[504]`@(760,0) + `Prguse[340]`@(8,90) + 14 槽）、英雄技能（`Title[504]` + `Title[508]`@(8,90) + 7 行）③ 新断言暴露并修掉 3 处真实偏差：设置窗原点按 C# **整除**应为 (382,207)（此前浮点除 → 382.5）、举报窗关闭键应 art 24x21（此前自造 20x20）、英雄装备/技能窗关闭键同样 20x20→24x21（并记录 C# 原版右缘就出面板 1px）④ 新增**批B 验收守卫** `every_dialog_kind_has_alignment_coverage`：49 个 `DialogKind` 逐个登记覆盖它的对齐测试名（穷尽 match：新增窗口漏登记会**编译失败**），并断言登记名在源码真实存在、判别值 0..49 无缺口。 | #2938 |
| 批54 #2892 钓鱼状态窗按 C# `S.FishingUpdate` 收口（进度/机会条 + 抛竿钮 + ESC 退出） | ① **协议**：`FishingUpdate(198)` 由自造 `[progress i32][success u8]` 相位码改为 C# `S.FishingUpdate` 字段形（`ObjectID + Fishing + ProgressPercent + ChancePercent + FoundFish`，服务端 `PlayerObject.GetFishInfo`，`PlayerObject.cs:11232-11245`）；服务端新增 `fishing_progress_percent`（C# `_fishCounter / FishingProgressMax * 100` 截断，`:11234`）与 `send_fishing_update`，抛竿/等待每 tick/咬钩/收竿/自动续抛全部改发百分比，自动钓鱼开关不再借 `FishingUpdate` 传（C# 客户端点按钮本地切 `AutoCastBox.Index`，`FishingDialog.cs:260-271`）② **机会条/进度条**：由「容器 + 绿色填充近似」改为 C# `BeforeDraw` 自绘的裁绘精灵 —— 机会条 `Prguse[1342]` @(14,64) 裁 `(0,0,2.16*ChancePercent,12)`、进度条 `Prguse[1349]` @(14,79) 裁 `(0,0,2.16*ProgressPercent,8)`（`:347-374`），`ChanceLabel` 收回 C# @(14,62) 216x12 居中并显示 `"{ChancePercent}%"` ③ **抛竿钮**：`FishButton(Title[170..179])` 只在咬钩（`FoundFish`）时显示，否则显示禁用帧 `Title[149]`（`PlayerObject.cs:2591-2597`）④ **ESC 退出勾选**：`Prguse[1346]` 做成可点按钮（三态同帧）+ `Prguse[1347]` 选中图 + `Title[45]` 文案，勾选状态进 `FishingState.esc_exit`，ESC 键只在勾选时取消钓鱼（`GameScene.cs:698` → `Cancel()` 发 `C.FishingCast{CastOut=false}` 并关状态窗，`FishingDialog.cs:376-382`）⑤ **状态窗显隐**改由 `S.FishingUpdate.Fishing` 驱动（`GameScene.cs:3056-3059`），不再「跟主窗」。 | #2936 |
| 批53 #2892 行会公告页补位置条与滚轮滚动 | `NoticePositionBar` = `Prguse2[206]` @(337,16)（C# `GuildDialog.cs:301-311`，`Movable`）：拖动 → `index = floor((y-16)/interval)` 夹 `[0, len-25]`（`:1356-1389`）；位置 `y = 16 + index*interval`，`interval = 289 / (len-25)`（**int/int 截断除**），y 夹 `[16, NoticeDownButton.Y-20 = 298]`（`:1343-1354`）；滚轮 `NoticePanel_MouseWheel` 光标在页内生效、到顶/到底停住（`:1390-1410`）。公告不足一屏（`len <= 25`）时位置条隐藏（C# 该分支会除零，本端按不可滚动处理）。 | #2930 |
| 批52 #2892 可见性两档（`Hidden` / `Sneaking`）与 ClearRing 语义 | C# `MapObject.AddBuff`（`MapObject.cs:654-667`）：`Hiding`/ClearRing 宝石只置 `Hidden`（`S.ObjectHidden` → 客户端 `MapObject.cs:5006` `SetOpacity(0.5F)`）+ `HideFromTargets()`；`MoonLight`/`DarkBody` 额外置 `Sneaking`（`MapObject.cs:112-131` → `Observer` → `S.ObjectRemove`，对他人完全消失）。本端拆为 `hidden_sessions`（Hidden）+ `gm_observer_sessions` + `invisible_sessions`（Observer），新增 `sync_player_visibility` 统一重算（宝石装卸、Hiding/MassHiding、MoonLight/MoonMist、DarkBody、NPC GIVEBUFF 两套脚本引擎、攻击/施法破隐、登录、10s 过期检查），`set_sneaking` 接 `SneakingActive → Observer`；ClearRing 判定走单一真源 `has_special_equipped(...CLEAR_RING)`；`SPELL_DARK_BODY` 补 `AddBuff(DarkBody)`；`ObjectPlayer` 携带真值 `hidden`。**客户端**（PR #2929）：新增 `HiddenObject` 唯一真源 + `apply_hidden_alpha`（只改本对象图层，修掉旧实现「遍历全部 `SpriteLayer` 把全场变半透明」），`S.ObjectHidden(147)` 接进事件总线，`NetObject::Player.hidden` 在生成/重发两条路径落地。 | #2928 #2929 |
| 批51 #2892 三种隐身的可见性规则按 C# 分档 | C# `MapObject.AddBuff`（`MapObject.cs:654-667`）：`Hiding`/`ClearRing` 只置 `Hidden`（`S.ObjectHidden`，客户端 `MapObject.cs:5006` 画 50% 透明）+ `HideFromTargets()`；`MoonLight`/`DarkBody` 额外置 `Sneaking`（`MapObject.cs:112-131` → `Observer` → `S.ObjectRemove`，对他人完全消失）。本端此前三者共用一套「对他人移除」判定：服务端新增 `is_sneaking_type`（MoonLight/DarkBody），把 3 处「对他人移除」语义（进图插入 `invisible_sessions`、`send_map_players_to` 过滤、移动广播抑制）与过期检查改用它；怪物 AI 的 `Hidden` 判定（`tick.rs`）仍用 `is_invisible_type`；客户端 `ObjectHidden` 由「本地 0.35 / 远程完全隐藏」改为 **一律 50% 透明**（对齐 C# `SetOpacity(0.5F)`），`ObjectShown` 恢复不透明。 | #2927 |
| 批50 #2892 钓鱼窗按 C# 拆成两窗 | 主窗 `FishingDialog`（`Prguse[1340]` 200x287 @ `Center`，`Movable`）恢复 C# 结构：标题 `@(10,4)`180x20、关闭 `Prguse2[360..362]` 改回 C# `@(175,3)` 24x21（原自造 20x20）、钓具格 5 个 34x30 保持 C# 坐标；**新增状态窗** `FishingStatusDialog`（`Prguse[1341]` 244x128 @(390,300)，独立 `DialogKind::FishingStatus` 保证两窗各自拖动）：`ProgressBar`@(14,62)216x12（填充宽度按 `progress` 0..3）、`ChanceLabel`@(14,79)、关闭`Prguse2[360..362]`@(216,4)、抛竿`Title[149]`禁用/`Title[170..179]`+按下`142`@(47,95)60x25、自动钓鱼`Title[180..182]`@(110,95)48x25、勾选框`Prguse[1343/1344]`@(172,95)、ESC 勾选`Prguse[1346/1347]`@(135,41)。差异：进度/机会条是容器+填充近似（C# `BeforeDraw` 自绘）、抛竿禁用帧未按「有无鱼竿」切换、ESC 勾选恒不勾（本端 ESC 直接关窗）——**三条已在批54 消除**（见上）。 | #2926 |
| 批49 清理好友窗自造输入链路 | 删除已无引用的好友窗内嵌输入框：`FriendInputBox` 组件与 spawn 块、`FriendPending::Add`、`TextInputSubmit` 消费块（原「内嵌框 + 自己发 `C.AddFriend`」链路）、`submits`/`input_box` 两个系统参数与其 `Without` 过滤、`show_input` 显隐逻辑。**行为等价论证**：该链路唯一的生产者（「添加」）在批48 已改为 `MirInputBox`、「备注」在批45 已改为 `MemoDialog`，`pending` 只余 `Memo` 且被 `friend_memo_open_system` 同帧消费 → 被删代码不可达；`rg FriendInputBox|submits|内嵌输入提交` 已零命中，`cargo test` 全绿（589 lib + 44 alignment）。 | #2925 |
| 批48 #2892 批D 收尾（好友「添加好友」走 C# `MirInputBox`） | `input_box.rs` 增客户端发起式入口 `open_input_box(...)` + `InputPurpose::AddFriend{blocked}`（OK → `C.AddFriend{Name, Blocked}`，文案 `FriendEnterAddName`/`FriendEnterBlockName` 逐字取 `Chinese.json`）；好友窗「添加」只记 `FriendState.add_request`，由 `friend_add_open_system` 打开（16 参上限）。内嵌 `FriendInputBox` 不再使用（保持隐藏，清理待后续）。 | #2924 |
| 批47 #2892 批B 收尾（行会公告页改多行可编辑框） | `Notice` 区域（322x330 @(13,1)）由「20 行只读 + 单行输入」改为**多行可编辑框**：裁剪容器 + `TextInputField(2)` + `TextInputMultiline`（批46 基建），显示实体定宽 316 折行；翻页钮（C# `Prguse2[197..199]`/`[207..209]` @(337,1)/(337,318)）改为平移显示实体 `top = 2 - scroll*16`（等价 C# `UpdateNotice` 的 `ScrollToCaret()` 逐行滚动）；保存键 `Prguse[554..556]` @(20,342) 不变。记录：C# 翻页钮右缘 353 比页宽 352 多 1px；未做 `NoticePositionBar`（`Prguse2[206]`）与滚轮滚动（**批53 已补**，见上）。 | #2923 |
...
| 批43 #2892 批D 单元①续（聊天窗拖动） | `chat_drag_system`：按「与上一帧偏移差」整体平移 `ChatPanelBg`/`ChatLine`/`ChatScrollBtn`/`ChatScrollTrack`/`ChatScrollKnob`/`ChatInputBg`/`ChatInputText`/`ChatInputCursor`（sprite 层无父子层级，y 轴朝上故位移取负），差值记 `ChatState::applied_drag` 防重复累加；`chat_size_system` 登记基准矩形 `(230, chat_panel_top(size))` + 档位 art 尺寸。C# 依据：`ChatDialog` 含面板/行/滚动/`ChatTextBox`（`MainDialogs.cs:585-601`）整体 `Movable`，`ChatControlBar` 是另一控件不随动。 | #2919 |
...
- 批58（英雄两窗合并为单窗四页）门禁：Client-Bevy `cargo test` 597 lib + 2 bin + 1 smoke + **49 alignment** 全绿；
  `fmt -- --check` 0 差异；`control::parse_dialog_kind_covers_all_variants` 计数同步（48 名/45 唯一变体，`hero_skill` 为别名）。
  **阳性对照**：把英雄快捷键改回「异窗」语义（按技能键不切页）→ `hero_hotkeys_require_hero_and_are_page_aware` 如期 FAILED；
  另外 `DialogKind` 少删/多删变体会让 `ui_alignment::every_dialog_kind_has_alignment_coverage`（判别值缺口）与
  `control::has_rpc_mapping` 的穷尽 match 编译期报错。
- 批57（英雄状态页/状态二页 + 四页签）门禁：三侧 `cargo fmt -- --check` 0 差异；Client-Bevy 597 lib + 2 bin + 1 smoke +
  50 alignment、ServerRust 746 lib + 6 integration、SharedRust 187 + 11 全绿。两条**阳性对照**：
  ① `hero_stat_texts_match_csharp` 里暴击率去掉 `%` → FAILED（left `15` / right `15%`）；
  ② 页签 x 由 132 改 133 → `hero_tabs_match_csharp` FAILED；均改回即绿。
  另有 `SharedRust::hero_information_roundtrip` 与 `handle_progress::hero_information_decode_to_server_event`
  逐字段钉住属性块 wire（首/尾 + 中段六个值）。
- 批56（#2892 批B 收口）门禁：Client-Bevy `cargo test` 595 lib + 2 bin + 1 smoke + **50 alignment** 全绿；
  三侧 `cargo fmt -- --check` 0 差异。三条**阳性对照**：
  ① 设置窗原点改回浮点除 → `settings_dialog_aligned` 如期 FAILED（left 382.5 / right 382）；
  ② 覆盖率守卫把 `K::Report` 的登记清空 → `every_dialog_kind_has_alignment_coverage` 如期 FAILED（非空断言）；
  ③ 登记名改成 `report_dialog_aligned_typo` → 同测试如期 FAILED（源码存在性断言）；均改回即绿。
- 批51 收尾（#2932 修 master 常驻 clippy 红）：CI 固定 1.95.0 而本机默认 1.97，两处既有 lint
  （`SharedRust special_systems.rs:483-485` rustdoc 列表缩进、`ServerRust player.rs:2856` `clone_on_copy`
  ——修后再暴露 `social.rs` 的 `needless_borrow`）让**每个 PR 的 check 都带红**。已按 CI 工具链修复：
  `cargo +1.95.0 clippy --lib -- -D warnings` / `fmt -- --check` / `test --lib` 三侧全绿，
  阳性对照=还原 `special_systems.rs` 后本地 1.95 clippy 如期复现 483/485 两条红。
- 批55（#2934 走/跑破隐粒度）：C# `HumanObject.cs` 攻击 `:2869-2886` / 施法 `:3418-3422` 只破
  `MoonLight`/`DarkBody`（`Hiding`/ClearRing 不受影响）、走 `:2460-2463` 只破 `Hiding`、
  跑 `:2540-2545` 在 `Hidden && !Sneaking` 时破三档；本端此前 `RemoveBuff` 把三档一起清且走/跑不破隐。
  门禁：`cargo +1.95.0 clippy/fmt/test` 全绿（ServerRust 746 lib）；阳性对照=删掉 `move_break_buffs`
  的 `!Sneaking` 守卫 → `move_break_matches_csharp_walk_run` 如期 FAILED，改回即绿。
- 批52（可见性两档 + ClearRing）门禁：ServerRust `cargo fmt -- --check` 0 差异；`cargo test` 743 lib + 6 integration 全绿。
  **阳性对照**：把 `combat::buff::has_sneaking` 改成 `is_invisible_type` → `hidden_and_sneaking_are_separate_tiers` 如期 FAILED，改回即绿。
  客户端接线（PR #2929）：Client-Bevy 591 lib + 45 alignment 全绿；**阳性对照**：把 `apply_hidden_alpha` 改回「遍历全部
  `SpriteLayer` 设 alpha」→ `hidden_alpha_only_affects_target_object` 如期 FAILED（旁观者 0.5 ≠ 1.0），改回即绿。
- 批53（行会公告位置条 + 滚轮）门禁：Client-Bevy 590 lib + 45 alignment 全绿。
  **阳性对照**：把 `notice_bar_interval` 改成 f32 除 → `notice_bar_and_wheel_match_csharp` 如期 FAILED（11.56 ≠ 11），改回即绿。
- 批54（钓鱼状态窗）门禁：三侧 `cargo fmt -- --check` 0 差异；Client-Bevy 592 lib + 2 bin + 1 smoke + 45 alignment、
  ServerRust 744 lib + 6 integration、SharedRust 187 + 11 全绿。三条**阳性对照**：
  ① `fishing_progress_percent` 的截断改四舍五入 → FAILED（7 ≠ 6）；
  ② `fishing_bar_width` 改浮点比例 → FAILED（28.08 ≠ 28）；
  ③ ESC 处理去掉 `fishing.esc_exit &&`（= 无条件取消）→ `esc_cancels_fishing_only_when_esc_exit_checked` FAILED。均改回即绿。
- `cargo test`（Client-Bevy）：**597 lib** + 2 bin + 1 smoke + **49 alignment** 通过（批50 后基线（批49 加清理不增用例）（批48 同为 589 lib，批49 为纯删除）；批47 时为 588 lib + 44 alignment，批46 时为 588 lib + 43 alignment，批45 时为 587 lib + 43 alignment，批44 时为 587 lib + 42 alignment，批43 时为 586 lib + 42 alignment，批42 时为 585 lib + 42 alignment，批41 时为 583 lib + 42 alignment，批40 时为 583 lib + 42 alignment（ServerRust 740 lib），批39 单元⑫ 时为 583 lib + 42 alignment（ServerRust 739 lib），批38 单元⑪ 时为 582 lib + 42 alignment，批37 单元⑩ 时为 581 lib + 42 alignment，批36 单元⑨ 时为 579 lib + 42 alignment，批35 单元⑧ 时为 579 lib + 41 alignment，批34 单元⑦ 时为 579 lib + 40 alignment，批33 单元⑥ 时为 579 lib + 39 alignment，批32 单元⑤ 时为 576 lib + 38 alignment，批31 单元④ 时为 571 lib + 37 alignment，批28 单元③ 时为 542 lib + 24 alignment，批27 收尾时 535 lib，批26 收尾时 528 lib，批25 收尾时 523 lib，批24 收尾时 514 lib）；ServerRust **746 lib** + 6 integration（批57 未增 ServerRust 用例数）（含 #2934 走/跑破隐 2 条）（同上，含 `QuestItemReward` ItemInfo 协议 + `ClientGTMap` 字段）；SharedRust **187 + 11**（2 ignored，含 `QuestItemReward` 往返无损）。三侧 `cargo fmt -- --check` 净零差异。
...
- 批43（#2892 批D 单元①续：聊天窗拖动）门禁：`cargo fmt -- --check` 0 差异；Client-Bevy `cargo test` 586 lib + 2 bin + 1 smoke + 42 alignment 全绿
  （新增 `chat::chat_drag_offset_moves_all_parts_once`）。
  **阳性对照**：去掉 `chat.applied_drag = want;`（不记录已应用偏移）→ 第二次运行会再平移一次，
  该测试的「同一偏移重复应用不再移动」断言如期 FAILED（250 ≠ 240），改回即绿。

- 批44（#2892 批D 单元①续：下拉框拖动）门禁：`cargo fmt -- --check` 0 差异；Client-Bevy `cargo test` 587 lib + 2 bin + 1 smoke + 42 alignment 全绿
  （新增 `ui::theme::tests::dropdown_popup_rect_includes_drag_offset`）。
  **阳性对照**：把 `popup_rect` 改回只用 `popup_pos`（忽略拖动偏移）→ 该测试如期 FAILED
  （`(100,50,80,56) != (112,42,80,56)`），改回即绿。

- 批45（#2892 批D 单元①收尾：好友备注拆窗）门禁：`cargo fmt -- --check` 0 差异；Client-Bevy `cargo test` 587 lib + 2 bin + 1 smoke + 43 alignment 全绿
  （新增 `panel_sprites_batch_b18_match_memo_dialog`）。
  **阳性对照**：把 `PANEL_SIZE` 改成 (196,165) → 该测试如期 FAILED（`(196,166) != (196,165)`），改回即绿。

- 批46（多行文本输入基建）门禁：`cargo fmt -- --check` 0 差异；Client-Bevy `cargo test` 588 lib + 2 bin + 1 smoke + 43 alignment 全绿
  （新增 `text_input::tests::multiline_enter_inserts_newline_instead_of_submit`：同一 world 跑两次，
...
- 批48（#2892 批D 收尾：好友添加走 MirInputBox）门禁：`cargo fmt -- --check` 0 差异；Client-Bevy `cargo test` 589 lib + 2 bin + 1 smoke + 44 alignment 全绿
  （新增 `input_box::tests::open_input_box_sets_state_and_focuses`）。
  **阳性对照**：去掉 `open_input_box` 里的 `input.active = Some(INPUT_FIELD_ID);` → 该测试如期 FAILED
  （`None != Some(40)`），改回即绿。

- 批49（清理好友窗自造输入链路）门禁：`cargo fmt -- --check` 0 差异；Client-Bevy `cargo test` 589 lib + 2 bin + 1 smoke + 44 alignment 全绿。
  **行为等价论证**（纯重构硬验收）：被删的 `FriendInputBox` + `TextInputSubmit` 链路只服务于「添加好友」与「备注」；
  前者批48 改走 `MirInputBox`（`add_request` → `friend_add_open_system`）、后者批45 改走 `MemoDialog`
  （`pending = Memo(idx)` 由 `friend_memo_open_system` 同帧消费）→ 删除的代码不可达；
  `rg -n "FriendInputBox|submits|内嵌输入提交" Client-Bevy/src/game/dialogs/friend.rs` 零命中，
  测试与批48 完全一致（589+2+1+44 全绿）。

- 批50（钓鱼窗拆两窗）门禁：`cargo fmt -- --check` 0 差异；Client-Bevy `cargo test` 589 lib + 2 bin + 1 smoke + 45 alignment 全绿
  （新增 `panel_sprites_batch_b23_match_fishing_split`：两窗面板尺寸/Center 取整坐标/子控件精灵与包含关系/钓具格 5 格）。
  **阳性对照**：把 `STATUS_SIZE` 改成 (244,127) → 该测试如期 FAILED（`(244,128) != (244,127)`），改回即绿。
  另：拆分后 `visibility` 两个查询同改 `Visibility` 触发 **B0001**（b0001_smoke 变红）→ 加 `Without<FishingWidget>` 显式互斥后恢复。

- 批51（三种隐身可见性分档）门禁：三侧 `cargo fmt -- --check` 0 差异；ServerRust `cargo test --lib` **742 passed**
  （新增 `combat::buff::tests::invisibility_levels_match_csharp`）+ 6 integration；Client-Bevy 589 lib + 2 bin + 1 smoke + 45 alignment。
  **阳性对照**：把 `is_sneaking_type` 改成与 `is_invisible_type` 等价（三者都算 sneaking，即修正前的合并行为）→
  该测试的「`Hiding` 只置 Hidden，不置 Sneaking」断言如期 FAILED，改回即绿。
...
- 悬停提示（`MirControl.Hint`）覆盖长尾：C# 侧 `Hint = ` 共 **203 处**（`rg -c "Hint\s*=" Client/MirScenes`，其中 MainDialogs 149）。**已对齐**：HUD 主按钮 8 个（含键位）、小地图条 3 个（邮件/大地图/小地图）、菜单窗 13 个（含键位）、物品/商品/角色格（`item_tooltip_lines`）、头顶名字（玩家/怪物/NPC）、技能栏格（`SkillMpCooldownKey`）、大地图（搜索NPC/队友名）、玩家右键菜单 5 项、Mail（发送/读取/删除）、Group（允许拒绝队伍请求/添加/移除/成员名）、Friend（添加/移除/备注/邮件/悄悄话）、Ranking 6 页签、Relationship 5 项、Mentor 3 项、GuildTerritory 4 项（退出/翻页/购买）、Hero 行为 4 项 + 英雄/药水腰带 2 项、耐久面板钮、设置窗 2 条音量滑条、**技能页 7 行魔法格**（103 条技能描述，`dialogs/skill_desc.rs`，按 C# `Spell` 分派）。
  ...
  **批52 已按 C# 拆成两档**（服务端 `MapObject.cs:654-667`）：`Hidden`（`Hiding`/ClearRing/`MoonLight`/`DarkBody`
...
- 聊天窗拖动（批43 / #2892 批D 单元①续 已对齐）：C# `ChatDialog` 是一个容器
  ...
  `ChatControlBar` 是**另一个**控件（批20 落地），不随动。
  本端聊天窗是 sprite 层的一堆独立实体（无父子层级），故新增 `chat_drag_system`：
  按「与上一帧的偏移差」对 `ChatPanelBg`/`ChatLine`/`ChatScrollBtn`/`ChatScrollTrack`/`ChatScrollKnob`/
  `ChatInputBg`/`ChatInputText`/`ChatInputCursor` 的 `Transform` 整体平移，差值记在
  `ChatState::applied_drag` 避免重复累加（sprite 层 y 轴朝上 → 屏幕位移取负）；
  `chat_size_system` 里登记窗口基准矩形（`(230, chat_panel_top(size))` + 档位 art 尺寸）。
  `window_drag_system` 命中后即可拖动，钳位与光标换算沿用批42 的公共实现。
...
- 快捷键语义（批28 已对齐；基准 `GameScene.cs:532-711` 的键盘 `switch`）：
  ...
  - **`Closeall`（ESC）集合**：C# 是**枚举**窗口（`:669-708` + `NPCDialog.Hide()` 级联 `NPCDialogs.cs:1026-1037`），
    ...
    `FishingStatusDialog.bEscExit` 的 `Cancel()`（`:698`）批54 已按 C# 落地：ESC 只在勾选「ESC 退出」时
    发 `C.FishingCast{CastOut=false}` 并关状态窗（`dialogs/fishing.rs::cancel_fishing`）；
    主窗仍由 Closeall 的 `FishingDialog.Hide()`（`:681`）关闭，状态窗另由 `S.FishingUpdate.Fishing` 驱动。
...
- Hero：C# 英雄装备/技能/状态/状态二页是**同一个 `CharacterDialog`** 的 4 页（页签 `Title[500..503]`
  @(8/70/132/194,70) 64x20，`CharacterDialog.cs:146-200`），英雄背包另立窗。**批57 补页签与状态两页、
  批58 把装备/技能两窗合并回单窗四页**（`hero_pages.rs` + `hero_equipment.rs`，`DialogKind::HeroSkill` 已删），
  拖动/显隐/快捷键/HUD 菜单均按「一个 dialog 四页」；本端英雄侧现与 C# **同构**，无遗留结构差异。
- Report：数据包缺 `Prguse[1633]`（面板图），本端用同尺寸 360x244 深色兜底面板并按 `Center` 落位；
  C# 原版在缺图时 `Size` 为 0（等于什么都不画）——本端取「可用的可见窗」而非「不可见」（属资源包缺失类偏差）。
- 技能列表：C# **无独立技能窗**（`CharacterDialog.SkillPage` + `SkillButton` 切页）；本端此前自造的
  `DialogKind::Skills` 独立窗（`Title[508]`@(360,180)）无任何打开入口，**批56 已按 C# 结构删除**。
...
- 行会 NoticePage（批38 / #2892 批B 单元11 正文渲染，批47 已并入多行编辑）：
  ...
  **（2026-09-15 更正）**：该条「编辑入口仍是单行输入框」的描述**已过时**——批47 已把 `Notice` 区域改成
  多行可编辑框（见上文 698-713 行），当前真实差异只剩「C# 保存/编辑键是同位置二选一显示，本端只挂保存键」。
