...
use ggez::graphics::{Canvas, Color, DrawParam, Drawable, Rect};
...
impl BuffDialog {
    ...
    pub fn add_debuff(&mut self, debuff: BuffItem) {
        if let Some(existing) = self
            .debuffs
            .iter_mut()
            .find(|d| d.buff_type == debuff.buff_type)
        {
            *existing = debuff;
            return;
        }
    ...
    pub fn update_hover(&mut self, mouse_x: f32, mouse_y: f32) {
        ...
        for (i, _buff) in self.buffs.iter().enumerate() {
            ...
            if mouse_x >= icon_x
                && mouse_x <= icon_x + self.icon_size
                && mouse_y >= icon_y
                && mouse_y <= icon_y + self.icon_size
            {
        ...
        if self.show_debuffs {
            ...
            for (i, _debuff) in self.debuffs.iter().enumerate() {
                ...
                if mouse_x >= icon_x
                    && mouse_x <= icon_x + self.icon_size
                    && mouse_y >= debuff_y
                    && mouse_y <= debuff_y + self.icon_size
                {
