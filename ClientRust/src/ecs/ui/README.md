...
UI 组件（聊天窗口、血条、技能栏等）位于此目录或其子模块。

要点：

- UI 通常以 `GameContext` 为输入（获取 `gfx`、`world`、`frame_input_events` 等）。
- 避免在 UI 直接修改网络状态；UI 只触发本地事件或将请求发送到网络系统。

建议：将大型 UI（例如背包/交易窗口）拆成子组件并提供自测场景，便于独立调试。
# ecs/ui - 游戏UI系统
