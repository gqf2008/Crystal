...
此目录包含游戏场景（Scene）实现与场景转换逻辑，例如登录场景、选角场景与主游戏场景。

常见类型：

- `GameScene`：主游戏场景，负责游戏循环中的渲染与逻辑驱动。
- `LoginScene`：登录 UI 与网络认证流程。
- `SelectScene`：角色选择界面与相关 UI。

要点：

- 场景通常持有对 `GameContext` 的可变引用，以访问 `world`、图形上下文与输入。
- 避免在场景中直接进行长时间阻塞操作；如需 IO，请移交给独立任务或网络模块。
# ecs/scenes - 游戏场景系统
