...
impl SelectScene {
        ...
        for event in events.iter() {
            match event {
                ...
                GameEvent::UserInformation {
                    ...
                    tracing::info!(
                        "✅ 收到UserInformation，玩家位置({}, {})，切换到游戏场景...",
                        location_x,
                        location_y
                    );
    ...
    fn handle_delete_character_success(&mut self, character_index: i32) {
        ...
        if self
            .character_select_ui
            .remove_character_by_index(character_index)
        {
            tracing::info!(
                "📋 已从列表移除角色 (index={}), 剩余角色数: {}",
                character_index,
                self.character_select_ui.character_count()
            );
        }
