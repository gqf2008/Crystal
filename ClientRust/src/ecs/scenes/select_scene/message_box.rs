        ...
        let (ok_button, yes_button, no_button) = match buttons {
            ...
            MessageBoxButtons::YesNo => {
                (
                    ...
                    Some(Button::new(
                        x + Self::OFFSET_YES_BUTTON_X,
                        y + Self::OFFSET_YES_BUTTON_Y,
                        LibraryName::Title,
                        206,
                    )),
                    // C# 原版: No按钮索引210(Normal), 211(Hover), 212(Pressed)
                    Some(Button::new(
                        x + Self::OFFSET_NO_BUTTON_X,
                        y + Self::OFFSET_NO_BUTTON_Y,
                        LibraryName::Title,
                        210,
                    )),
                )
    ...
    pub fn draw(&self, ctx: &mut ggez::graphics::GraphicsContext, canvas: &mut Canvas) -> anyhow::Result<()> {
        ...
        for line in self.message.lines() {
            ...
            text.set_scale(PxScale::from(14.0))
                .set_font("AlibabaPuHuiTi"); // 使用中文字体
            ...
            canvas.draw(
                ...
                ggez::graphics::DrawParam::default()
                    .dest([
                        self.x + Self::OFFSET_TEXT_X,
                        self.y + Self::OFFSET_TEXT_Y + y_offset,
                    ])
                    .color(Color::WHITE),
            );
