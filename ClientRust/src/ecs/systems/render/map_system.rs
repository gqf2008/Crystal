...
impl DrawSystem for MapRenderSystem {
    ...
    fn draw(
        ...
        for (layer, should_show) in layers.iter() {
            ...
            // 步骤3.4: 收集静态瓦片（排除动画瓦片）
            // ================================================================
            if config.show_static_tiles {  // 检查静态瓦片开关（S键切换）
            ...
            if config.show_animated_tiles {  // 检查动画瓦片开关
                // 根据 show_animations 决定是否播放动画
                // 暂停时仍显示第一帧，只是不更新帧索引
                if config.show_animations {  // 检查动画播放开关
                    // 查询所有拥有MapTile和AnimatedTile的实体，并筛选当前层
