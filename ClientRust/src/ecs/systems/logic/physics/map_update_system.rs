...
use crate::ecs::components::{
    Camera, Draggable, LocalPlayer, MapData, Movement, MovementVelocity, NetworkObjectType,
...
use crate::ecs::{Coord, GameContext, GameWorld, MapLoader, MapUtils};
...
impl MapUpdateSystem {
    ...
    fn do_update(world: &mut GameWorld) -> GameResult {
        ...
        if let Some(new_path) = map_path {
            ...
            // 保存相机位置和状态
            let (camera_pos, camera_zoom, screen_width, screen_height) = world
            ...
            match MapReader::new(&new_path) {
                Ok(reader) => {
                    ...
                    let (spawn_x, spawn_y) =
                        Coord::grid_to_world_center(spawn_grid_x, spawn_grid_y);
    ...
    pub fn trigger_map_selection(world: &mut World) {
            ...
            .pick_file()
        {
            ...
            // 创建新的地图切换请求
            world.spawn((MapSwitchRequest { map_path: path_str },));
        }
    }
