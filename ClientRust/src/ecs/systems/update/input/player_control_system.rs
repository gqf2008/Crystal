...
use crate::ecs::components::{
    GlobalEvents, MouseEvent, KeyboardEvent,
...
impl PlayerControlSystem {
    ...
    /// 处理鼠标事件并更新内部状态
    fn process_mouse_events(&mut self, events: &[MouseEvent]) {
        for event in events {
            match event {
                        ...
                        MouseButton::Left => {
                            ...
                            if let Some(last_click) = self.mouse_state.left_last_click_time {
                                if now.duration_since(last_click) < self.double_click_threshold {
                                    ...
                                    // 双击事件将在 update 中处理
                                    self.mouse_state.left_last_click_time = None; // 防止三击
                                } else {
    ...
    fn process_player_input(
        ...
        // 1. 处理鼠标事件
        self.process_mouse_events(&events.mouse_events);
        ...
        let (camera_pos, camera) = world.query_mut::<(&Position, &Camera)>()
            ...
            .unwrap_or((
                ...
                Camera { zoom: 1.0, screen_width: 1280.0, screen_height: 720.0 }
            ));
        ...
        for (_entity, (_, player_input, player)) in world.query_mut::<(
            ...
            // 4. 检测双击 → 移动命令（使用寻路）
            if let Some((screen_x, screen_y)) = self.detect_double_click(MouseButton::Left)
                ...
                tracing::info!("🖱️ 双击移动（寻路）: 世界坐标({:.1}, {:.1}), 跑步={}", 
                    world_x, world_y, is_running);
            }
            // 5. 检测长按 → 跟随命令（不使用寻路）
            else if let Some((screen_x, screen_y)) = self.detect_long_press(MouseButton::Left)
...
impl System for PlayerControlSystem {
    ...
    fn update(&mut self, world: &mut hecs::World, _delay_time: f32) -> GameResult {
        ...
        let single_click_command = if let Some((screen_x, screen_y)) = self.detect_single_click(MouseButton::Left) {
            let (world_x, world_y) = Self::screen_to_world(screen_x, screen_y, &camera_pos, &camera);
        ...
        if let Some(command) = single_click_command {
            if let Some((_, events)) = world.query::<&GlobalEvents>().iter().next() {
                events.send_network_command(command);
            }
        }
        ...
        for (_entity, (_, player_input, player)) in world.query_mut::<(
            ...
            // 检测长按 → 跟随命令（不使用寻路）
                ...
                tracing::info!("🖱️ 长按跟随: 世界坐标({:.1}, {:.1}), 跑步={}", 
                    world_x, world_y, is_running);
            }
        }
...
mod tests {
    ...
    fn test_player_control_system_creation() {
        ...
        assert_eq!(system.priority(), crate::ecs::systems::priority::PLAYER_CONTROL);
    ...
    fn test_screen_to_world_conversion() {
        let camera_pos = Position { x: 1000.0, y: 500.0 };
        let camera = Camera { zoom: 1.0, screen_width: 1280.0, screen_height: 720.0 };
        
        // 中心点应该等于相机位置
        let (world_x, world_y) = PlayerControlSystem::screen_to_world(640.0, 360.0, &camera_pos, &camera);
