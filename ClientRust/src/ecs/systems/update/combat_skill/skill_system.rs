...
impl SkillSystem {
    ...
    fn calculate_direction_to_target(world: &World, target_id: u32) -> MirDirection {
        ...
        let target_pos = {
            ...
            // 检查玩家
            if pos.is_none() {
                for (_, (_, net_sync, target_pos)) in world.query::<(&Player, &NetworkSync, &Position)>().iter() {
                    if net_sync.object_id == target_id {
                        pos = Some((target_pos.x, target_pos.y));
                        break;
                    }
            ...
            // 检查NPC
            if pos.is_none() {
                for (_, (_, net_sync, target_pos)) in world.query::<(&NPC, &NetworkSync, &Position)>().iter() {
                    if net_sync.object_id == target_id {
                        pos = Some((target_pos.x, target_pos.y));
                        break;
                    }
    ...
    /// 计算朝向指定位置的方向
    fn calculate_direction_to_location(world: &World, target_x: i32, target_y: i32) -> MirDirection {
        let player_pos = {
            let mut pos = None;
            for (_, (_, player_pos)) in world.query::<(&LocalPlayer, &Position)>().iter() {
                pos = Some((player_pos.x, player_pos.y));
