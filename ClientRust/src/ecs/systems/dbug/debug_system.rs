...
impl DebugSystem {
    ...
    fn draw_player_footprint(
        ...
        use crate::ecs::components::{LocalPlayer, MovementVelocity, Player};
        use crate::ecs::{CELL_HEIGHT, CELL_WIDTH};
            ...
            .iter()
        {
            ...
            let foot_screen_x =
                (player_foot_x - camera_pos.x) * camera.zoom + camera.screen_width / 2.0;
            let foot_screen_y =
                (player_foot_y - camera_pos.y) * camera.zoom + camera.screen_height / 2.0;
            ...
            let debug_text = format!(
                "Grid: ({}, {})\nPos: ({:.1}, {:.1})\nVel: ({:.1}, {:.1})\nAction: {:?}",
                grid_x, grid_y, player_foot_x, player_foot_y, velocity.x, velocity.y, player.action
            );
            ...
            canvas.draw(
                &text,
                DrawParam::default().dest([text_x + 5.0, text_y + 5.0]),
            );
            ...
            for (_, map) in world.query::<&MapData>().iter() {
                if grid_x >= 0 && grid_y >= 0 && grid_x < map.width && grid_y < map.height {
                    if (grid_x as usize) < map.cells.len()
                        && (grid_y as usize) < map.cells[grid_x as usize].len()
                    {
...
impl RenderSystem for DebugSystem {
    fn update(&mut self, ctx: &mut GameContext, _dt: f32) -> GameResult {
        ...
        // 查询 InputState 组件进行边缘检测
        // 先收集所有刚按下的按键，避免借用冲突
        let pressed_keys: Vec<KeyCode> = {
            let mut query = ctx.world.query::<&InputState>();
            if let Some((_, state)) = query.iter().next() {
                keys_to_check
                    .iter()
                    .filter(|&&key| ctx.input().key_just_pressed(key, state))
                    .copied()
                    .collect()
            } else {
                Vec::new()
