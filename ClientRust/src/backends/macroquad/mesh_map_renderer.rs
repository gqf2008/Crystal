        ...
        for y in start_y..end_y {
            ...
            for x in start_x..end_x {
                ...
                if let Some((file_index, base_image_index)) = cell.front_tile() {
                    // front_tile()返回 (front_index, (front_image & 0x7FFF) - 1)
                    let mut image_index = base_image_index;
                    
                    // 检查动画 - 参考C# GameScene.cs:11928-11942
                    // C#逻辑：
                    // animation = FrontAnimationFrame;
                    // if ((animation & 0x80) > 0) {
                    //     blend = true;
                    //     animation &= 0x7F;  // ← blend=true时才& 0x7F
                    // }
                    // if (animation > 0) { /* 使用修改后的animation */ }
                    
                    let mut animation = cell.front_animation_frame;
                    let use_blend = if (animation & 0x80) > 0 {
                        animation &= 0x7F;  // 修改animation值
                        true
                    } else {
                        false
                    };
