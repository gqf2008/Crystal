...
impl MeshMapRenderer {
    ...
    pub fn render(
        ...
        _show_texture_border: bool,
    ) -> u32 {
        // 计算视口范围
        let half_width = (viewport_width / 2.0) / zoom;
        let half_height = (viewport_height / 2.0) / zoom;
        
        let view_left = camera_x - half_width;
        let view_right = camera_x + half_width;
        let view_top = camera_y - half_height;
        let view_bottom = camera_y + half_height;
        ...
        for y in (start_y_even..end_y).step_by(2) {
            ...
            for x in (start_x_even..end_x).step_by(2) {
                ...
                if let Some((file_index, image_index)) = cell.back_tile() {
                    ...
                    if let Some(texture) = library_manager.get_or_create_texture(&lib_name, image_index as usize) {
                        // Back瓦片从偶数坐标开始绘制
