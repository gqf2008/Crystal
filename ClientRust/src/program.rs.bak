...
use crate::scenes::{LoginScene, Scene, SceneManager, SceneType, SelectScene};
...
impl CrystalGame {
    ...
    pub fn process_network_events(&mut self) {
        while let Ok(event) = self.event_rx.try_recv() {
            ...
            match &event {
                ...
                crate::network::game_client::GameEvent::MapInformation {
                    map_index,
                    ref file_name,
                    ref title,
                } => {
    ...
    pub fn check_login_to_select_transition(&mut self, ctx: &mut ggez::Context) {
        ...
        unsafe {
            CHECK_COUNTER += 1;
            if CHECK_COUNTER % 120 == 1 {
                // 每2秒打印一次(60fps)
                tracing::debug!(
                    "🔍 检查场景切换: ready_for_character_select={:?}, should_switch={}",
                    ready_flag,
        ...
        if should_switch_to_game {
            ...
            if let Err(e) = scene_manager.switch_scene(SceneType::Game) {
                ...
                if let Some((map_index, file_name, title)) = cached_map {
                    ...
                    tracing::info!(
                        "🔄 Resending cached MapInformation to GameScene: {} ({})",
                        title,
                        file_name
                    );
                ...
                if let Some(ref user_info) = self.cached_user_info {
                    ...
                    tracing::info!(
                        "🔄 Resending cached UserInformation to GameScene: {}",
                        user_info.name
                    );
    ...
    fn load_select_scene_textures(&mut self, ctx: &mut ggez::Context) {
        ...
        for i in 2426..=2440 {
            // 2426-2428 (战士), 2429-2431 (法师), 2432-2434 (道士), 2435-2437 (刺客), 2438-2440 (弓箭手)
            textures_to_load.push((LibraryName::Prguse, i));
        }

        // 性别按钮 (男、女)
        for i in 2420..=2425 {
            // 2420-2422 (男), 2423-2425 (女)
            textures_to_load.push((LibraryName::Prguse, i));
        }
...
impl ggez::event::EventHandler for CrystalGame {
    ...
    fn mouse_button_down_event(
        ...
        let scene_button = match button {
            ...
            ggez::input::mouse::MouseButton::Back | ggez::input::mouse::MouseButton::Forward => {
                SceneMouseButton::Other(0)
            }
