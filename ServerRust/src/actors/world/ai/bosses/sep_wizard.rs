...
impl Default for SepWizardBehavior {
    fn default() -> Self {
        Self::new()
    }
}
        ...
        if dist <= VIEW_RANGE && ctx.tick_count >= monster.next_attack_tick {
            ...
            if ctx.tick_count >= self.next_repulsion_tick {
                ...
                let pushed: Vec<(u64, u8)> = nearby
                    ...
                    .collect();
                if !pushed.is_empty() {
                    self.next_repulsion_tick =
                        ctx.tick_count + 100 + fastrand::i32(0..20) as u64 * 10; // 10-30s
