...
impl MonsterBehavior for VampireSpiderBehavior {
    ...
    fn on_die(&mut self, monster: &mut MonsterState, ctx: &mut AiCtx) {
        ...
        if let Some(master) = monster.master_session {
            ...
            let die_targets: Vec<crate::actors::world::ai::PlayerSnap> = ctx
                .find_targets_in_range(monster.x, monster.y, EXPLOSION_RADIUS, monster.map_index)
                .into_iter()
                .copied()
                .collect();
            if !die_targets.is_empty() {
                ctx.out_player_heals
                    .push((master, per.saturating_mul(die_targets.len() as i32)));
