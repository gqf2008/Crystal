...
//! 机制：
//!   - 近战（MACAgility），命中后 8% 施加绿毒（5s，tick 2000）
//!   - Die：1 格内 AOE 伤害（ACAgility）+ 5% 绿毒（C# Effect==1 时，此处近似恒生效）
...
impl MonsterBehavior for ToxicGhoulBehavior {
    fn process_tick(&mut self, monster: &mut MonsterState, ctx: &mut AiCtx) {
        ...
        if dist <= 1 && ctx.tick_count >= monster.next_attack_tick {
            ...
            // C# CompleteAttack：命中后 8% 绿毒（5s，tick 2000）
            if fastrand::i32(0..100) < 8 {
                ctx.out_poisons.push(crate::actors::world::ai::PoisonPlayer {
                    session_id: target.session_id,
                    poison: crate::combat::poison::Poison::new(
                        mir2_shared::enums::PoisonType::GREEN, 5, 0, 2000),
                });
            }
            return;
        }
    ...
    /// C# Die/CompleteDeath：1 格内 AOE 伤害 + 5% 绿毒
