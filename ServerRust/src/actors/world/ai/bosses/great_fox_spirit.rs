    ...
    fn process_tick(&mut self, monster: &mut MonsterState, ctx: &mut AiCtx) {
        ...
        if dist > 3 && fastrand::i32(0..10) == 0 && ctx.tick_count >= self.next_recall_tick {
            ...
            let targets: Vec<crate::actors::world::ai::PlayerSnap> = ctx
                .find_targets_in_range(monster.x, monster.y, 30, monster.map_index)
                .into_iter()
                .copied()
                .collect();
            for t in targets {
                ...
                if td > 3 {
                    ...
                    ctx.out_player_teleports
                        .push((t.session_id, nx, ny, dir as u8));
                    break; // C# 一次只传送一个
                }
            }
        }
        ...
        let hits: Vec<crate::actors::world::ai::PlayerSnap> = ctx
            .find_targets_in_range(monster.x, monster.y, radius, monster.map_index)
            .into_iter()
            .copied()
            .collect();
        for h in hits {
            ...
            if ranged {
                ctx.out_effects.push((
                    h.object_id,
                    mir2_shared::enums::SpellEffect::GreatFoxSpirit,
                    0,
                    0,
                ));
            }
