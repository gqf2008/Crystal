...
impl MonsterBehavior for FlameSpearBehavior {
    fn process_tick(&mut self, monster: &mut MonsterState, ctx: &mut AiCtx) {
        ...
        // C# InAttackRange：4 格内 (x==0)||(y==0)||(x==y)
        let in_range = dx <= ATTACK_RANGE && dy <= ATTACK_RANGE && (dx == 0 || dy == 0 || dx == dy);
