...
impl MonsterBehavior for KirinBehavior {
    fn process_tick(&mut self, monster: &mut MonsterState, ctx: &mut AiCtx) {
        ...
        if ctx.tick_count >= monster.next_move_tick {
            let (nx, ny, dir) =
                if dist <= MELEE_RANGE && monster.x == target.x && monster.y == target.y {
...
impl KirinBehavior {
    ...
    fn ice_thrust(
        ...
        target: crate::actors::world::ai::PlayerSnap,
        ctx: &mut AiCtx,
    ) {
        let damage =
        ...
        let hit: Vec<u64> = ctx
            .find_all_targets_in_cells(&cells, monster.map_index)
            .iter()
            .map(|p| p.session_id)
            .collect();
        for sid in hit {
