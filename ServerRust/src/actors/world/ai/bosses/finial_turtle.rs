//! FinialTurtle（终结龟）behavior
//!
//! C# 参考：Server/MirObjects/Monsters/FinialTurtle.cs
//! 机制：AttackRange=6；近战 DC / 远程 MC（MACAgility）；
//!      远程命中后 1/8 减速毒（15s，tick 1000）+ 1/15 冰冻毒（5s，tick 1000）
...
impl MonsterBehavior for FinialTurtleBehavior {
    fn process_tick(&mut self, monster: &mut MonsterState, ctx: &mut AiCtx) {
        ...
        if dist <= ATTACK_RANGE && ctx.tick_count >= monster.next_attack_tick {
            ...
            if dist <= 1 {
                ...
                // C# CompleteRangeAttack：1/8 Slow(15s) + 1/15 Frozen(5s)
