...
impl WorldActor {
    ...
    pub(crate) async fn apply_spell_pvp_hits(
        ...
        let Some(record) = self.players.get(&caster_session) else {
            return;
        };
        let Ok(Some(caster_state)) = record.actor_ref.ask(GetPlayerState).await else {
            return;
        };
        ...
        for &target_session in target_sessions {
            let Some(other_actor) = self.players.get(&target_session) else {
                continue;
            };
            let Ok(Some(other_state)) = other_actor.actor_ref.ask(GetPlayerState).await else {
                continue;
            };
            if other_state.is_dead {
                continue;
            }
            if self.gm_protected.contains(&target_session) {
                continue;
            }
            // 攻击模式/行会战（C# IsAttackTarget）
            if !can_attack_player(&caster_state, &other_state, &self.guild_wars) {
                continue;
            }
            // 安全区：双方任一在安全区则禁止伤害
            ...
            if damage > 0 {
                ...
                if attack_result.reflected > 0 {
                    ...
                    debug!(
                        ...
                        target_session, attack_result.reflected, caster_session
                    );
                }
                ...
                if attack_result.defender_heal > 0 {
                    let _ = other_actor
                        .actor_ref
                        .ask(crate::actors::player::Heal {
                            amount: attack_result.defender_heal,
                        })
                        .await;
                }
                let _ = other_actor
                ...
                self.broadcast_pvp_hit(
                    other_state.object_id,
                    caster_state.object_id,
                    other_state.x,
                    other_state.y,
                    other_state.direction,
                    damage,
                    other_state.map_index,
...
pub(crate) fn range_attack_out_of_range(
    caster_x: i32,
    caster_y: i32,
    target_x: i32,
    target_y: i32,
...
impl WorldActor {
        ...
        for d in 0..=1i32 {
            for y in (front_y - d)..=(front_y + d) {
                for x in (front_x - d)..=(front_x + d) {
                    if let Some((oid, c)) = self
                        .corpses
                        .iter()
                        .find(|(_, c)| c.map_index == state.map_index && c.x == x && c.y == y)
                    {
    ...
    async fn harvest_corpse(&mut self, session_id: u64, corpse_id: u32) {
        let Some((x, y, map_index, direction)) = self
            .corpses
            .get(&corpse_id)
            .map(|c| (c.x, c.y, c.map_index, c.direction))
        else {
            return;
        };
        let remaining_skins = self
            .corpses
            .get(&corpse_id)
            .map(|c| c.remaining_skins)
            .unwrap_or(0);
        if remaining_skins > 0 {
            ...
            if new_remaining == 0 {
                ...
                if empty {
                    ...
                    self.broadcast_object_harvested(corpse_id, x, y, map_index, direction)
                        .await;
                    self.corpses.remove(&corpse_id);
                    let remove_packet = Self::build_object_remove_packet(corpse_id);
                    broadcast_to_map(&self.gate_ref, &self.players, map_index, &remove_packet)
                        .await;
                }
            } else {
                if let Some(c) = self.corpses.get_mut(&corpse_id) {
        ...
        loop {
            let next = self
                .corpses
                .get(&corpse_id)
                .and_then(|c| c.drops.first().cloned());
    ...
    async fn grant_item_to_player(
        ...
        let can = record
            .actor_ref
            .ask(crate::actors::player::CanGainItems)
            .await
            .unwrap_or(false);
        if !can {
            return false;
    ...
    async fn roll_harvest_drops_for_corpse(
        &mut self,
        corpse_id: u32,
    ) -> Vec<mir2_shared::data::item::UserItem> {
        let Some(corpse) = self.corpses.get(&corpse_id).cloned() else {
            return Vec::new();
        };
        let Some(drops) = self.monster_drops.get(&corpse.monster_index).cloned() else {
            return Vec::new();
        };
...
pub(crate) fn mine_drop_succeeds(
    drop_rate: i32,
    mine_rate_percent: i32,
    guild_mine_rate: i32,
    roll: i32,
) -> bool {
    ...
    async fn handle(
        ...
        if let Some(ref state) = attacker_state {
            ...
            let atk_spd_bonus = crate::combat::buff::get_stat_bonus(
                &state.buffs,
                &crate::combat::buff::BuffType::AttackSpeedBoost { percent: 0 },
            );
            ...
            let total_atk_spd = ((state.attack_speed + atk_spd_bonus) * (100 - curse_pct) / 100)
                * (100 + state.attack_speed_rate_percent)
                / 100;
            ...
            self.player_logout_block_ms
                .insert(msg.session_id, now_ms + LOGOUT_DELAY_MS);
            ...
            if !self.slaying_armed.contains(&msg.session_id) {
                if let Some(lv) =
                    find_attack_skill(&state.magics, SPELL_SLAYING).map(|m| m.level as i32)
                {
        ...
        if let (Some(ref state), Ok(Some(result))) = (
            ...
            for (oid, monster) in &mut self.monsters {
                ...
                if dist == 0 && monster.behavior.is_attackable() {
                    ...
                    let mut raw_damage = combat_attack::get_attack_power(
                        attacker_stats.min_atk,
                        attacker_stats.max_atk,
                        attacker_stats.luck,
                    );
                    ...
                    if hemorrhage_armed {
                        if let Some(magic) = state
                            .magics
                            .iter()
                            .find(|m| m.spell == (SPELL_HEMORRHAGE as i32 - 3))
                        {
                            let lv = magic.level as i32;
                            raw_damage =
                                ((raw_damage as f32) * (0.2 + 0.05 * lv as f32)).max(1.0) as i32;
                        }
                    }
                    ...
                    if let Some((expire, lv, kind)) =
                        self.double_hit_melee.get(&msg.session_id).copied()
                    {
                    ...
                    if let Some(magic) = state
                        .magics
                        .iter()
                        .find(|m| m.spell == (SPELL_FATAL_SWORD as i32 - 3))
                    {
                    ...
                    if let Some(magic) = state
                        .magics
                        .iter()
                        .find(|m| m.spell == (SPELL_MPEATER as i32 - 3))
                    {
                        let lv = magic.level as i32;
                        ...
                        *count += add;
                        debug!(
                            "Player {} MPEater count={} (add={})",
                            result.object_id, *count, add
                        );
                        if *count >= 100 {
                            ...
                            let _ = record
                                .actor_ref
                                .ask(crate::actors::player::AddMP { amount: add_mp })
                                .await;
                            *count = 0;
                    ...
                    let percent =
                        ((monster.hp.max(0) as f32 / monster.max_hp as f32) * 100.0) as u8;
                ...
                for (cx, cy) in &halfmoon_cells {
                    ...
                    if let Some(mid) = mid {
                        if let Some(sm) = self.monsters.get_mut(&mid) {
                            ...
                            sm.register_hit(msg.session_id, self.tick_count);
                            self.pending_gather.push(msg.session_id);
                            // #1990：C# 溅射命中同样经 Attacked → HPDrain（按溅射净伤害近似）
                            let drain = ((splash_dmg as f32 * state.hp_drain_rate_percent as f32
                                / 100.0)
                                .floor() as i32)
                                .max(0);
                            if drain > 0 {
                                if let Some(r) = self.players.get(&msg.session_id) {
                                    let _ = r
                                        .actor_ref
                                        .ask(crate::actors::player::Heal { amount: drain })
                                        .await;
                                }
                            }
                            sm.provoked = true;
            ...
            if hit_monster {
                if let Some(record) = self.players.get(&msg.session_id) {
                    ...
                    if broke {
                        ...
                        if let Some(state) =
                            self.recalculate_and_set_stat_bonuses(msg.session_id).await
                        {
            ...
            if hit_monster {
                ...
                if mp_eater_triggered {
                    self.grant_attack_skill_exp(msg.session_id, SPELL_MPEATER)
                        .await;
                }
                if hemorrhage_triggered {
                    self.grant_attack_skill_exp(msg.session_id, SPELL_HEMORRHAGE)
                        .await;
                }
            }
            ...
            if !hit_monster {
                for (other_actor, other_session) in others {
                    ...
                    if let Ok(Some(other_state)) = other_actor.ask(GetPlayerState).await {
                        ...
                        if front_hit {
                                ...
                                if self.tick_count < expire {
                                        ...
                                        && fastrand::i32(0..40) <= lv as i32 + 1
                                    {
                                        ...
                                        let _ = other_actor
                                            ...
                                            .await;
                                        debug!(
                                            "Player {} TwinDrakeBlade stunned player {}",
                                            result.object_id, other_session
                                        );
                                    }
                                    debug!(
                                        "Player {} {} second hit +{} on player {}",
                                        result.object_id,
                                        if kind == 0 {
                                            "TwinDrakeBlade"
                                        } else {
                            ...
                            if let Some((expire, lv)) =
                                self.flaming_sword.get(&msg.session_id).copied()
                            {
                                self.flaming_sword.remove(&msg.session_id);
                                if self.tick_count < expire {
                                    ...
                                    self.damage_armor_on_pvp_hit(other_session).await;
                                    debug!(
                                        "Player {} FlamingSword bonus +{} on player {}",
                                        result.object_id, bonus, other_session
                                    );
                                }
                            }
                            ...
                            if fatal_armed {
                                    ...
                                    .find(|m| m.spell == (SPELL_FATAL_SWORD as i32 - 3))
                                {
                                    ...
                                    self.damage_armor_on_pvp_hit(other_session).await;
                                    debug!(
                                        "Player {} FatalSword bonus +{} on player {}",
                                        result.object_id, bonus, other_session
                                    );
                                }
                            } else if state
                                .magics
                                .iter()
                                .any(|m| m.spell == (SPELL_FATAL_SWORD as i32 - 3))
                                && fastrand::i32(0..10) == 0
                            {
                                self.fatal_sword_armed.insert(msg.session_id);
                                debug!("Player {} FatalSword armed", result.object_id);
                            ...
                            if let Some((skill_shared, lv)) = halfmoon_pvp {
                                ...
                                if is_halfmoon {
                                    for k in 0..4usize {
                                        ...
                                        cells.push((
                                            state.x + MON_DIR_DX[d],
                                            state.y + MON_DIR_DY[d],
                                        ));
                                    }
                                } else {
                                    for d in 0..8usize {
                                ...
                                for sid in &splash_hits {
                                    if let Some(r) = self.players.get(sid) {
                                        ...
                                        let drain = ((splash_dmg as f32
                                            * state.hp_drain_rate_percent as f32
                                            / 100.0)
                                            .floor()
                                            as i32)
                                            .max(0);
                                ...
                                debug!(
                                    "Player {} {} PvP splash dmg={} on {} players",
                                    result.object_id,
                                    if skill_shared == SPELL_HALFMOON {
                                        "HalfMoon"
                                    } else {
                                        "CrossHalfMoon"
                                    },
                            ...
                            if let Some((expire, lv)) =
                                self.counter_attack.get(&other_session).copied()
                            {
                                if self.tick_count <= expire {
                                    ...
                                    let _ = record
                                        ...
                                        .await;
                                    debug!(
                                        "Player {} counter-attacked player {} ({} dmg)",
                            ...
                            if attack_result.reflected > 0 {
                                ...
                                debug!(
                                    "Player {} reflected {} to player {}",
                                    other_session, attack_result.reflected, msg.session_id
                                );
                            }
                            ...
                            if pvp_died {
                                ...
                                let victim_brown =
                                    crate::actors::world::is_brown(other_state.brown_until_ms);
                                ...
                                if at_war || victim_war_zone {
                                    ...
                                    let _ = record
                                        .actor_ref
                                        .ask(crate::actors::player::AddPkPoints { points: 100 })
                                        .await;
                                    ...
                                    if let Ok(Some(weapon)) = record
                                        ...
                                        .await
                                    {
                                        if weapon.added_stats.get(mir2_shared::enums::Stat::Luck)
                                            > -(crate::combat::attack::max_luck())
                                            && fastrand::i32(..4) == 0
                                        {
                                            // #2424：C# Settings.MaxLuck
                                            let _ = record
                                                .actor_ref
                                                .ask(crate::actors::player::AddWeaponLuck {
                                                    delta: -1,
                                                })
                                                .await;
                                            send_system_message(
                                            ...
                                            debug!(
                                                "Weapon cursed on player kill: {} -> {}",
                                                record.name, weapon.item_index
                                            );
                                        }
                                    }
                                }
                            ...
                            let hit_dist =
                                (other_state.x - result.x).abs() + (other_state.y - result.y).abs();
                            debug!(
                                "Hit! {} damaged {} for {} (dist={}, crit={})",
                                result.object_id,
                                other_state.name,
                                damage,
                                hit_dist,
                ...
                for (_other_actor, other_session) in &self
                    .players
                    .iter()
...
impl Message<HarvestRequest> for WorldActor {
    ...
    async fn handle(
        ...
        let in_mine_zone = map_info
            .mine_zones
            .iter()
            .any(|z| (target_x - z.x).abs() <= z.size && (target_y - z.y).abs() <= z.size);
        if !in_mine_zone {
        ...
        if hit {
            ...
            let rubble_oid = if let Some(existing) = self.spell_objects.values_mut().find(|so| {
                ...
                self.spell_objects.insert(
                    oid,
                    spell::SpellObject::new(
                        oid,
            ...
            if drops.is_empty()
                || !mine_drop_succeeds(
                    drop_rate,
                    state.mine_rate_percent,
                    state.guild_buff_mine_rate_percent,
                    fastrand::i32(0..100),
                )
            {
        ...
        debug!(
            "Harvest: {} mine_index={} spot({},{}) hit={} msg={}",
...
impl Message<TownReviveRequest> for WorldActor {
    ...
    async fn handle(&mut self, msg: TownReviveRequest, _ctx: &mut Context<Self, Self::Reply>) {
        ...
        let _ = record
            .actor_ref
            .ask(crate::actors::player::RevivePlayer {
                x: spawn_x,
                y: spawn_y,
...
impl Message<RangeAttackRequest> for WorldActor {
    ...
    async fn handle(&mut self, msg: RangeAttackRequest, _ctx: &mut Context<Self, Self::Reply>) {
        ...
        let ranged_chance =
            ranged_chance_to_hit(attack_dist, focus, self.setup_cfg.range_accuracy_bonus);
        ...
        let _ = self
            ...
            .tell(SendToClient {
                session_id: msg.session_id,
                data: build_packet_bytes(
                    mir2_shared::enums::ServerPacketIds::RangeAttack as i16,
                    &proj_body,
                ),
            })
            .await;
        ...
        if let Some(monster_level) = targeted_monster_level {
            ...
            for other in &others {
                if let Ok(Some(other_state)) = other.actor_ref.ask(GetPlayerState).await {
                    ...
                    if self.gm_protected.contains(&other.session_id) {
                        continue;
                    }
                    if !can_attack_player(&state, &other_state, &self.guild_wars) {
                        continue;
...
impl WorldActor {
    ...
    pub(crate) async fn resolve_ranged_pvp_hit(
        ...
        let attacker_safe = self
            .maps
            .get(&attacker_state.map_index)
            .map(|m| m.is_safe_zone(attacker_state.x, attacker_state.y))
            .unwrap_or(false);
        let target_safe = self
            .maps
            .get(&defender_state.map_index)
        ...
        if self
            .map_infos
            .get(&(attacker_state.map_index as i32))
            .map(|mi| mi.no_fight)
            .unwrap_or(false)
            || self
                .map_infos
                .get(&(defender_state.map_index as i32))
                .map(|mi| mi.no_fight)
                .unwrap_or(false)
        {
            return false;
        ...
        if damage > 0 {
            self.broadcast_pvp_hit(
                defender_state.object_id,
                attacker_object_id,
                defender_state.x,
                defender_state.y,
                defender_state.direction,
                damage,
                defender_state.map_index,
                attack_result.is_critical,
            )
            .await;
        }
        ...
        if attack_result.reflected > 0 {
            ...
            debug!(
                "Player {} reflected {} to player {}",
                defender_session, attack_result.reflected, attacker_session
            );
        }
        // 吸血：攻击方 HPDrainRatePercent 回血（:7175-7183）
        if attack_result.hp_drain > 0 {
        ...
        if pvp_died {
            ...
            if at_war || victim_war_zone {
                ...
                // C# Die：击杀玩家 1/4 概率诅咒武器（Luck -1，Luck > -MaxLuck 时）
                if let Ok(Some(weapon)) = attacker_record
                    .actor_ref
                    .ask(crate::actors::player::GetEquipmentInfo {
                        slot: crate::actors::inventory::EquipmentSlot::Weapon,
                    })
                    .await
                {
                        ...
                        && fastrand::i32(..4) == 0
                    {
                        ...
                        let _ = attacker_record
                            .actor_ref
                            .ask(crate::actors::player::AddWeaponLuck { delta: -1 })
                            .await;
                        send_system_message(
                            &self.gate_ref,
                            attacker_session,
                        ...
                        debug!(
                            "Weapon cursed on player kill: {} -> {}",
                            attacker_record.name, weapon.item_index
                        );
                    }
                }
            }
            ...
            if let Some(r) = self.players.get_mut(&attacker_session) {
                if let Ok(Some(attacker_state2)) =
                    attacker_record.actor_ref.ask(GetPlayerState).await
                {
        ...
        debug!(
            "RangeAttack PvP resolve: {} damaged {} for {}",
            attacker_state.name, defender_state.name, damage
        );
        true
    }
}
...
fn nearest_k_chebyshev(
    items: &[(u32, i32, i32)],
    cx: i32,
    cy: i32,
    k: usize,
) -> Vec<(u32, i32, i32)> {
    let mut v: Vec<(u32, i32, i32, i32)> = items
        .iter()
...
impl Message<MagicRequest> for WorldActor {
    ...
    async fn handle(&mut self, mut msg: MagicRequest, _ctx: &mut Context<Self, Self::Reply>) {
        ...
        let spell_oid = if needs_spell_obj {
            Some(self.alloc_object_id())
        } else {
            None
        };
        ...
        if !basic_spells.contains(&spell_cs)
            && !state.magics.iter().any(|m| m.spell == spell_cs as i32)
        {
        ...
        if obj_spell.write_body(&mut ob).is_ok() {
            let pkt =
                build_packet_bytes(mir2_shared::enums::ServerPacketIds::ObjectSpell as i16, &ob);
            for (sid, r) in &self.players {
                if *sid == msg.session_id {
                    continue;
                }
        ...
        if let Some(spell_info) = spell_db {
            ...
            if last_cast > 0 && (now_ms - last_cast) < delay_ms as i64 {
                ...
                send_system_message(
                    &self.gate_ref,
                    msg.session_id,
                    &format!("技能冷却中，还需 {} 秒", remaining / 1000),
                );
                return;
            }
        }
        ...
        let mp_cost = {
            let base = spell_db
                .map(|m| crate::combat::magic::magic_cost(m, spell_level))
                .unwrap_or(5);
        ...
        if spell_enum == mir2_shared::enums::Spell::ExplosiveTrap {
            let active = self
                ...
                .values()
                .filter(|so| {
                    so.spell == mir2_shared::enums::Spell::ExplosiveTrap
        ...
        if meditation_lv > 0 && fastrand::i32(0..8) < meditation_lv as i32 {
            let _ = record
                .actor_ref
                .ask(crate::actors::player::AddMP {
                    amount: mp_cost as i32,
                })
                .await;
            send_system_message(
                &self.gate_ref,
                msg.session_id,
                &format!("冥想恢复 {} 魔法值", mp_cost),
            );
        ...
        if magic_cast.write_body(&mut cast_body).is_ok() {
            let _ = self
                ...
                .tell(SendToClient {
                    session_id: msg.session_id,
                    data: build_packet_bytes(
                        mir2_shared::enums::ServerPacketIds::MagicCast as i16,
                        &cast_body,
                    ),
                })
                .await;
        ...
        let is_persistent = matches!(
            spell_enum,
            mir2_shared::enums::Spell::FireWall
                | mir2_shared::enums::Spell::Blizzard
                | mir2_shared::enums::Spell::MeteorStrike
                | mir2_shared::enums::Spell::PoisonCloud
                | mir2_shared::enums::Spell::HealingCircle
                | mir2_shared::enums::Spell::ExplosiveTrap
                | mir2_shared::enums::Spell::Portal
                | mir2_shared::enums::Spell::DelayedExplosion
        );
        ...
        if let Some(mut spell_obj) = persistent_spell {
            ...
            if spell_obj.spell == mir2_shared::enums::Spell::DelayedExplosion {
                ...
                spell_obj.target_id = if msg.target_id != 0 {
                    Some(msg.target_id)
                } else {
                    None
                };
            ...
            if object_spell.write_body(&mut os_body).is_ok() {
                ...
                let session_ids: Vec<u64> = std::iter::once(msg.session_id)
                    .chain(
                        self.same_map_players(msg.session_id, state.map_index)
                            .await
                            .iter()
                            .map(|p| p.session_id),
                    )
        ...
        match msg.spell {
            ...
            SPELL_HEALING | SPELL_MASS_HEALING => {
                ...
                if msg.spell == SPELL_HEALING {
                    ...
                    debug!(
                        "Magic: {} casts Healing on session {} (+{} HP)",
                        state.name, target_session, amount
                    );
                } else {
                    let cx = if target_x == 0 && target_y == 0 {
                        state.x
                    } else {
                        target_x
                    };
                    let cy = if target_x == 0 && target_y == 0 {
                        state.y
                    } else {
                        target_y
                    };
                    ...
                    debug!(
                        "Magic: {} casts MassHealing (3x3, healed {} players, +{} HP)",
            ...
            SPELL_HEALING_CIRCLE => {
                debug!(
                    "Magic: {} casts HealingCircle (persistent field)",
                    state.name
                );
            }
            // --- Buff 类 ---
            ...
            SPELL_MAGIC_SHIELD => {
                ...
                let _ = record
                    .actor_ref
                    .ask(crate::actors::player::ApplyDamageReduction {
                    ...
                    .await;
                debug!(
                    "Magic: {} casts MagicShield (damage -{}%)",
                    state.name, reduction_pct
                );
            }
            ...
            SPELL_POISONING => {
                ...
                if let Some(mid) = mid {
                    if let Some(monster) = self.monsters.get_mut(&mid) {
                        ...
                        debug!(
                            "Magic: {} casts Poisoning -> monster {} ({}s, {}dmg/tick)",
                            state.name, mid, duration, poison_value
                        );
                    }
                } else {
                    debug!(
                        "Magic: {} casts Poisoning (no target near {},{})",
                        state.name, target_x, target_y
                    );
                }
            }
            ...
            SPELL_TRAP_HEXAGON => {
                ...
                debug!(
                    "Magic: {} casts TrapHexagon (trapped {} monsters, {}s)",
                    state.name, trapped_count, duration
                );
            }
            // --- 道士 Buff/辅助类 ---
            ...
            SPELL_MASS_HIDING => {
                ...
                let duration_ticks =
                    ((sc_power / 2 + (spell_level as i32 + 1) * 2).max(1) as u32) * 10;
            ...
            SPELL_PURIFICATION => {
                ...
                debug!(
                    "Magic: {} casts Purification on session {} (success)",
                    state.name, target_session
                );
            }
            ...
            SPELL_ENTRAPMENT => {
                ...
                debug!(
                    "Magic: {} casts Entrapment -> monster {} pulled {} tiles ({}s paralysis)",
            ...
            SPELL_SHOULDER_DASH => {
                ...
                for _ in 0..dist {
                    ...
                    // C#：路径上等级 < 施法等级的目标才推送
                    let hit: Option<(u32, i32)> = self
                        .monsters
                        .iter()
                        .find(|(_, m)| {
                            m.map_index == state.map_index && m.x == nx && m.y == ny && m.hp > 0
                        })
                        .map(|(id, m)| {
                            (
                                *id,
                ...
                if travelled > 0 {
                    let _ = record
                        ...
                        .await;
                    self.broadcast_player_dash(msg.session_id, new_x, new_y, msg.direction)
                        .await;
                } else {
                    ...
                    if fail.write_body(&mut body).is_ok() {
                        let _ = self
                            ...
                            .tell(SendToClient {
                                session_id: msg.session_id,
                                data: build_packet_bytes(
                                    mir2_shared::enums::ServerPacketIds::UserDashFail as i16,
                                    &body,
                                ),
                            })
                            .await;
                    ...
                    if ofail.write_body(&mut obody).is_ok() {
                        let data = build_packet_bytes(
                            mir2_shared::enums::ServerPacketIds::ObjectDashFail as i16,
                            &obody,
                        );
                        broadcast_to_map(&self.gate_ref, &self.players, state.map_index, &data)
                            .await;
                    }
                    let msg_text = if self
                        .maps
                        .get(&state.map_index)
                        .map(|m| m.is_safe_zone(state.x, state.y))
                        .unwrap_or(false)
                ...
                debug!(
                    "Magic: {} casts ShoulderDash (dashed to {},{}, pushed {} monsters)",
            ...
            SPELL_THRUSTING => {
                ...
                let raw_damage =
                    ((raw as f32) * (0.25 + 0.25 * spell_level as f32)).max(1.0) as i32;
                let mut cx = state.x;
                let mut cy = state.y;
                for _ in 0..2 {
                    cx += MON_DIR_DX[dir];
                    ...
                    let hit = self
                        ...
                        .iter()
                        .find(|(_, m)| {
                            m.map_index == state.map_index && m.x == cx && m.y == cy && m.hp > 0
                        })
                        .map(|(id, _)| *id);
                    if let Some(mid) = hit {
                        if let Some(m) = self.monsters.get_mut(&mid) {
                            ...
                            if r.is_hit && r.damage > 0 {
                                ...
                                m.register_hit(msg.session_id, self.tick_count);
                                self.pending_gather.push(msg.session_id);
                                m.provoked = true;
            ...
            SPELL_TELEPORT => {
                ...
                if let Some((rx, ry)) = dest {
                    ...
                    let _ = record
                        ...
                        .await;
                    debug!(
                        "Magic: {} MagicTeleport to bind map {} ({},{})",
                        state.name, bind_map, rx, ry
                    );
                } else {
            ...
            SPELL_BLINK | SPELL_STORM_ESCAPE => {
                ...
                let _ = record
                    ...
                    .await;
                self.broadcast_player_teleport(
                    msg.session_id,
                    state.x,
                    state.y,
                    tx,
            ...
            | SPELL_METEOR_SHOWER => {
                ...
                let delay_ms = match msg.spell {
                    ...
                    | SPELL_METEOR_SHOWER => target_dist * 50 + 500,
            ...
            SPELL_FIRE_BOUNCE => {
                ...
                debug!(
                    "Magic: {} casts FireBounce dmg={} bounce={} delay={}ms",
                    state.name,
                    raw_damage,
                    spell_level as i32 + 2,
                    delay_ms
                );
            }
            // --- 即时 AoE 类法术（任务4）---
            ...
            SPELL_FIREBANG | SPELL_ICE_STORM => {
                ...
                let player_hit_ids: Vec<u64> = {
                    ...
                    for (sid, r) in &self.players {
                        ...
                        if let Ok(Some(ps)) = r.actor_ref.ask(GetPlayerState).await {
                            if ps.is_dead || ps.map_index != state.map_index {
                                continue;
                            }
                ...
                debug!(
                    "Magic: {} casts FireBang/IceStorm (3x3) dmg={}",
                    state.name, raw_damage
                );
            }
            ...
            SPELL_LIGHTNING => {
                ...
                let mut player_hit_ids: Vec<u64> = Vec::new();
                {
                    ...
                    for _ in 0..6 {
                        ...
                        if let Some(sid) = first {
                            if !player_hit_ids.contains(&sid) {
                                player_hit_ids.push(sid);
                            }
                        }
                    }
                }
                ...
                debug!(
                    "Magic: {} casts Lightning (line 6) dmg={}",
                    state.name, raw_damage
                );
            }
            ...
            SPELL_THUNDERSTORM | SPELL_FLAME_FIELD => {
                ...
                debug!(
                    "Magic: {} casts ThunderStorm/FlameField (5x5) dmg={}",
                    state.name, raw_damage
                );
            }
            ...
            SPELL_HELLFIRE => {
                ...
                let hit_ids: Vec<u32> = self
                    ...
                    .iter()
                    .filter(|(_, m)| {
                        m.hp > 0 && m.map_index == state.map_index && cells.contains(&(m.x, m.y))
                    })
                    .map(|(id, _)| *id)
                    .collect();
                ...
                debug!(
                    "Magic: {} casts HellFire ({} cells) dmg={} hits={}",
                    state.name,
                    cells.len(),
                    raw_damage,
                    spell_hits.len()
            ...
            SPELL_ICETHRUST => {
                ...
                self.broadcast_spell_hit(&spell_hits, object_id).await;
                debug!(
                    "Magic: {} casts IceThrust dmg={} hits={}",
                    state.name,
                    raw_damage,
                    spell_hits.len()
                );
            ...
            SPELL_CURSE => {
                ...
                if !record
                    .actor_ref
                    .ask(crate::actors::player::ConsumeAmuletForSummon { amount: 1 })
                    .await
                    .unwrap_or(false)
                {
                ...
                for (sid, os) in player_targets {
                    ...
                    if !can_attack_player(&state, &os, &self.guild_wars) {
                        continue;
                    }
                    ...
                    if let Some(record2) = self.players.get(&sid) {
                        ...
                        let _ = record2
                            .actor_ref
                            .ask(crate::actors::player::SetPlayerState { state: ns })
                            .await;
                    }
                }
                debug!(
                    "Magic: {} casts Curse (7x7, monsters={} players={}, rate={}%)",
            ...
            | SPELL_CAT_TONGUE | SPELL_VAMPIRE_SHOT | SPELL_POISON_SHOT | SPELL_CRIPPLE_SHOT
            | SPELL_ELEMENTAL_SHOT => {
                ...
                if msg.spell == SPELL_ELEMENTAL_SHOT {
                    if !state.has_elemental {
                        ...
                        debug!(
                            "Magic: {} casts ElementalShot without orbs -> gather orb",
                            state.name
                        );
                        return;
                    }
                ...
                debug!(
                    "Magic: {} casts Archer projectile spell={} dmg={} delay={}ms (DoubleShot={})",
                    state.name,
                    msg.spell,
                    raw_damage,
                    base_delay_ms,
                    msg.spell == SPELL_DOUBLE_SHOT
                );
            }
            ...
            SPELL_CONCENTRATION => {
                ...
                self.broadcast_set_concentration(state.object_id, true, false, state.map_index)
                    .await;
                debug!(
                    "Magic: {} casts Concentration (MP regen +{}, {}s)",
                    state.name,
                    bonus,
                    45 + 15 * spell_level as i32
                );
            }
            ...
            SPELL_ELEMENTAL_BARRIER => {
                ...
                if state.buffs.iter().any(|b| {
                    ...
                    debug!(
                        "Magic: {} ElementalBarrier already active, skip",
                        state.name
                    );
                    return;
                }
                ...
                if !state.has_elemental {
                    self.obtain_element(msg.session_id, true).await;
                    debug!(
                        "Magic: {} casts ElementalBarrier without orbs -> gather orb",
                        state.name
                    );
                    return;
                }
                ...
                self.broadcast_object_effect(
                    state.object_id,
                    mir2_shared::enums::SpellEffect::ElementalBarrierUp,
                    0,
                    0,
                    state.map_index,
                )
                .await;
                debug!(
                    "Magic: {} casts ElementalBarrier (damage -{}%, {}s, orb +{}s)",
                    state.name,
                    reduction_pct,
                    mc_power + barrier_power,
                    barrier_power
                );
            }
            ...
            SPELL_MIRRORING => {
                ...
                let existing: Option<u32> = self
                    ...
                    .find(|(_, m)| {
                        m.master_session == Some(msg.session_id)
                            && m.name.eq_ignore_ascii_case(CLONE_NAME)
                            && m.hp > 0
                    })
                    .map(|(id, _)| *id);
                if let Some(oid) = existing {
                    ...
                    debug!(
                        "Magic: {} Mirroring removed existing clone #{}",
                        state.name, oid
                    );
                    return;
                }
                ...
                let mon_index = self
                    .monster_name_index
                    .get(CLONE_NAME.to_lowercase().as_str())
                    .copied();
                match mon_index {
                    Some(idx) => {
                        if let Some(info) = self.monster_infos.get(&idx).cloned() {
                            ...
                            let min_mc = info
                                .stats
                                .get(&(mir2_shared::enums::Stat::MinMC as u8))
                                .copied()
                                .unwrap_or(0);
                            let max_mc = info
                                .stats
                                .get(&(mir2_shared::enums::Stat::MaxMC as u8))
                                .copied()
                                .unwrap_or(0);
                            ...
                            debug!(
                                "Magic: {} casts Mirroring -> clone #{} at ({},{})",
                                state.name, new_oid, sx, sy
                            );
                        } else {
                            warn!(
                                "Mirroring '{}' found index {} but no MonsterInfo",
                                CLONE_NAME, idx
                            );
                        }
                    }
                    None => {
                        warn!(
                            "Mirroring '{}' not in monster_name_index (DB may lack this mob)",
                            CLONE_NAME
                        );
                    }
                }
            ...
            SPELL_HASTE => {
                ...
                let _ = record
                    ...
                    .await;
                debug!(
                    "Magic: {} casts Haste (attack speed +{}%, {}s)",
                    state.name,
                    pct,
                    25 + spell_level as i32 * 15
                );
            }
            ...
            SPELL_LIGHT_BODY => {
                ...
                let _ = record
                    .actor_ref
                    .ask(crate::actors::player::ApplyBuff { buff: buff1 })
                    .await;
                debug!(
                    "Magic: {} casts LightBody (agility +{}, {}s)",
                    state.name,
                    agi_bonus,
                    (spell_level as i32 + 1) * 30
                );
            }
            ...
            SPELL_FURY => {
                ...
                let _ = record
                    ...
                    .await;
                debug!(
                    "Magic: {} casts Fury (attack speed +4, {}s)",
                    state.name,
                    60 + spell_level as i32 * 10
                );
            }
            // Rage：DC 提升（C# HumanObject.cs Rage：MaxDC/MinDC += round(MaxDC*(0.12+0.03Lv))，18+6Lv 秒）
            SPELL_RAGE => {
                let add_value =
                    (state.max_attack as f32 * (0.12 + 0.03 * spell_level as f32)).round() as i32;
                let buff = crate::combat::buff::BuffInstance::new(
                    crate::combat::buff::BuffType::AttackBoost {
                        bonus: add_value.max(1),
                    },
                ...
                let _ = record
                    ...
                    .await;
                debug!(
                    "Magic: {} casts Rage (DC +{}, {}s)",
                    state.name,
                    add_value.max(1),
                    18 + spell_level as i32 * 6
                );
            }
            // SwiftFeet：移动速度大幅提升
            SPELL_SWIFT_FEET => {
                ...
                let buff = crate::combat::buff::BuffInstance::new(
                    crate::combat::buff::BuffType::MoveSpeedBoost { percent: spd_pct },
                    250 + spell_level as u32 * 50,
                    5,
                );
                let _ = record
                    .actor_ref
                    ...
                    .await;
                debug!(
                    "Magic: {} casts SwiftFeet (move speed +{}%)",
                    state.name, spd_pct
                );
            }
            ...
            SPELL_DARK_BODY => {
                ...
                if let Some(oid) = existing {
                    ...
                    debug!(
                        "Magic: {} DarkBody removed existing clone #{}",
                        state.name, oid
                    );
                    return;
                }
                ...
                match mon_index {
                        ...
                        if let Some(info) = self.monster_infos.get(&idx).cloned() {
                            ...
                            self.monsters.insert(
                                ...
                                MonsterState {
                                    ...
                                    sit_down_tick: 0,
                                    target_session,
                                    provoked: target_session.is_some(),
                                    last_hitter_session: None,
                                    exp_owner_session: None,
                                    exp_owner_tick: 0,
                                    pending_brown_attacker: None,
                    ...
                    None => {
                        warn!(
                            "DarkBody '{}' not in monster_name_index (DB may lack this mob)",
                            CLONE_NAME
                        );
                    }
                }
            ...
            SPELL_HEAVENLY_SWORD => {
                ...
                for _ in 0..3 {
                    cx += MON_DIR_DX[dir];
                    cy += MON_DIR_DY[dir];
                    if let Some((&mid, _)) = self.monsters.iter().find(|(_, m)| {
                        m.map_index == state.map_index && m.x == cx && m.y == cy && m.hp > 0
                    }) {
                        hit_ids.push(mid);
                    }
                ...
                for mid in hit_ids {
                    if let Some(m) = self.monsters.get_mut(&mid) {
                        ...
                        // #1455：LevelOffset = Level > attacker.Level ? 0 : min(10, attacker.Level - Level)
                        let level_offset =
                            crate::combat::attack::level_offset(state.level, m.level.max(0) as u16);
                        let r = combat_attack::resolve_attack(
                            &attacker_stats,
                            &ds,
                            raw_damage,
            ...
            SPELL_BLADE_AVALANCHE => {
                ...
                for col_dir in [prev, dir, next] {
                    ...
                    for j in 0..3i32 {
                        ...
                        let cell_dmg = if j <= 1 {
                            ...
                            ((raw as f64) * 0.6) as i32
                        };
                        let hit_ids: Vec<u32> = self
                            .monsters
                            .iter()
                            .filter(|(_, m)| {
                                m.map_index == state.map_index && m.x == hx && m.y == hy && m.hp > 0
                            })
                            .map(|(id, _)| *id)
                            .collect();
                        for mid in hit_ids {
                ...
                debug!(
                    "Magic: {} casts BladeAvalanche (3x3 front, hits {})",
                    state.name, hit_count
                );
            }
            ...
            SPELL_CRESCENT_SLASH => {
                ...
                for fd in fan_dirs {
                    ...
                    if let Some((&mid, _)) = self.monsters.iter().find(|(_, m)| {
                        m.map_index == state.map_index && m.x == tx && m.y == ty && m.hp > 0
                    }) {
                        hit_ids.push(mid);
                    }
                }
            ...
            SPELL_FLASH_DASH => {
                ...
                if travel > 0 {
                    let _ = record
                        ...
                        .await;
                    self.broadcast_player_dash_attack(
                        msg.session_id,
                        cx,
                        cy,
                        msg.direction,
                    ...
                    self.broadcast_player_object_attack(
                        msg.session_id,
                        state.x,
                        state.y,
                        msg.direction,
                ...
                for mid in hit_ids {
                    if let Some(monster) = self.monsters.get_mut(&mid) {
                        ...
                        let r = combat_attack::resolve_attack(
                            ...
                            &ds,
                            raw,
                            mir2_shared::enums::DefenceType::Ac,
                            level_offset,
                        );
                        if r.is_hit && r.damage > 0 {
                            monster.take_damage(r.damage);
                ...
                if hit_count > 0 {
                    self.grant_attack_skill_exp(msg.session_id, SPELL_FLASH_DASH)
                        .await;
                }
                ...
                debug!(
                    "Magic: {} casts FlashDash (travel={} hit={})",
                    state.name, travel, hit_count
                );
            }
                ...
                if travel > 0 {
                    let _ = record
                        ...
                        .ask(crate::actors::player::SetPlayerPosition {
                            x: cx,
                            y: cy,
                            direction: back_dir as u8,
                            map_index: None,
                            is_mounted: None,
                        })
                        .await;
                    self.broadcast_player_backstep(msg.session_id, cx, cy, back_dir as u8, travel)
                        .await;
                    ...
                    self.broadcast_player_backstep(
                        msg.session_id,
                        state.x,
                        state.y,
                        back_dir as u8,
            ...
            | SPELL_SUMMON_SHINSU
            | SPELL_SUMMON_HOLY_DEVA
            | SPELL_SUMMON_VAMPIRE
            | SPELL_SUMMON_TOAD
            | SPELL_SUMMON_SNAKES => {
                ...
                let existing: Option<u32> = self
                    ...
                    .find(|(_, m)| {
                        m.master_session == Some(msg.session_id)
                            && m.name.eq_ignore_ascii_case(summon_name)
                            && m.hp > 0
                    })
                    .map(|(id, _)| *id);
                if let Some(oid) = existing {
                    if let Some(m) = self.monsters.get_mut(&oid) {
                        ...
                        broadcast_to_map(&self.gate_ref, &self.players, m.map_index, &walk_packet)
                            .await;
                        debug!(
                            "Magic: {} recalls existing summon '{}' #{}",
                            state.name, summon_name, oid
                        );
                    }
                    return;
                }
                ...
                if matches!(
                    ...
                    let ok = record
                        .actor_ref
                        .ask(crate::actors::player::ConsumeAmuletForSummon {
                ...
                match mon_index {
                    Some(idx) => {
                        ...
                        if let Some(info) = info_opt {
                            ...
                            let base_hp = info
                                ...
                                .copied()
                                .unwrap_or(50);
                            let base_min_dmg = info
                                .stats
                                .get(&(mir2_shared::enums::Stat::MinDC as u8))
                                .copied()
                                .unwrap_or(5);
                            let base_max_dmg = info
                                .stats
                                .get(&(mir2_shared::enums::Stat::MaxDC as u8))
                                .copied()
                                .unwrap_or(10);
                            ...
                            self.monsters.insert(
                                ...
                                MonsterState {
                                    ...
                                    recall_at_tick: if recall_at_tick > 0 {
                                        self.tick_count + recall_at_tick
                                    } else {
                                        0
                                    },
                                    can_recall: false,
                            ...
                            if let Some(m) = self.monsters.get_mut(&new_oid) {
                                ...
                                if matches!(
                                    ...
                                    m.ai_profile.move_interval =
                                        m.ai_profile.move_interval.saturating_sub(move_save).max(1);
                                    m.ai_profile.attack_cooldown = m
                                        .ai_profile
                                        .attack_cooldown
                                        .saturating_sub(atk_save)
                                        .max(1);
                                }
                            }
                            ...
                            self.pet_levels.insert(new_oid, spell_level as i32);
                            debug!(
                                "Magic: {} casts summon '{}' as #{} at ({},{}) (slave of {})",
                                state.name, summon_name, new_oid, sx, sy, msg.session_id
                            );
                        } else {
                            warn!(
                                "Summon '{}' found index {} but no MonsterInfo (DB missing mob)",
                                summon_name, idx
                            );
                            send_system_message(
                                &self.gate_ref,
                    ...
                    None => {
                        warn!(
                            "Summon '{}' not in monster_name_index (DB may lack this mob)",
                            summon_name
                        );
                ...
                match mon_index {
                    Some(idx) => {
                        ...
                        if let Some(info) = info_opt {
                            ...
                            let display_name = format!("{}({})", spawn.name, state.name);
                            let packet =
                                build_object_monster_packet(&spawn, new_oid, &display_name);
                            broadcast_to_map(
                                &self.gate_ref,
                                &self.players,
                                state.map_index,
                            ...
                            debug!(
                                "Magic: {} casts Stonetrap '{}' as #{} at ({},{}) ({}s)",
                                state.name,
                                STONE_NAME,
                                new_oid,
                                sx,
                                sy,
                                spell_level as u64 * 5 + 10
                            );
                        } else {
                            warn!(
                                "Stonetrap '{}' found index {} but no MonsterInfo (DB missing mob)",
                                STONE_NAME, idx
                            );
                            send_system_message(
                                &self.gate_ref,
                    ...
                    None => {
                        warn!(
                            "Stonetrap '{}' not in monster_name_index (DB may lack this mob)",
                            STONE_NAME
                        );
                        send_system_message(&self.gate_ref, msg.session_id, "召唤失败：未知怪物");
                    }
            ...
            SPELL_LION_ROAR => {
                ...
                for mid in hit_ids {
                    ...
                    if state.level as i32 + 3 < mon_level {
                        continue;
                    }
                    if let Some(monster) = self.monsters.get_mut(&mid) {
                        monster.apply_poison_defended(crate::combat::poison::Poison::new(
                ...
                debug!(
                    "Magic: {} casts LionRoar (LRParalysis {} monsters)",
            ...
            SPELL_BATTLE_CRY => {
                ...
                debug!(
                    "Magic: {} casts BattleCry (taunted {} monsters, roll={} threshold={})",
            ...
            SPELL_PROTECTION_FIELD => {
                let add_value =
                    (state.max_ac as f32 * (0.2 + 0.03 * spell_level as f32)).round() as i32;
                ...
                let _ = record
                    ...
                    .await;
                debug!(
                    "Magic: {} casts ProtectionField (AC +{}, {}s)",
                    state.name,
                    add_value.max(1),
                    45 + spell_level as i32 * 15
                );
            }
            ...
            SPELL_COUNTER_ATTACK => {
                ...
                if let Some((expire, _)) = self.counter_attack.get(&msg.session_id).copied() {
                    if self.tick_count < expire {
                        debug!(
                            "Magic: {} casts CounterAttack but already active",
                            state.name
                        );
                        return;
                    }
                ...
                self.counter_attack
                    .insert(msg.session_id, (self.tick_count + 70, spell_level));
            ...
            SPELL_TURN_UNDEAD => {
                ...
                debug!(
                    "Magic: {} casts TurnUndead (killed {} undead)",
                    state.name, killed
                );
            }
            ...
            SPELL_TWIN_DRAKE_BLADE => {
                self.double_hit_melee
                    .insert(msg.session_id, (self.tick_count + 100, spell_level, 0));
                debug!(
                    "Magic: {} casts TwinDrakeBlade (next melee double-hit, 10s)",
                    state.name
                );
            }
            ...
            SPELL_DOUBLE_SLASH => {
                self.double_hit_melee
                    .insert(msg.session_id, (self.tick_count + 100, spell_level, 1));
                debug!(
                    "Magic: {} casts DoubleSlash (next melee double-hit, 10s)",
                    state.name
                );
            }
            ...
            SPELL_SLASHING_BURST => {
                ...
                let raw_damage =
                    ((raw as f32) * (3.25 + 0.25 * spell_level as f32)).max(1.0) as i32;
                let mut new_x = state.x;
                let mut new_y = state.y;
                ...
                debug!(
                    "Magic: {} casts SlashingBurst (dashed to {},{}, dealt {} dmg)",
            ...
            SPELL_PLAGUE => {
                ...
                self.broadcast_spell_hit(&spell_hits, object_id).await;
                debug!(
                    "Magic: {} casts Plague (3x3, {} hit, dmg={})",
                    state.name,
                    spell_hits.len(),
                    damage
                );
            ...
            SPELL_TRAP => {
                ...
                if let Some((mid, mlevel)) = hit {
                    if mlevel >= state.level as i32 + 2 {
                        debug!(
                            "Magic: {} casts Trap -> monster {} level {} too high",
                            state.name, mid, mlevel
                        );
                        return;
                    }
                    if let Some(monster) = self.monsters.get_mut(&mid) {
                        ...
                        monster.target_session = Some(msg.session_id);
                        debug!(
                            "Magic: {} casts Trap -> monster {} paralyzed 60s",
                            state.name, mid
                        );
                    }
                } else {
                    debug!(
                        "Magic: {} casts Trap (no target at {},{})",
                        state.name, target_x, target_y
                    );
                }
            }
            ...
            SPELL_MOON_MIST => {
                ...
                self.broadcast_spell_hit(&spell_hits, object_id).await;
                debug!(
                    "Magic: {} casts MoonMist (invisible {}s + 5x5 AC dmg={} hits={})",
                    state.name,
                    duration_ticks / 10,
                    raw_damage,
                    spell_hits.len()
            ...
            SPELL_IMMORTAL_SKIN => {
                ...
                if dc_penalty > 0 {
                    ...
                    let _ = record
                        .actor_ref
                        .ask(crate::actors::player::ApplyBuff { buff: dc_buff })
                        .await;
                }
                debug!(
                    "Magic: {} casts ImmortalSkin (AC +{}, DC -{}, {}s)",
                    state.name,
                    ac_bonus.max(1),
                    dc_penalty,
                    60 + spell_level as i32
                );
            }
            ...
            SPELL_HALLUCINATION => {
                if hallucination_success(spell_level, state.level) {
                    ...
                    if let Some(mid) = hit {
                        ...
                        debug!(
                            "Magic: {} casts Hallucination -> monster {} confused {}s",
                            state.name, mid, dur
                        );
                    } else {
                        debug!(
                            "Magic: {} casts Hallucination (no target at {},{})",
                            state.name, target_x, target_y
                        );
                    }
                } else {
            ...
            SPELL_ONE_WITH_NATURE => {
                ...
                for mid in hit_ids {
                    if let Some(monster) = self.monsters.get_mut(&mid) {
                        ...
                        if r.is_hit && r.damage > 0 {
                            ...
                            if has_vamp {
                                vamp_total +=
                                    (raw_damage as f32 * (spell_level as f32 + 1.0) * 0.25) as i32;
                            }
                        }
                ...
                if has_vamp || has_poison {
                    let _ = record
                        .actor_ref
                        .ask(crate::actors::player::SetSpecialShotArmed { armed: 0 })
                        .await;
                }
                self.broadcast_spell_hit(&spell_hits, object_id).await;
                debug!(
                    "Magic: {} casts OneWithNature (5x5, {} hit, dmg={}, vamp={})",
                    state.name,
                    spell_hits.len(),
                    raw_damage,
                    vamp_total
            ...
            SPELL_ULTIMATE_ENHANCER => {
                ...
                {
                    debug!(
                        "Magic: {} casts UltimateEnhancer but has no amulet",
                        state.name
                    );
                    return;
                }
                ...
                let (buff, label) = match target_class {
                    mir2_shared::enums::MirClass::Wizard | mir2_shared::enums::MirClass::Archer => {
                        (
                            crate::combat::buff::BuffType::McBoost { bonus: value },
                            "MC",
                        )
                    }
                    mir2_shared::enums::MirClass::Taoist => (
                        crate::combat::buff::BuffType::ScBoost { bonus: value },
                        "SC",
                    ),
                    _ => (
                        crate::combat::buff::BuffType::AttackBoost { bonus: value },
                        "DC",
                    ),
                };
                ...
                if target_session == msg.session_id {
                    let _ = record
                        .actor_ref
                        .ask(crate::actors::player::ApplyBuff { buff: inst })
                        .await;
                ...
                debug!(
                    "Magic: {} casts UltimateEnhancer on session {} ({} +{}, {}s)",
                    state.name,
                    target_session,
                    label,
                    value,
            ...
            SPELL_FATAL_SWORD => {
                debug!(
                    "Magic: {} casts FatalSword (passive-only in C#, no active effect)",
                    state.name
                );
            }
            ...
            SPELL_PET_ENHANCER => {
                ...
                if let Some(pid) = pet {
                    ...
                    debug!(
                        "Magic: {} casts PetEnhancer -> pet {} (DC+{} AC+{}, {}s)",
                        state.name, pid, dc_inc, ac_inc, duration_s
                    );
                } else {
                    debug!(
                        "Magic: {} casts PetEnhancer (no pet near {},{})",
                        state.name, target_x, target_y
                    );
                }
            }
            ...
            SPELL_FLAMING_SWORD => {
                ...
                self.flaming_sword
                    .insert(msg.session_id, (self.tick_count + 100, spell_level));
                ...
                .await;
                debug!(
                    "Magic: {} casts FlamingSword (next melee +{:.2}x, 10s)",
                    state.name,
                    1.4 + 0.4 * spell_level as f32
                );
            }
            ...
            SPELL_ENERGY_SHIELD => {
                ...
                let _ = record
                    ...
                    .await;
                debug!(
                    "Magic: {} casts EnergyShield (damage -{}%, {}s)",
                    state.name,
                    percent,
                    30 + 50 * spell_level as i32
                );
            }
            ...
            SPELL_REPULSION | SPELL_ENERGY_REPULSOR | SPELL_FIRE_BURST => {
                ...
                for (id, m) in self.monsters.iter() {
                    ...
                    if m.map_index != state.map_index {
                        continue;
                    }
                    if m.hp <= 0 || m.master_session.is_some() {
                        continue;
                    }
                    let dx = m.x - state.x;
                    let dy = m.y - state.y;
                ...
                for (mid, pdir, start_x, start_y) in candidates {
                    ...
                    for _ in 0..push_range {
                        ...
                        if tx < 0 || ty < 0 || tx >= max_x || ty >= max_y {
                            break;
                        }
                        let walkable = self
                            .maps
            ...
            SPELL_ELECTRIC_SHOCK => {
                ...
                let target_mid: Option<u32> = self
                    .monsters
                    .iter()
                    .find(|(_, m)| {
                        let dist = (m.x - target_x).abs() + (m.y - target_y).abs();
                ...
                if let Some(mid) = target_mid {
                    ...
                    self.grant_electric_shock_exp(msg.session_id, msg.spell, now_ms, spell_cs)
                        .await;
                    electric_shock_exp_handled = true;
                    ...
                    if !can_tame {
                        ...
                        debug!(
                            "Magic: {} ElectricShock: monster {} not tamable",
                            state.name, mid
                        );
                        return;
                    }
                    ...
                    if fastrand::i32(0..2) > 0 {
                        ...
                        debug!(
                            "Magic: {} ElectricShock stunned monster {}",
                            state.name, mid
                        );
                        return;
                    }
                    ...
                    if target_level > state.level as i32 + 2 {
                        debug!(
                            "Magic: {} ElectricShock: target level {} > player {} + 2",
                            state.name, target_level, state.level
                        );
                        return;
                    }
                    ...
                    if fastrand::i32(0..(state.level as i32 + 20 + spell_level as i32 * 5))
                        <= target_level + 10
                    {
                        ...
                        debug!(
                            "Magic: {} ElectricShock: monster {} resisted tame",
                            state.name, mid
                        );
                        return;
                    }
                    ...
                    if pet_count >= spell_level as usize + pet_bonus {
                        debug!(
                            "Magic: {} ElectricShock: pet cap reached ({} >= {} + {})",
                            state.name, pet_count, spell_level, pet_bonus
                        );
                        return;
                    }
                    ...
                    if fastrand::i32(0..rate) != 0 {
                        debug!(
                            "Magic: {} ElectricShock: final tame roll failed (1/{})",
                            state.name, rate
                        );
                        return;
                    }
                    ...
                    if let Some(name) = tamed_name {
                        ...
                        debug!(
                            "Magic: {} casts ElectricShock (tamed monster {})",
                            state.name, mid
                        );
            ...
            SPELL_REVELATION => {
                ...
                if let Some(oid) = target_oid {
                    ...
                    broadcast_to_map(
                        &self.gate_ref,
                        &self.players,
                        self.monsters.get(&oid).map(|m| m.map_index).unwrap_or(0),
                        &pkt,
                    )
                    .await;
                    debug!(
                        "Magic: {} casts Revelation -> oid {} ({}s)",
                        state.name, oid, value
                    );
                } else {
                    debug!(
                        "Magic: {} casts Revelation (no target at {},{})",
                        state.name, target_x, target_y
                    );
                }
            }
            ...
            SPELL_POISON_SWORD => {
                ...
                debug!(
                    "Magic: {} casts PoisonSword (arc 5, poisoned {})",
                    state.name, poisoned
                );
            }
            // --- 默认：其他伤害类（接入战斗公式 MAC）---
            _ => {
                ...
                .max(1);
                let attacker_stats = state.to_combat_stats();
                let hit_monster_ids: Vec<u32> = self
                    .monsters
                    .iter()
                    .filter(|(_, m)| {
                        let dist = (m.x - target_x).abs() + (m.y - target_y).abs();
                ...
                for monster_id in hit_monster_ids {
                    if let Some(monster) = self.monsters.get_mut(&monster_id) {
                        ...
                        debug!(
                            "Magic: {} spell={} lv={} -> monster {} for {} dmg (crit={})",
...
mod tests {
    ...
    fn test_cast_disabled_by_poison() {
        ...
        assert!(cast_disabled_by_poison(&[Poison::new(
            PoisonType::STUN,
            5,
            0,
            1000
    ...
    fn test_attack_disabled_by_poison() {
        ...
        // 麻痹/冰冻/眩晕：禁攻
        assert!(attack_disabled_by_poison(&[Poison::new(
            PoisonType::PARALYSIS,
            5,
            0,
            1000
