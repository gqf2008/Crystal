...
pub use item::*;
pub use mail::*;
pub use map_sync::TeleportPlayerWithResync;
pub use market::*;
pub use npc::*;
pub use quest::*;
pub use session::*;
...
pub struct WorldActor {
    ...
    /// CounterAttack 反击窗口（session → (到期 tick, 等级)；C# 7s 窗口，受击消耗）
    pub(crate) counter_attack: HashMap<u64, (u64, u8)>,
    ...
    /// 怪物追击寻路缓存（oid -> 路径瓦片序列，不含起点；#1691 八方寻路）
                            ...
                            if let Ok(Some(mut state)) = record.actor_ref.ask(GetPlayerState).await
                            {
                                    ...
                                    || state.mp < state.effective_max_mp()
                                {
                                    ...
                                    if self
                                        ...
                                        .is_err()
                                    {
                                        warn!("gate mailbox full: SendToClient dropped (session={} packet=HealthChanged)", session_id);
                                    }
                                    send_system_message(
                                        &self.gate_ref,
                                        session_id,
                    ...
                    "TELEPORT" | "MOVE" => {
                        ...
                        if let Some(map_index) = target_map_index {
                            if let Some(record) = self.players.get(&session_id) {
                                if let Ok(Some(state)) = record.actor_ref.ask(GetPlayerState).await
                                {
                                    // 统一传送入口：跨图补 MapChanged/MapInformation/resync
                                    // 全量重发新图对象（此前只发 UserLocation，落图空图）
                                    map_sync::teleport_core(
                                        self,
                                        session_id,
                                        map_index,
                                        tx,
...
fn send_creature_list_packet(
    ...
    if gate_ref
        ...
        .is_err()
    {
        warn!("gate mailbox full: SendToClient dropped (session={} packet=UpdateIntelligentCreatureList)", session_id);
    }
}
...
impl Message<GmGotoRequest> for WorldActor {
    ...
    async fn handle(
        ...
        if let Some(mi) = self.map_infos.get(&(dest_map as i32)).cloned() {
            ...
            // 统一传送核心：SetPlayerPosition + 跨图 MapChanged/MapInformation/
            // UserLocation + resync 全量重发新图对象（此前缺 UserLocation，与
            // teleport_player 不同构；同图位移只发 UserLocation）
            map_sync::teleport_core(self, msg.session_id, dest_map, dest_x, dest_y, 4).await;
        }
        send_system_message(
            &self.gate_ref,
            msg.session_id,
