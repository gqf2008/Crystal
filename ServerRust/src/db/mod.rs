...
pub async fn init_db_pool(db_url: &str) -> anyhow::Result<DbPool> {
    ...
    sqlx::raw_sql(
        ...
        CREATE TABLE IF NOT EXISTS conquest_guards (
            ...
            FOREIGN KEY (conquest_idx) REFERENCES conquest_infos(idx)
        );
        CREATE INDEX IF NOT EXISTS idx_conquest_guards ON conquest_guards(conquest_idx);
        ...
        CREATE INDEX IF NOT EXISTS idx_conquest_gates ON conquest_gates(conquest_idx);
        ...
        CREATE INDEX IF NOT EXISTS idx_conquest_walls ON conquest_walls(conquest_idx);
        ...
        CREATE INDEX IF NOT EXISTS idx_conquest_sieges ON conquest_sieges(conquest_idx);
        ...
        CREATE INDEX IF NOT EXISTS idx_conquest_flags ON conquest_flags(conquest_idx);
        ...
        CREATE INDEX IF NOT EXISTS idx_conquest_control_points ON conquest_control_points(conquest_idx);
...
/// 加载攻城战配置（对齐 C# ConquestInfo(BinaryReader)）
/// 主表 + 子表（守卫/城门/城墙/攻城器械/旗帜/控制点）
pub async fn load_conquest_infos(pool: &DbPool) -> anyhow::Result<Vec<ConquestInfo>> {
    let rows = sqlx::query("SELECT * FROM conquest_infos ORDER BY idx").fetch_all(pool).await?;
    let mut result = Vec::new();
    for r in rows {
        let idx: i32 = r.get("idx");
        let extra_maps: Vec<i32> =
            serde_json::from_str(&r.get::<String, _>("extra_maps_json")).unwrap_or_default();

        // 子表查询
        let guards_rows = sqlx::query("SELECT * FROM conquest_guards WHERE conquest_idx = ? ORDER BY idx")
            .bind(idx).fetch_all(pool).await?;
        let guards = guards_rows.into_iter().map(|g| ConquestArcherInfo {
            index: g.get("idx"), x: g.get("x"), y: g.get("y"),
            mob_index: g.get("mob_index"), name: g.get("name"),
            repair_cost: g.get("repair_cost"),
        }).collect::<Vec<_>>();

        let gates_rows = sqlx::query("SELECT * FROM conquest_gates WHERE conquest_idx = ? ORDER BY idx")
            .bind(idx).fetch_all(pool).await?;
        let gates = gates_rows.into_iter().map(|g| ConquestGateInfo {
            index: g.get("idx"), x: g.get("x"), y: g.get("y"),
            mob_index: g.get("mob_index"), name: g.get("name"),
            repair_cost: g.get("repair_cost"),
        }).collect::<Vec<_>>();

        let walls_rows = sqlx::query("SELECT * FROM conquest_walls WHERE conquest_idx = ? ORDER BY idx")
            .bind(idx).fetch_all(pool).await?;
        let walls = walls_rows.into_iter().map(|g| ConquestWallInfo {
            index: g.get("idx"), x: g.get("x"), y: g.get("y"),
            mob_index: g.get("mob_index"), name: g.get("name"),
            repair_cost: g.get("repair_cost"),
        }).collect::<Vec<_>>();

        let sieges_rows = sqlx::query("SELECT * FROM conquest_sieges WHERE conquest_idx = ? ORDER BY idx")
            .bind(idx).fetch_all(pool).await?;
        let sieges = sieges_rows.into_iter().map(|g| ConquestSiegeInfo {
            index: g.get("idx"), x: g.get("x"), y: g.get("y"),
            mob_index: g.get("mob_index"), name: g.get("name"),
            repair_cost: g.get("repair_cost"),
        }).collect::<Vec<_>>();

        let flags_rows = sqlx::query("SELECT * FROM conquest_flags WHERE conquest_idx = ? ORDER BY idx")
            .bind(idx).fetch_all(pool).await?;
        let flags = flags_rows.into_iter().map(|g| ConquestFlagInfo {
            index: g.get("idx"), x: g.get("x"), y: g.get("y"),
            name: g.get("name"), file_name: g.get("file_name"),
        }).collect::<Vec<_>>();

        let cp_rows = sqlx::query("SELECT * FROM conquest_control_points WHERE conquest_idx = ? ORDER BY idx")
            .bind(idx).fetch_all(pool).await?;
        let control_points = cp_rows.into_iter().map(|g| ConquestFlagInfo {
            index: g.get("idx"), x: g.get("x"), y: g.get("y"),
            name: g.get("name"), file_name: g.get("file_name"),
        }).collect::<Vec<_>>();

        result.push(ConquestInfo {
            index: idx,
