...
### 5c-2h. **找到并修掉**主因：`monster_paths` 按 object_id 键、清理不删（2026-09-25 晚）

**怎么找到的**：上一条的"下一步"（活块登记表 + 年龄过滤 + 地址差集 + 内容识别）落地后一眼就看到了——
`materialize`/`cleanup` 区域里"熬过 ≥2 轮仍在"的新块（20 会话单轮约 **1132 个**）内容长这样：

```
size=344  hex=2c010000770100002c010000780100002c01000079010000...
          → (u32,u32) 配对： (300,375) (300,376) (300,377) ...   ← 同一"类型" + 连续 object_id
size=392  hex=55020000290200005602000029020000...
          → (597,553) (598,553) (599,553) ...
```

即**路径/格子坐标数组**（直线或斜线的一串相邻格）。再对着"必须随对象一起清"的清单核对：
脚本枚举世界 actor 里所有 `HashMap<u32/u64, …>` 字段并交叉查 `remove/clear/retain` 调用，
一眼看出 **`monster_paths` / `monster_path_targets` 从不清**（同批还有 `hero_paths` / `hero_path_targets` /
`hero_ai_states`，也是 session 键、也从不清）。它们正是 AI 寻路缓存的落点：

- monster 的**对象 id 由 `next_object_id` 单调发号、永不复用** ⇒ 每轮重新物化 = 每轮一批新 id，
  而 `cleanup_map_spawns` 只删 `monsters`/`npcs`，路径表里的条目和它背后的 `Vec` **永久留下**；
- 只有"真的寻过路"的怪才建条目 ⇒ 每轮 ~560 个（而不是 1912），每个 256–544B（32–68 格 × 8B）⇒ **~280KB/轮**；
- 这解释了此前所有对照：**0 怪 → 没有寻路 → 不涨**；**1 会话与 20 会话一样**（怪的行为不随会话数变）；
  **HoldSec ×5 不放大**（寻路是"发现目标时算一次"，不是每 tick 都算）。

**修法**（`actors/world/session.rs`）：

1. `cleanup_map_spawns` 在删每只怪的同一个循环里，把 `monster_paths` / `monster_path_targets` 的对应条目一起删；
2. 再加一层**兜底清扫**：怪物还会从死亡/回收等 12 处 `self.monsters.remove(...)` 消失，
   与其逐个改（容易再漏），在清理里按"当前世界里还有没有这个 object_id"`retain` 一遍；
3. 同型漏清一并修：登出/断线两条路径补 `hero_paths` / `hero_path_targets` / `hero_ai_states`（session 键）；
4. `MAP_PROBE` 补上这几张表的长度——**它们此前不在探针里，所以"看不见"**（这是上一轮"逐个排除"漏掉它们的直接原因）。

**判据（门禁）**：新增 `e2e_cleanup_forgets_object_id_keyed_monster_ai_cache`——
注入"真实怪 id + 一个过期 id"两条路径条目（断言读得到 (2,2)，证明判据不是空的），
调清理后必须回到 (0,0)。为了不被"登出 → 判空"的调度抖动带偏，清理体拆成
`cleanup_map_spawns`（带"地图上没人"前置）/`cleanup_map_spawns_force`（清理体），门禁直接调后者。
**阳性对照实做**：把兜底清扫改成 `retain(|_,_| true)` → 门禁立刻红（`left: (2,2) right: (0,0)`）。

**修复前后（`leak_plateau`，20 会话连登连退，release + mem-probe）**：

| | 每轮 live_bytes 净增 | `J4_plateau_not_leak` |
|---|---|---|
| 修复前 | **+474,418 / +326,081 / +693,526 …（≈ +450KB/轮，单调）** | **false（红）** |
| 修复后 | **+68,743 / +125,429 / +15,699 / −52,932（≈ 0，不再单调）** | **true（绿，RSS 斜率 0.41）** |

⇒ 这既是缺陷修复，也是这条门禁自 2026-09-25 以来第一次转绿。

**并把判据换成对的量（J5）**：RSS 上剩下的漂移属分配器高水位噪声（活跃字节已经不涨），
RSS 斜率实测在阈值 0.5 两边横跳（0.365–0.763），拿它判泄漏会时红时绿。所以给夹具加了
**J5：测量轮 idle「活跃字节」斜率 ≤ `-MaxLiveSlopePerCycleMb`（默认 0.1MB/轮）**——
它才是"有没有每轮线性泄漏"的判据；RSS 的 J4 保留作粗判（供默认构建用）。
J5 需要被测二进制带 `--features mem-probe` 且 `MIR2_LEAK_PROBE=1`；没开探针时 `J5=null`
（不参与 `ok`），但报告里会明确写「本轮**无法判真泄漏**」，不让"没测到"被读成"没问题"。

A/B（同一夹具同一参数，20 会话）：

| 被测二进制 | 活跃字节斜率 | `J5_live_bytes_plateau` | 退出码 |
|---|---|---|---|
| 修复后（master） | −147,620 B/轮 / +47,383 B/轮（两次） | **true** | 0 |
| **把守卫去掉的泄漏版**（两处 retain 恒真 + 不删缓存） | **+462,153 B/轮** | **false**（"存在每轮线性泄漏"） | **10** |

⇒ J5 能确定性抓住我们刚修掉的那条泄漏（RSS 版在同一次里也红，但它不是可靠判据）。

### 5c-2i. J5 的**定标没做完** ⇒ 判据换成 `dips`，并发版前一条命令（2026-09-25 晚·同轮）

上一条把判据换成 J5（活跃字节斜率 ≤ 0.1MB/轮）之后，**又拿"去掉守卫的泄漏版"复测了两轮**，
发现斜率在 3 轮的窗口里**分不开修复版与泄漏版**：

| 被测二进制 | 3 轮斜率 | 8 轮斜率 | `dips`（窗口内回落次数） | 旧判据（只看斜率） |
|---|---|---|---|---|
| 修复后（master） | −62,543 / +3,585 B | **+87,704 B**（12.15→12.33→12.41→12.47→12.47→**12.08**→12.76→12.76 MB） | **≥1** | true（绿） |
| **泄漏版**（同参数重建） | **+462,153 B** / **+93,626 B** | — | **0**（按定义只累加） | 前者红；**后者被静默放过（exit 0）** |

泄漏版的 +93,626 B **落在 0.1MB 阈值（104,857 B）以内**，而修复版的 +87,704 B 比它更大
——按斜率排，"泄漏"反而更小。所以**阈值化的斜率不是可靠判据**，本轮把它降为诊断量。

**新判据（已固化）**：看测量窗口内活跃字节**有没有回落**——

- `dips == 0`（窗口单调上升）⇒ **判泄漏** `exit 10`，**不看斜率是否在阈值内**（就是要堵上上面那个被放过的读数）；
- `dips > 0` ⇒ 通过 `exit 0`（斜率超阈值的那种在输出里标"回落解释"，并提示加长窗口复测）；
- 没跑出 J5（没带 `mem-probe`/没设 `MIR2_LEAK_PROBE`）⇒ `exit 3`「**没判成**」，不许当通过。

方向上是**偏严**的：3 轮窗口偶发单调（假红）的代价只是"加长窗口复测"，比放过真泄漏（假绿）小得多；
发版签字建议 `-MeasureCycles 6`~`8`。

**固化成的交付物**：`tools/ops/mem_leak_gate.ps1`（发版前**一条命令**：默认自己构建 `--features mem-probe`
的 release，再调 `leak_plateau`，再按上面的判据给结论；也接进 `docs/DELIVERY.md` 的"发版日关键路径"）。
它自带 `-SelfTest`：把本轮四个真实读数（+462,153 `/+93,626` 泄漏、+87,704 `/−62,543` 修复）
加一条"探针没生效"负例钉成判据象限 —— 不起服、秒级。

**验证（本轮实做）**：

- `pwsh tools/ops/mem_leak_gate.ps1 -SelfTest` → 6 例全过（三种判定 + 老报告偏严）。
- **阳性对照（去掉守卫应立红）**：把判据临时改回"只看斜率"跑同一组用例 →
  `+93,626（dips=0）` 与"老报告缺 dips 字段"两条**立刻红**，正是修复前会放过的那两个读数。
- **实机端到端**（10 会话 / 3 测量轮 / 复用 release 产物）：斜率 **+71,732 B**（在阈值内）、
  `dips=1` ⇒ **PASS exit 0**；同一夹具在同机、同参数下的物理 A/B（修复版 vs 去掉守卫的泄漏版）
  数据见上表（那一轮是判据定标的数据来源）。
- 端口不对齐（把 `-Port` 传成 7470 而部署目录里是 7450）时**硬失败** `exit 3`
  （`J0：第 1 轮会话失败 ok=0 failed=10 —— 标定无效，不产出报告`），不会静默给绿。

### 5c-2g. 三条对照把「每次物化 × 怪物数量」钉死；两条走不通的路（2026-09-25）

**四条对照（同夹具同二进制，只改一个参数/一份数据）**：

| 对照 | 结果 | 结论 |
|---|---|---|
| **1 会话** vs 20 会话（当前二进制复测） | 1 会话 +438,570 / +509,662 B 每轮；20 会话 +474,418 / +326,081 B 每轮 | 与会话数**无关**（不是按会话） |
| 空闲窗口 12s → 90s | 每轮仍 +460,658 / +488,385 B | 空闲期拉长**不放大** |
| **怪在线时长 8s → 40s**（`-HoldSec` ×5） | 每轮 +276,172 / +693,526 B（≈不变） | **不是按怪物 tick / 在线时长** |
| **怪数 1912 → 0**（删 `map_respawns` 地图 1 的 358 行） | 每轮 −41,488 / −54,416 B（≈不涨） | **与每轮生成多少只怪成正比** |

⇒ 增长是「**每次物化 × 怪物数量**」的函数：与"怪活了多久""有几个会话"都无关。
物化段一次分配约 1MB，约 2/3 在游玩+登出段被释放，**约 300KB/轮留下**（§5c-2e 的分段）。

**按标签的尺寸直方图**（新加 `MEM_PROBE_FOCUS`，焦点标签 = `materialize`）：该标签每轮
`dpos` 300–500KB、`dneg` −78KB…−257KB，净增忽大忽小；尺寸榜被 15–64KB 的**瞬态大缓冲**（各 1–2 个）
与 ~800B × 16–48 的 churn 主导 ⇒ **单看"哪一档在涨"仍然分不出泄漏**——泄漏是区间记账里的一小块残差。

**两条走不通的路（记下来，别重复）**：

1. **进程堆普查**：本想用 `HeapWalk` 在 idle 直接枚举"此刻真活着"的块（尺寸 + 内容开头）。
   实测 `HeapWalk(GetProcessHeap())` 只看到 **9 个 busy 块 / 5,904 B**（free 65MB）；
   换 UCRT 的 `_get_heap_handle()` 仍是同样的 9 个 ⇒ **Rust 默认 `System` 分配器的分配不在这两个堆里**，
   这条路在本工具链上不成立（探针代码已删除，只留这条结论）。
2. 用全局 `live_bytes` 的**窗口差**定位（会被其它线程的分配污染，见 §5c-2f 第 3 条）。

**下一步（唯一还没试、且有保证的路）**：在**我们自己的分配器里**做活块登记表——
alloc/dealloc 时登记 `ptr → (size, tag, 轮次)`（只记 `≥256B` 的块以控成本），
idle 时做存活普查 + 跨轮**地址差集**，直接列出"本轮新增且仍存活"的块，
并把最大的几个块的前 48 字节按 hex/ascii 打出来——用**内容**认（怪物名 / 包头 / 结构体正文）。

### 5c-2f. 泄漏与**怪物生成量**成正比（0 怪 → 不涨）；以及「线程局部标签遇 await 会失准」（2026-09-25）

**A/B：把地图 1 的刷怪点删掉（只留 43 个 NPC），其余参数不变**（改的是我自己的部署副本 DB，
`map_respawns` 358 行 → 0；跑完已还原）：

| 运行 | idle live_bytes（3 个采样点） | 每轮净增 |
|---|---|---|
| A：1912 只怪 | 16,718,435 → 17,192,853 → 17,518,934 | **+474,418 / +326,081 B** |
| B：**0 只怪** | 14,304,601 → 14,263,113 → 14,208,697 | **−41,488 / −54,416 B（≈不涨）** |

⇒ **每轮 +450KB 的保留与「每轮生成了多少只怪」成正比**（约 200 B/只/轮），与会话数无关、与时间无关
（§5c-2e 已排除两者）。A 比 B 在 idle 高约 3MB，其中约 1.6MB 可由 `MAP_CAP` 直接解释：
`size_of::<MonsterState>()=456`、清空后 `monsters_len=0` 但 `monsters_cap≈3542` ⇒
**表缓冲留在高水位（≈1.6MB）**，这是"一次性保留"而不是每轮泄漏。

**踩坑（已写成 LESSON）**：标签是线程局部的，而 `spawn_npcs_and_monsters(…).await` 之后 actor 任务
可能被调度到**另一条线程** ⇒ await 之后的整段（入库循环、精英广播、发包）全部记到 `unknown`
（实测：不重打标时 materialize 只显示 await 前那 1.9MB，每轮增长的 ~450KB 全落在 unknown/cleanup）。
**纠偏：每个 await 之后在当前线程重新打标**；重打标后 materialize 立刻显示
**+480,847 / +394,631 B/轮**，而 unknown 稳定。

**窗口测量也要用对量**：同步窗口（无 await）前后读 **`tag_live(TAG_MATERIALIZE)`** 的差，
才是"这段时间里本线程按该标签分配的净字节"；用全局 `live_bytes` 的窗口差会被**别的线程**的分配污染
（实测怪物入库循环那个窗口全局 +950KB，其中绝大部分是其它线程在跑）。实测入库窗口：
`tag_materialize` **+1,883,470 B**（首次建表，即 §上面的 1.6MB 缓冲）。

**下一步（未做）**：给标签加**按标签的尺寸直方图**（现在直方图是全局的），
把 materialize 标签上每轮保留的那 ~950 个 / ~400B 的对象钉到具体尺寸，再回代码里对结构；
或做 live 块登记表（alloc/dealloc 登记，idle 时按标签/尺寸做存活普查 + 跨轮地址差集）。
