...
impl Packet for ClientVersion {
    ...
    fn read_body<R: Read>(reader: &mut R) -> SharedResult<Self> {
        ...
        let length = usize::try_from(length).map_err(|_| crate::data::stats::SharedError::NegativeLength {
            field: "version_hash",
            length,
        })?;
    ...
    fn write_body<W: Write>(&self, writer: &mut W) -> SharedResult<()> {
        let length = i32::try_from(self.version_hash.len())
            .map_err(|_| crate::data::stats::SharedError::PacketTooLarge(self.version_hash.len()))?;
